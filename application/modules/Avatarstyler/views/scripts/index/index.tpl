...
<div class = container>
    ...
    <?php foreach($this->photoIDs as $photoID):?>
      <?php if($photoID && $photoID !=='null'){?>
        <div style="margin-top: 20px; position: relative; width: 200px; display:inline-block; height: 150px; background-image: url(<?php echo Engine_Api::_()->avatarstyler()->getLayer($photoID);?>);background-size: contain;background-repeat:no-repeat;background-position: center;">
<!--            <img src="--><?php //echo Engine_Api::_()->avatarstyler()->getLayer($photoID);?><!--" alt="phoyo" style="max-width: 100%;"/>-->
            <a href="javascript:void(0);" class="round_button hei hei-check-circle" onclick="test(<?php echo $photoID?>)" ><i class="fa fa-check"></i></a>
        </div>
      <?php } endforeach ?>
</div>
