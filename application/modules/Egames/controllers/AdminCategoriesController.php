 ...
 * @version    $Id: AdminCategoriesController.php 2021-08-19 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Egames_AdminCategoriesController extends Core_Controller_Action_Admin {
	 public function indexAction() {
		if(isset($_POST['selectDeleted']) && $_POST['selectDeleted']){
			if(isset($_POST['data'])  && is_array($_POST['data'])){
				$gamesTable = Engine_Api::_()->getDbtable('games', 'egames');
				$deleteCategoryIds = array();
				foreach($_POST['data'] as $key=>$valueSelectedcategory){
					$categoryDelete = Engine_Api::_()->getItem('egames_category', $valueSelectedcategory);
					$deleteCategory = Engine_Api::_()->getDbtable('categories', 'egames')->deleteCategory($categoryDelete);
					if($deleteCategory){
          				$deleteCategoryIds[] = $categoryDelete->category_id;
						$gamesTable->update(array(
							'category_id' => 0,
						), array(
		...
		if(isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1){
			...
			$slugExists = Engine_Api::_()->getDbtable('categories', 'egames')->slugExists($value['slug']);
			if(!$slugExists){
					//echo	json_encode(array('slugError'=>true));die;
			}
			if($cat_id != -1){
				$categoryData = Engine_Api::_()->getItem('egames_category', $cat_id);
				if($categoryData->subcat_id == 0 ){
					$value['subcat_id']=$cat_id;
					$seprator = '&nbsp;&nbsp;&nbsp;';
					$tableSeprator = '-&nbsp;';
					$parentId = $cat_id;
					$value['order'] = Engine_Api::_()->getDbtable('categories', 'egames')->orderNext(array('subcat_id'=>$cat_id));
				}else{
					$value['subsubcat_id'] = $cat_id;
					$seprator = '3';
					$tableSeprator = '--&nbsp;';
					$value['order'] = Engine_Api::_()->getDbtable('categories', 'egames')->orderNext(array('subsubcat_id'=>$cat_id));
					$parentId = $cat_id;
				}
			}else{
					$parentId = 0;
					$seprator = '';
					$value['order'] = Engine_Api::_()->getDbtable('categories', 'egames')->orderNext(array('category_id'=>true));
					$tableSeprator = '';
			}
			$db = Engine_Db_Table::getDefaultAdapter();
      $db->beginTransaction();
      try {
				$categoriesTable = Engine_Api::_()->getDbtable('categories', 'egames');
        ...
        $db->commit();
			...
			$tableData = '<tr id="categoryid-'.$row->category_id.'"><td><input type="checkbox" name="delete_tag[]" class="checkbox" value="'.$parentId.'" /></td><td style="display:none;">'.$data.'</td><td>'.$tableSeprator.$row->category_name.' <div class="hidden" style="display:none" id="inline_'.$row->category_id.'"><div class="parent">'.$parentId.'</div></div></td><td style="display:none;">'.$row->slug.'</td><td>'.$this->view->htmlLink(array("route" => "admin_default", "module" => "egames", "controller" => "categories", "action" => "edit-category", "id" => $row->category_id,"catparam" => "subsub"), $this->view->translate("Edit"), array()) .' | '.  $this->view->htmlLink('javascript:void(0);', $this->view->translate("Delete"), array("class" => "deleteCat","data-url"=>$row->category_id)) .'</td></tr>';
		...
		$this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('egames_admin_main', array(), 'egames_admin_main_categories');
	
    //Get all categories
    $this->view->categories = Engine_Api::_()->getDbtable('categories', 'egames')->getCategory(array('column_name' => '*','profile_type'=>true));
  }
 public function changeOrderAction(){
		if($this->_getParam('id',false) || $this->_getParam('nextid',false)){
			...
			$categoryTypeId = '';
			$checkTypeCategory = $dbObject->query("SELECT * FROM engine4_egames_categories WHERE category_id = ".$category_id)->fetchAll();
			...
			$currentOrder = Engine_Api::_()->getDbtable('categories', 'egames')->order($categoryType,$categoryTypeId);
			...
			$categoryTable = Engine_Api::_()->getDbtable('categories', 'egames');
			...
			$checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_egames_categories WHERE subcat_id = '".$id."' || subsubcat_id = '".$id."' || subcat_id = '".$nextid."' || subsubcat_id = '".$nextid."'")->fetchAll();
  ...
  public function editCategoryAction() {
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('egames_admin_main', array(), 'egames_admin_main_categories');
	// $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('egames_admin_categories', array(), 'egames_admin_main_subcategories');
    $this->view->form = $form = new Egames_Form_Admin_Category_Edit();
    $cat_id = $this->_getParam('id');
    $category = Engine_Api::_()->getItem('egames_category', $cat_id);
	...
	// if(isset($_POST['slug'])){
	// $slugExists = Engine_Api::_()->getDbtable('categories', 'egames')->slugExists($value['slug'],$cat_id);
	// 	if(!$slugExists){
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'egames', 'action' => 'index', 'controller' => 'categories'), 'admin_default', true);
  }
