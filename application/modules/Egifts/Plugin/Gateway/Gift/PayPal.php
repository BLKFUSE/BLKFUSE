 ...
 */
class Egifts_Plugin_Gateway_Gift_PayPal extends Engine_Payment_Plugin_Abstract {
    ...
    $this->getGateway()->getLog()->log('ExpressCheckoutDetail: '
      ...
      if( $paymentStatus == 'okay' || ($paymentStatus == 'pending' && $giveBenefit) ) {
        ...
        $orderPayment->gateway_transaction_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
				...
				$select = $tableRemaining->select()->from($tableName)->where('user_id =?',$item->owner_id);
				$select = $tableRemaining->fetchAll($select);
				$remainingAmt = $select[0]['remaining_payment'] - $rdata['AMT'];
				if($remainingAmt < 0) {
					$orderAmount = 0;
				} else {
					$tableRemaining->update(array('remaining_payment' => $remainingAmt),array('user_id =?'=>$item->owner_id));
				}

        //Notification work
        $viewer = Engine_Api::_()->user()->getViewer();
        $owner = $itemOwner->getOwner();
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $itemOwner, 'egifts_adminpayaprov', array());
  ...
  {
    return new Egifts_Form_Admin_Gateway_PayPal();
  }
