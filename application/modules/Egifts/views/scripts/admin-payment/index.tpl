							...
							<tr>
								...
								<td>
									<?php if ($item->state == 'pending'){ ?>
										<?php echo $this->htmlLink($this->url(array('module' => 'egifts', 'controller' => 'payment','owner_id' => $user->getIdentity(),'action'=>'approve','id'=>$item->userpayrequest_id), 'admin_default', true), $this->translate("Approve"), array('class' => 'smoothbox')); ?> |
										<?php echo $this->htmlLink($this->url(array('module' => 'egifts', 'controller' => 'payment','action' => 'cancel', 'id' => $item->userpayrequest_id, 'owner_id' => $user->getIdentity()), 'admin_default', true), $this->translate("Reject"), array('class' => 'smoothbox')); ?> |
									<?php } ?>
									<?php //echo $this->htmlLink($this->url(array('action' => 'payment-requests', 'owner_id' => $user->getIdentity()), 'egifts_general', true), $this->translate("Edit"), array('class' => '','target'=>'_blank')); ?>
									<?php echo $this->htmlLink($this->url(array('action' => 'detail-payment', 'id' => $item->userpayrequest_id, 'owner_id' => $user->getIdentity()), 'egifts_general', true), $this->translate("Details"), array('class' => 'smoothbox')); ?>
								</td>
							</tr>
							<?php endforeach; ?>
						</tbody>
