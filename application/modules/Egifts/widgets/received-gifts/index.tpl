...
<?php if($this->paginator->count() > 0){ ?>
      ...
      <div class="col-lg-4 col-md-6 col-sm-6 col-12">
    	<div class="egifts_listing_item">
          ...
          <?php if(isset($this->imageActive)){ ?>
          	<div class="egifts_listing_item_thumb" style="height:<?php echo is_numeric($this->params['height']) ? $this->params['height'].'px' : $$this->params['height']; ?>;">
          ...
          <div class="egifts_listing_item_info">
            ...
            <?php if(isset($this->descriptionActive)): ?>
              <div class="egifts_listing_item_des">
              	 <?php echo $description; ?>
              </div>
            <?php endif; ?>
            <?php if(isset($this->displayMsgActive) || isset($this->sendByActive)): ?>
              <div class="egifts_listing_item_user_info">
                <?php if(isset($this->sendByActive) && (($order->is_private == 1  && ($this->viewer->getIdentity() == $order->purchase_user_id)) || $order->is_private == 0)): ?>
                  <?php $sender = Engine_Api::_()->getItem('user', $order->sender_id); ?>
                  <div class="egifts_listing_item_user sesbasic_text_light">
                     from <?php echo $sender; ?>
                  </div>
                <?php endif; ?>
                <?php if(isset($this->displayMsgActive) && (($order->is_private == 1  && ($this->viewer->getIdentity() == $order->purchase_user_id)) || $order->is_private == 0)): ?>
                  <div class="egifts_listing_item_msg">
  ...
  <?php if(!$this->is_ajax){ ?>
    ...
    <!--
      <div class="sesbasic_load_btn" id="" onclick="">
    		<a href="javascript:void(0);" class="sesbasic_animation sesbasic_link_btn" id=""><span>View More</span></a>
      </div>
      <div class="sesbasic_load_btn" style="display: none;"><span class="sesbasic_link_btn"><i class="fa fa-spinner fa-spin"></i></span></div> -->
  </div>
      <?php if($this->params['pagging'] != 'pagging'): ?>
        <div class="sesbasic_load_btn" style="display: none;" id="view_more_<?php echo $randonNumber;?>" onclick="viewMore_<?php echo $randonNumber; ?>();" >
          <a href="javascript:void(0);" class="sesbasic_animation sesbasic_link_btn" id="feed_viewmore_link_<?php echo $randonNumber; ?>"><span><?php echo $this->translate('View More');?></span></a>
        </div>  
...
<?php }else { ?>
    <div class="sesbasic_tip clearfix">
    	<img src="application/modules/Egifts/externals/images/gift-icon.png" alt="" />
      <span class="sesbasic_text_light">
        <?php echo $this->translate('No gift Received yet.') ?>
      </span>
    </div>
<?php } ?>
 ...
 <?php if($this->params['pagging'] == 'auto_load'){ ?>
    scriptJquery( window ).load(function() {
      scriptJquery(window).scroll( function() {
        if(scriptJquery('#egift-browse-widget_<?php echo $randonNumber; ?>').length) {
