...
class Epaidcontent_Model_DbTable_Orders extends Engine_Db_Table {

  protected $_rowClass = "Epaidcontent_Model_Order";
 	...
 	public function getViewerOrder($params = array()) {
		...
		$select = $this->select()
                  ...
                  ->where('package_owner_id =?', $params['package_owner_id'])
                  ->limit(1);
	...
	public function packageStatsSale($params = array()) {
    ...
    if(isset($params['action']) && $params['action'] == 'myorders') {
      ...
      $select->where('package_owner_id =?',$params['user_id']);
    }
	...
	public function getReportData($params = array()) {
		...
		if(isset($params['package_owner_id']))
      $select->where($orderTableName.'.package_owner_id =?',$params['package_owner_id']);
    ...
    $select->where($orderTableName.'.state =?','complete');
		...
		if(isset($params['type'])) {
			if($params['type'] == 'month') {
              ...
              ->group("$orderTableName.package_owner_id")
