...
class Epaidcontent_IndexController extends Core_Controller_Action_Standard {
  ...
  public function detailPaymentAction() {
    ...
    $this->view->item = $paymnetReq = Engine_Api::_()->getItem('epaidcontent_userpayrequest', $this->getRequest()->getParam('id'));
  ...
  public function paymentRequestAction() {
    ...
    $this->view->thresholdAmount = $thresholdAmount = Engine_Api::_()->authorization()->getPermission($viewer, 'epaidcontent', 'epaidcontent_threamt');

    $remainingAmount = Engine_Api::_()->getDbtable('remainingpayments', 'epaidcontent')->getRemainingAmount(array('user_id' => $viewer->getIdentity()));
    if (!$remainingAmount) {
    ...
    $orderDetails = Engine_Api::_()->getDbtable('orders', 'epaidcontent')->packageStatsSale(array('package_owner_id' => $viewer->getIdentity()));
    $this->view->form = $form = new Epaidcontent_Form_Paymentrequest();
    $value = array();
    ...
    try {
      ...
      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner_admin, $viewer, $viewer, 'epaidcontent_payrequest', array('requestAmount' => round($_POST['requested_amount'], 2)));
      
      //Payment request mail send to admin
      Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner_admin, 'epaidcontent_entrypayment_requestadmin', array('buyer_name' => $viewer->geTitle(), 'object_link' => $viewer->getHref(), 'host' => $_SERVER['HTTP_HOST']));
  ...
  public function paymentRequestsAction() {
    ...
    $this->view->userGateway = Engine_Api::_()->getDbtable('usergateways', 'epaidcontent')->getUserGateway(array('user_id' => $viewer_id));
    
    $this->view->orderDetails = Engine_Api::_()->getDbtable('orders', 'epaidcontent')->packageStatsSale(array('package_owner_id' => $viewer_id));
    
    $remainingAmount = Engine_Api::_()->getDbtable('remainingpayments', 'epaidcontent')->getRemainingAmount(array('user_id' => $viewer_id));
    if (!$remainingAmount) {
      $this->view->remainingAmount = 0;
    } else
      $this->view->remainingAmount = $remainingAmount->remaining_payment;
      
    $this->view->isAlreadyRequests = Engine_Api::_()->getDbtable('userpayrequests', 'epaidcontent')->getPaymentRequests(array('owner_id' => $viewer_id, 'isPending' => true));
    
    $this->view->paymentRequests = Engine_Api::_()->getDbtable('userpayrequests', 'epaidcontent')->getPaymentRequests(array('owner_id' => $viewer_id, 'isPending' => true));
  }
  ...
  public function paymentTransactionAction() {
    ...
    $this->view->paymentRequests = Engine_Api::_()->getDbtable('userpayrequests', 'epaidcontent')->getPaymentRequests(array('package_owner_id' => $viewer->getIdentity(), 'state' => 'both'));
  }
  ...
  public function salesReportsAction() {
    ...
    $this->view->form = $form = new Epaidcontent_Form_Searchsalereport();
    $value = array();
    ...
    if (isset($_GET['csv']) && $_GET['csv'] != '')
      $value['download'] = 'csv';
    $form->populate($value);
    $value['package_owner_id'] = $viewer->getIdentity();
    $this->view->eventSaleData = $data = Engine_Api::_()->getDbTable('orders', 'epaidcontent')->getReportData($value);
  ...
  public function salesStatsAction() {
    ...
    $this->view->todaySale = Engine_Api::_()->getDbtable('orders', 'epaidcontent')->getSaleStats(array('stats' => 'today', 'package_owner_id' => $viewer_id));
    $this->view->weekSale = Engine_Api::_()->getDbtable('orders', 'epaidcontent')->getSaleStats(array('stats' => 'week', 'package_owner_id' => $viewer_id));
    $this->view->monthSale = Engine_Api::_()->getDbtable('orders', 'epaidcontent')->getSaleStats(array('stats' => 'month', 'package_owner_id' => $viewer_id));

    $this->view->packageStatsSale = Engine_Api::_()->getDbtable('orders', 'epaidcontent')->packageStatsSale(array('package_owner_id' => $viewer_id));
  }
  ...
  public function showpackageAction() {
    ...
    $this->view->enablePackage = Engine_Api::_()->getDbtable('packages','epaidcontent')->getEnabledPackages($user_id);
  }
  
  public function makepaymentAction() {

    $this->view->package_id = $package_id = $this->_getParam('package_id', null);
    $this->view->package = $package = Engine_Api::_()->getItem('epaidcontent_package', $package_id);
    $user_id = $package->user_id;
  ...
  public function processAction() {
    ...
    if ($package_id) {
      ...
      if ($package) {
		...
		$this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();		
    if( !$gatewayId ||
        !($gateway = Engine_Api::_()->getItem('epaidcontent_gateway', $gatewayId)) ||
        !($gateway->enabled) ) {
      header("location:".$this->view->escape($package->getHref()));
			die;
    }		
    ...
    $params['return_url'] = $schema . $host
      . $this->view->escape($this->view->url(array('action' => 'return','order_id'=>$order->getIdentity(),'package_id'=>$order->package_id)))
      . '/?state=' . 'return';
...
// 		}
    ...
    $ordersTable->insert(array(
        ...
        'source_type' => 'epaidcontent_order',
    ...
    $priceTotal = @round($package->price*$currencyValue,2);
    ...
    $creditCode =  'credit'.'-epaidcontent-'.$order->package_id.'-'.$order->package_id;
    ...
    if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
      ...
      $this->view->session = $plugin->createOrderTransaction($viewer,$order,$package,$params);
      $this->renderScript('/application/modules/Sesadvpmnt/views/scripts/payment/index.tpl');
        ...
        $paytmParams = $plugin->createOrderTransaction($viewer,$order,$package,$params);
      ...
      // Process transaction
      $transaction = $plugin->createOrderTransaction($viewer,$order,$package,$params);
  ...
  public function returnAction() {
    ...
    if (!$orderPayment || ($orderId != $orderPayment->source_id) ||
			 ($orderPayment->source_type != 'epaidcontent_order') ||
			 !($user_order = $orderPayment->getSource()) ) {
			return $this->_helper->redirector->gotoRoute(array("action" => "my-orders"), 'epaidcontent_general', true);
    }
    $gateway = Engine_Api::_()->getItem('epaidcontent_gateway', $orderPayment->gateway_id);    
    if( !$gateway )
      return $this->_helper->redirector->gotoRoute(array(), 'epaidcontent_general', true);
    $package_id = $this->_getParam('package_id', null);    
    $package = null;
    if ($package_id) {
      $package = Engine_Api::_()->getItem('epaidcontent_package', $package_id);
  ...
  public function createpackageAction() {
    ...
    if( !$this->_helper->requireAuth()->setAuthParams('epaidcontent', $viewer, 'create')->isValid())
      return;
    
    // Make form
    $this->view->form = $form = new Epaidcontent_Form_Index_Create();
    ...
    $packageTable = Engine_Api::_()->getDbtable('packages', 'epaidcontent');
    ...
    // Redirect
    return $this->_helper->redirector->gotoRoute(array('action' => 'manage-packages'));
  }
  ...
  public function editpackageAction() {
    ...
    // Get package
    if( null === ($packageIdentity = $this->_getParam('package_id')) || !($package = Engine_Api::_()->getDbtable('packages', 'epaidcontent')->find($packageIdentity)->current()) ) {
      throw new Engine_Exception('No package found');
    }

    // Make form
    $this->view->form = $form = new Epaidcontent_Form_Index_Edit();
  ...
  public function managePackagesAction() {
    
    $this->_helper->content->setEnabled();
    $this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
    $table = Engine_Api::_()->getDbtable('packages', 'epaidcontent');
  ...
  public function accountDetailsAction() {
    ...
    $userGateway = Engine_Api::_()->getDbtable('usergateways', 'epaidcontent')->getUserGateway(array('user_id' => $user->user_id, 'enabled' => true,'gateway_type'=>$gateway_type,));
    $settings = Engine_Api::_()->getApi('settings', 'core');
    $userGatewayEnable = $settings->getSetting('sespage.userGateway', 'paypal');
    if($gateway_type == "paypal") {
        $userGatewayEnable = 'paypal';
        $this->view->form = $form = new Epaidcontent_Form_Index_PayPal();
        $gatewayTitle = 'Paypal';
        $gatewayClass= 'Epaidcontent_Plugin_Gateway_PayPal';
    } else if($gateway_type == "stripe") {
        $userGatewayEnable = 'stripe';
        $this->view->form = $form = new Sesadvpmnt_Form_Admin_Settings_Stripe();
        $gatewayTitle = 'Stripe';
        $gatewayClass= 'Epaidcontent_Plugin_Gateway_Event_Stripe';
    } else if($gateway_type == "paytm") {
        $userGatewayEnable = 'paytm';
        $this->view->form = $form = new Epaytm_Form_Admin_Settings_Paytm();
        $gatewayTitle = 'Paytm';
        $gatewayClass= 'Epaidcontent_Plugin_Gateway_Event_Paytm';
    }
    ...
    $db->beginTransaction();
    $userGatewayTable = Engine_Api::_()->getDbtable('usergateways', 'epaidcontent');
    ...
    try {
      if (empty($userGateway)) {
        ...
        $gatewayObject = Engine_Api::_()->getItem("epaidcontent_usergateway", $userGateway['usergateway_id']);
      }
      $db->commit();
    } catch (Exception $e) {
      echo $e->getMessage();
