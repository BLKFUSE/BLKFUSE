      ...
      <table class="epaidcontent_manage_table">
        ...
        <tbody>
          <?php foreach ($this->paymentRequests as $item): ?>
            <tr>
              <td data-label=" <?php echo $this->translate("Requested Amount") ?>" class='epaidcontent_manage_table_price'><?php echo Engine_Api::_()->epaidcontent()->getCurrencyPrice($item->requested_amount,$defaultCurrency); ?></td>
              <td data-label=" <?php echo $this->translate("Released Amount") ?> " class="epaidcontent_manage_table_bold payment_found"><?php echo Engine_Api::_()->epaidcontent()->getCurrencyPrice($item->release_amount	,$defaultCurrency); ?></td>
              <td data-label=" <?php echo $this->translate("Released Date") ?> " ><?php echo $item->release_date != "0000-00-00 00:00:00" ? Engine_Api::_()->epaidcontent()->dateFormat($item->release_date) :  '-'; ?></td> 
              ...
              <td data-label=" <?php echo $this->translate("Status") ?> " class="epaidcontent_manage_table_bold"><?php echo ucfirst($item->state); ?></td>
              <td data-label=" " class="table_options">
                <?php //echo $this->htmlLink($this->url(array('action' => 'detail-payment', 'id' => $item->userpayrequest_id, 'page_id' => $this->page->custom_url), 'epaidcontent_dashboard', true), $this->translate(""), array('title' => $this->translate("View Details"), 'class' => 'sessmoothbox fa fa-eye')); ?>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
