...
class Eusertip_Api_Core extends Core_Api_Abstract {
  ...
  public function isViewerPlanActive($item) {
    ...
    if(Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('eusertip')) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eusertip.allow',1) && isset($tip_id) && !empty($tip_id) && $owner_id != $viewer_id) {
      $tip = Engine_Api::_()->getItem('eusertip_tip', $tip_id); 
      $getViewerOrder = Engine_Api::_()->getDbTable('orders','eusertip')->getViewerOrder(array('owner_id' => $viewer_id, 'tip_owner_id' => $tip->user_id, 'noCondition' => 1)); 
      if((float) $getViewerOrder->total_amount < (float) $tip->price) {
        $plan = true;
      }
    }
    return $plan;
