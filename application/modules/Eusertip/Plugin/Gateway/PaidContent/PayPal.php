 ...
 */
class Eusertip_Plugin_Gateway_PaidContent_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      ...
      $gateway = new $class(array(
        ...
        'currency' => Engine_Api::_()->eusertip()->defaultCurrency(),
      ));
    ...
    $this->getGateway()->getLog()->log('ExpressCheckoutDetail: '
      ...
      if( $paymentStatus == 'okay' || ($paymentStatus == 'pending' && $giveBenefit) ) {
        ...
        $orderPayment->gateway_transaction_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
				...
				$select = $tableRemaining->select()->from($tableName)->where('user_id =?', $orderPayment->owner_id);
				...
				if($remainingAmt < 0)
					...
					$tableRemaining->update(array('remaining_payment' => $remainingAmt),array('user_id =?' => $orderPayment->owner_id));
        ...
        if( $orderPayment->state == 'complete' ) {
          /*Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_subscription_active', array(
            'subscription_title' => $tip->title,
            'subscription_description' => $tip->description,
            'subscription_terms' => $tip->getTipDescription(),
