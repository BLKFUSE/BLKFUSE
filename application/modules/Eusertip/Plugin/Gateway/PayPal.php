 ...
 */
class Eusertip_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract
{
  ...
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      ...
      $gateway = new $class(array(
        ...
        'currency' => Engine_Api::_()->eusertip()->getCurrentCurrency(),
      ));
   ...
   * @param Zend_Db_Table_Row_Abstract $tip
   * @param array $params
   * @return Engine_Payment_Gateway_Transaction
   */
	public function createSubscriptionTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $user_order, Payment_Model_Package $tip, array $params = array()){}
	/**
   ...
   * 
	*/
  public function createOrderTransaction($viewer,$order,$tip,array $params = array()) {
  
    // description
    $description = $tip->getTitle();
    ...
    if( function_exists('iconv') && strlen($description) != iconv_strlen($description) ) {
      ...
      $description = @iconv("UTF-8", "ISO-8859-1//TRANSLIT", $description);
		...
		$currentCurrency = Engine_Api::_()->eusertip()->getCurrentCurrency();
    ...
    $order->currency_symbol = Engine_Api::_()->eusertip()->getCurrentCurrency();
    ...
    //Admin commission
    $commissionType = Engine_Api::_()->authorization()->getPermission($viewer,'eusertip','eusertip_admcosn');
    $commissionTypeValue = Engine_Api::_()->authorization()->getPermission($viewer,'eusertip','eusertip_commival');
    ...
    // Get related info
    $user = $order->getUser();
    $orderPaidContent = $order->getSource();
    if ($orderPaidContent->state == 'pending') 
    {
      return 'pending';
    }
    ...
    $this->getGateway()->getLog()->log('ExpressCheckoutDetail: '
      ...
      if( $paymentStatus == 'okay' ||
        ...
        $orderPaidContent->gateway_id = $this->_gatewayInfo->gateway_id;
        $orderPaidContent->gateway_transaction_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
				$orderPaidContent->currency_symbol = $rdata['PAYMENTINFO'][0]['CURRENCYCODE'];
				$orderPaidContent->change_rate = $currencyValue;
				$orderPaidContent->save();
				$orderAmount = round($orderPaidContent->total_amount,2);
				$commissionValue = round($orderPaidContent->commission_amount,2);
				if(isset($commissionValue) && $orderAmount > $commissionValue){
					$orderAmount = $orderAmount - $commissionValue;	
				}else{
					$orderPaidContent->commission_amount = 0;
				}
				
				//Update content OWNER REMAINING amount
				$tableRemaining = Engine_Api::_()->getDbtable('remainingpayments', 'eusertip');
				$tableName = $tableRemaining->info('name');
				$select = $tableRemaining->select()->from($tableName)->where('user_id =?',$orderPaidContent->tip_owner_id);
				$select = $tableRemaining->fetchAll($select);
				if(engine_count($select)){
					$tableRemaining->update(array('remaining_payment' => new Zend_Db_Expr("remaining_payment + $orderAmount")),array('user_id =?'=>$orderPaidContent->tip_owner_id));
				} else {
					$tableRemaining->insert(array(
						'remaining_payment' => $orderAmount,
						'user_id' => $orderPaidContent->tip_owner_id,
					));
				}
				
        //For Coupon
        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('ecoupon')){
           $orderPaidContent->ordercoupon_id = Engine_Api::_()->ecoupon()->setAppliedCouponDetails($params['couponSessionCode']);
           $orderPaidContent->save();
        }
        ...
        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sescredit') && isset($params['creditCode'])) {
          ...
          $orderPaidContent->credit_point = $sessionCredit->credit_value;  
          $orderPaidContent->credit_value =  $sessionCredit->purchaseValue;
          $transaction->save();
        ...
        // Payment success
        $orderPaidContent->onOrderComplete();
        $orderPaidContent->save();
        
        // send notification
        if( $order->state == 'complete' ) {
        
          //Cancelled all previous order of current user
          $getViewerAllOrders = Engine_Api::_()->getDbTable('orders', 'eusertip')->getViewerAllOrders(array('order_id' => $orderPaidContent->order_id, 'owner_id' => $user->user_id, 'noCondition' => 1, 'tip_owner_id' => $orderPaidContent->tip_owner_id));
          if(engine_count($getViewerAllOrders) > 0) {
            foreach($getViewerAllOrders as $getViewerAllOrder) {
              $getViewerAllOrder->state = 'cancelled';
              $getViewerAllOrder->save();
            }
            ...
            $order->save();
		      ...
		      //Notification Work
		      $tip = Engine_Api::_()->getItem('eusertip_tip', $orderPaidContent->tip_id);
					$owner = Engine_Api::_()->getItem('user',$tip->user_id);
          $totalAmounts = '[';
		      $totalAmounts .= 'Total:';
          $totalAmount = @round($orderPaidContent->total_amount,2);
          $totalAmounts .= Engine_Api::_()->eusertip()->getCurrencyPrice(@round($totalAmount,2),$orderPaidContent->currency_symbol, $orderPaidContent->change_rate);
          $totalAmounts .= ']';
          $body .= '<table style="background-color:#f9f9f9;border:#ececec solid 1px;width:100%;"><tr><td><div style="margin:0 auto;width:600px;font:normal 13px Arial,Helvetica,sans-serif;padding:20px;"><div style="margin-bottom:10px;overflow:hidden;"><div style="float:left;"><b>Order Id: #' . $orderPaidContent->order_id . '</b></div><div style="float:right;"><b>'.$totalAmounts.'</b></div></div><table style="background-color:#fff;border:#ececec solid 1px;margin-bottom:20px;" cellpadding="0" cellspacing="0" width="100%"><tr valign="top" style="width:50%;"><td><div style="border-bottom:#ececec solid 1px;padding:20px;"><b style="display:block;margin-bottom:5px;">Ordered For</b><span style="display:block;margin-bottom:5px;">'.$tip->getTitle().'</span></div><div style="padding:20px;border-bottom:#ececec solid 1px;"> <b style="display:block;margin-bottom:5px;">Ordered By</b><span style="display:block;margin-bottom:5px;"><a href="'.( isset($_SERVER["HTTPS"]) && (strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] .$orderPaidContent->getOwner()->getHref().'" style="color:#39F;text-decoration:none;">'.$orderPaidContent->getOwner()->getTitle().'</a></span><span style="display:block;margin-bottom:5px;">'.$orderPaidContent->getOwner()->email.'</span></div><div style="padding:20px;"><b style="display:block;margin-bottom:5px;">Payment Information</b><span style="display:block;margin-bottom:5px;">Payment Method: '.$orderPaidContent->gateway_type.'</span></div></td><td style="border-left:#ececec solid 1px;width:50%;"><div style="padding:20px;"><b style="display:block;margin-bottom:5px;">Order Information</b><span style="display:block;margin-bottom:5px;">Ordered Date: '.$orderPaidContent->creation_date.'</span>';			   
			    $body .= '</div>';
			   
			    $body .= '</td></tr></table><div style="margin-bottom:10px;"><b class="bold">Order Details</b></div><div style="background-color:#fff;border:1px solid #ececec;padding:10px;"><div style="margin-bottom:5px;overflow:hidden;"><span style="float:left;">Sub Total</span><span style="float:right;">'.$totalAmounts.'</span> </div>';
			    $body .= '<div style="margin-bottom:5px;overflow:hidden;"><span style="float:left;"><b>Grand Total</b></span><span style="float:right;"><b>'.$totalAmounts.'</b></span></div></div></div> </td></tr></table>';

          //Send to tip owner
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($owner, $user, $tip, 'eusertip_order', array('tip_name' => $tip->title));
          
          //invoice mail to buyer
			    Engine_Api::_()->getApi('mail', 'core')->sendSystem($orderPaidContent->getOwner(), 'eusertip_orderinvoice_buyer', array('invoice_body' => $body, 'host' => $_SERVER['HTTP_HOST']));
			    
			    //Mail to tip Owner
			    Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner, 'eusertip_orderpurchased_paidcontentowner', array('object_link' => $owner->getHref(), 'tip_title' => $tip->title, 'buyer_name' => $user->getTitle(), 'host' => $_SERVER['HTTP_HOST']));
        }
        return 'active';
      }
        ...
        $order->gateway_profile_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
				$order->save();
        // Order pending
        $orderPaidContent->onOrderPending();
        //Send Mail
        $tip = Engine_Api::_()->getItem('eusertip_tip', $orderPaidContent->tip_id);
        
				Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'eusertip_payment_order_pending', array('tip_title' => $tip->title, 'tip_description' => $tip->description, 'host' => $_SERVER['HTTP_HOST']));
        
        return 'pending';
      }
      else if( $paymentStatus == 'failed' ) {
        // Cancel order and subscription?
        $orderPaidContent->onOrderFailure();
				//update ticket state
        // Payment failed
  ...
  {}
    ...
    if( !empty($rawData['case_type']) && $rawData['case_type'] == 'chargeback' ) {
      $orderPaidContent->onOrderFailure(); // or should we use pending?
			//update ticket state
    }
    // Transaction Type --------------------------------------------------------
          ...
          // Only allowed for one-time
            switch( $rawData['payment_status'] ) {
              ...
              case 'Pending':
                ...
                if( $giveBenefit ) {
                  $orderPaidContent->onOrderSuccess();
									//update ticket state
                } else {
                  $orderPaidContent->onOrderPending();
									//update ticket state
              ...
              case 'Reversed':
                ...
                break;
              case 'Refunded':
                $orderPaidContent->onOrderRefund();
								//update ticket state
                break;
  ...
  public function onIpn(Engine_Payment_Ipn $ipn)
  {
    ...
    if( $order ) {
      $ipnProcessed = false;
      // Subscription IPN
      if( $order->source_type == 'eusertip_order' ) {
  ...
  {
    return new Eusertip_Form_Admin_Gateway_PayPal();
  }
