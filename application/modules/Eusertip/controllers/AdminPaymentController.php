 ...
 */
class Eusertip_AdminPaymentController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eusertip_admin_main_paymentr', array(), 'eusertip_admin_main_paymentrequestsub');
    
    $this->view->formFilter = $formFilter = new Eusertip_Form_Admin_Filterpaymentorder();
    
		$values = array();
    if ($formFilter->isValid($this->_getAllParams()))
      $values = $formFilter->getValues();
      
    $userpayrequestTable = Engine_Api::_()->getItemTable('eusertip_userpayrequest');
		$userpayrequestTableName = $userpayrequestTable->info('name');
		
		$tipTableName = Engine_Api::_()->getItemTable('eusertip_tip')->info('name');
    ...
    $gateway_enable = Engine_Api::_()->getDbtable('usergateways', 'eusertip')->getUserGateway(array('user_id'=>$user->user_id));
    if(empty($gateway_enable)) {
      $this->view->disable_gateway = true;		
    } else {
      $this->view->disable_gateway = false;	
 
      $this->view->form = $form = new Eusertip_Form_Admin_Payment_Approve(array('userId' => $user->user_id));
      $defaultCurrency = Engine_Api::_()->eusertip()->defaultCurrency();
      $remainingAmount  =  Engine_Api::_()->getDbtable('remainingpayments', 'eusertip')->getRemainingAmount(array('user_id' => $user->user_id));
      $orderDetails  =  Engine_Api::_()->getDbtable('orders', 'eusertip')->tipStatsSale(array('tip_owner_id' => $user->user_id));
      $value = array();
      $value['total_amount'] = Engine_Api::_()->eusertip()->getCurrencyPrice($orderDetails['totalAmountSale'],$defaultCurrency);
      $value['total_tax_amount'] = Engine_Api::_()->eusertip()->getCurrencyPrice($orderDetails['totalTaxAmount'],$defaultCurrency);
      ...
      if($this->_getParam('id',false)){
        ...
        if($item) {
          ...
          $value['requested_amount'] = Engine_Api::_()->eusertip()->getCurrencyPrice($itemValue['requested_amount'],$defaultCurrency);
      ...
      try {
        ...
        $owner = $user->getOwner();
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $user, 'eusertip_adminpayaprov', array());
        
        //Payment approve mail send to event owner
        Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner, 'eusertip_payment_adminrequestapproved', array('object_link' => $user->getHref(), 'host' => $_SERVER['HTTP_HOST']));
        ...
        return $this->_forward('success', 'utility', 'core', array(
          'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('route' => 'default','module' => 'eusertip', 'controller' => 'payment', 'action' => 'process','gateway_id'=>$_POST['gateway_id']),'admin_default',true),
	...
	public function processAction() {
		...
		$item = Engine_Api::_()->getItem('eusertip_userpayrequest', $session->payment_request_id);
		
		$user = Engine_Api::_()->getItem('user', $item->owner_id);
		
    // Get gateway
    $gatewayId = $this->_getParam('gateway_id', null); 
		$gateway = Engine_Api::_()->getItem('eusertip_usergateway',$gatewayId);
		if( !$gatewayId ||
        !($gateway) ||
        !($gateway->enabled) ) {
    ...
    $params['return_url'] = $schema . $host
      .  $this->view->url(array('action' => 'return', 'controller' => 'payment', 'module' => 'eusertip'), 'admin_default', true)
    ...
    //Process transaction
    if($gateway->plugin == "Eusertip_Plugin_Gateway_Event_Stripe") {
    ...
    } elseif($gateway->plugin  == "Eusertip_Plugin_Gateway_Event_Paytm") {
    ...
    // Get order
		...
		$item_id = $session->eusertip_item_id ;
		$item = Engine_Api::_()->getItem('eusertip_userpayrequest', $item_id);
    if (!$orderPayment || ($orderId != $orderPaymentId) ||
			 ($orderPayment->source_type != 'eusertip_userpayrequest') ||
			 !($user_order = $orderPayment->getSource()) ) {
			return $this->_helper->redirector->gotoRoute(array(), 'admin_default', true);
    }
    
    $gateway = Engine_Api::_()->getItem('eusertip_usergateway', $orderPayment->gateway_id); 
		if( !$gateway )
      return $this->_helper->redirector->gotoRoute(array(), 'admin_default', true);
    // Get gateway plugin
    $plugin = $gateway->getPlugin();
  ...
  public function finishAction() {
    $session = new Zend_Session_Namespace();
    if (!empty($session->eusertip_order_id))
      $session->eusertip_order_id = '';
		
    $orderTrabsactionDetails = array('state' => $this->_getParam('state'), 'errorMessage' => $session->errorMessage);
    $session->eusertip_order_details = $orderTrabsactionDetails;
    ...
    try {
      ...
      $viewer = Engine_Api::_()->user()->getViewer();
			$owner = $user->getOwner();
			Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $user, 'eusertip_adminpaycancl', array());
			
      //Payment cancel mail send to page owner
			//Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner, 'eusertip_payment_adminrequestcancel', array('user_title' => $user->getTitle(), 'object_link' => $user->getHref(), 'host' => $_SERVER['HTTP_HOST']));
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
