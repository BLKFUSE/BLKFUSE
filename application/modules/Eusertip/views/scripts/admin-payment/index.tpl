        ...
        <table class='admin_table' style="width:100%;">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?php echo Engine_Api::_()->eusertip()->getCurrencyPrice($item->requested_amount,$defaultCurrency); ?></td>
              <td><?php echo Engine_Api::_()->eusertip()->dateFormat($item->creation_date	); ?></td> 
              <td><?php echo ucfirst($item->state); ?></td>
              <td>
                <?php if ($item->state == 'pending'){ ?>
                  <?php echo $this->htmlLink($this->url(array('module' => 'eusertip', 'controller' => 'payment','user_id' => $user->user_id,'action'=>'approve','id'=>$item->userpayrequest_id), 'admin_default', true), $this->translate("Approve"), array('class' => 'smoothbox')); ?> |
                  <?php echo $this->htmlLink($this->url(array('module' => 'eusertip', 'controller' => 'payment','action' => 'cancel', 'id' => $item->userpayrequest_id, 'user_id' => $user->user_id), 'admin_default', true), $this->translate("Reject"), array('class' => 'smoothbox')); ?> |
                <?php } ?>
                <?php echo $this->htmlLink($this->url(array('action' => 'payment-requests', 'user_id' => $user->user_id), 'eusertip_general', true), $this->translate("Edit"), array('class' => '','target'=>'_blank')); ?> |
                <?php echo $this->htmlLink($this->url(array('action' => 'detail-payment', 'id' => $item->userpayrequest_id, 'user_id' => $user->user_id), 'eusertip_general', true), $this->translate("Details"), array('class' => 'smoothbox')); ?>
              </td>
            </tr>
            <?php endforeach; ?>
          </tbody>
