 ...
 * @version    $Id: Action.php 2017-01-12  00:00:00 socialnetworking.solutions $
...
class Sesadvancedactivity_Model_Action extends Activity_Model_Action
{
  ...
  public function getHref($params = array())
  {
    $params = array_merge(array(
      'route' => 'sesadvancedactivity_view',
      'reset' => true,
    ...
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedcomment')) {
        ...
        if($table->info('name') == 'engine4_core_comments') {
            ...
            $select->setIntegrityCheck(false)
                ...
                $select->where($coreCommentTableName.'.comment_id > '.$comment_id);
        } else if($table->info('name') == 'engine4_activity_comments') {
            ...
            $select->setIntegrityCheck(false)
                ...
                $select->where($activityCommentTableName.'.comment_id > '.$comment_id);
        }
    }
  ...
  protected function _delete() {
    ...
    $db->query("DELETE FROM engine4_sesadvancedactivity_buysells WHERE action_id = " . $this->action_id);
