 ...
 * @version    $Id: GetContent.php 2017-01-12  00:00:00 socialnetworking.solutions $
...
class Sesadvancedactivity_View_Helper_GetContent
{
  ...
  public function getContent($actions = null, array $data = array(),$break = true,$change = false)
  {
    ...
    $sesAdvancedactivityfonttextsize = $settings->getSetting('sesadvancedactivity.fonttextsize',18);
    ...
    if(engine_count($getAllTextColors) > 0) {
      if(!$change)
        $content[1] = ' ' . $content[1] . ' ';

        $content[1] = str_replace("#",'klasjdkznmxcdsjweio',$content[1]);
        $content[1] = str_replace("=",'QEWRTYWUJDJMDND',$content[1]);
   ...
   // return $parsedMessage = preg_replace(array('/(?i)\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?«»“”‘’]))/', '/(^|[^a-z0-9_])@([a-z0-9_]+)/i', '/(^|[^a-z0-9_])#([a-z0-9_]+)/i'), array('<a href="$1">$1</a>', '$1@$2', '$1<a href="hashtag?hashtag=$2">#$2</a>'), $content);
    ...
    foreach($matches[0] as $value){
      if(!engine_in_array($value,$searchword)){
        $searchword[]=$value;
        $replaceWord[] = '<a target="_blank"  href="hashtag?hashtag='.str_replace('#','',strip_tags($value)).'">'.strip_tags($value).'</a>';
      }
    }
