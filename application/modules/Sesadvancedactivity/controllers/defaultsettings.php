 ...
 * @version    $Id: defaultsettings.php 2017-01-12  00:00:00 socialnetworking.solutions $
...
if( !$page_id ) {
  ...
  $db->insert('engine4_core_content', array(
    ...
    'parent_content_id' => $main_middle_id,
    'order' => $widgetOrder++,
    'params' => '{"title":"","design":"1","enablestatusbox":"2","feeddesign":"1","sesact_pinboard_width":"300","scrollfeed":"0","autoloadTimes":"3","userphotoalign":"left","nomobile":"0","name":"sesadvancedactivity.feed"}',
  ));
  // insert left content
  $db->insert('engine4_core_content', array(
    'type' => 'widget',
...
$page_id = $db->select()
  ->from('engine4_core_pages', 'page_id')
  ->where('name = ?', 'sesadvancedactivity_index_view')
  ->limit(1)
  ->query()
  ->fetchColumn();
$widgetOrder = 1;
...
if( !$page_id ) {
  // Insert page
  $db->insert('engine4_core_pages', array(
    'name' => 'sesadvancedactivity_index_view',
    'displayname' => 'SNS - Professional Activity & Comments - Profile Page',
    'title' => 'Profile feed',
    'description' => 'This page show Profile feeds.',
    'custom' => 0,
  ));
  $page_id = $db->lastInsertId();
...
("sesadvancedactivity.fonttextsize", "18"),
...
("sesadvancedactivity.visiblesearchfilter", "5");');


$filterlists_table = $db->query('SHOW TABLES LIKE \'engine4_sesadvancedactivity_filterlists\'')->fetch();
if($filterlists_table) {
  $icon = $db->query('SHOW COLUMNS FROM engine4_sesadvancedactivity_filterlists LIKE \'icon\'')->fetch();
  if (empty($icon)) {
    $db->query("ALTER TABLE `engine4_sesadvancedactivity_filterlists` ADD `icon` VARCHAR(128) NULL DEFAULT NULL;");
  }
}
...
$db->query("INSERT IGNORE INTO `engine4_core_settings` (`name`, `value`) VALUES ('sesadvancedactivity.notificationdaytext','<div style=\"padding: 20px 10px; font-size: 17px; font-weight: bold; position: relative;\"><span style=\"color: #f8b62d;\"> NOTIFICATION_TIME NOTIFICATION_USER! </span> <span style=\"position: absolute; right: 10px; bottom: 5px; top: 5px; text-align: right;\"> <img style=\"max-height: 100%;\" src=\"NOTIFICATION_IMAGE\" alt=\"Notification Image\"> </span></div>')");
...
$db->query("DELETE FROM `engine4_core_menuitems` WHERE `engine4_core_menuitems`.`menu` = 'sesfeedgif_admin_main';");
$db->query("DELETE FROM `engine4_core_menuitems` WHERE `engine4_core_menuitems`.`name` = 'sesfeedgif_admin_main_fegifsettings';");

$db->query("UPDATE `engine4_sesadvancedactivity_filterlists` SET `icon`= 'fas fa-sync' WHERE `filtertype` = 'all';");
$db->query("UPDATE `engine4_sesadvancedactivity_filterlists` SET `icon`= 'fas fa-network-wired' WHERE `filtertype` = 'my_networks';");
$db->query("UPDATE `engine4_sesadvancedactivity_filterlists` SET `icon`= 'fas fa-users' WHERE `filtertype` = 'my_friends';");
$db->query("UPDATE `engine4_sesadvancedactivity_filterlists` SET `icon`= 'fas fa-comment' WHERE `filtertype` = 'posts';");
...
$db->query("UPDATE `engine4_sesadvancedactivity_filterlists` SET `icon`= 'fa fa-file-alt' WHERE `filtertype` = 'post_self_file';");
$db->query("UPDATE `engine4_sesadvancedactivity_filterlists` SET `icon`= 'fa fa-comments' WHERE `filtertype` = 'sesblog' OR `filtertype` = 'blog';");

$db->query("ALTER TABLE `engine4_sesadvancedactivity_details` ADD `gif_url` TEXT NULL DEFAULT NULL;");
