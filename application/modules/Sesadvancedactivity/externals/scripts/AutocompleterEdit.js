...
var AutocompleterEdit = new Class({
	...
	build: function() {
		if ($(this.options.customChoices)) {
			...
			if(!this.options.ignoreOverlayFix) this.fix = new OverlayFixEdit(this.choices);
		}
	...
	choiceSelect: function(choice) {
		...
		if( !this.options.alwaysOpen ) {
                ...
                if (this.options.autocompleteType=='message') {
                  ...
                  if (token.friends){
                    ...
                    this.doAddValueToHidden(token.label, friend_ids, 'toValuesEdit', true, ' tag_friend');
                  ...
                  else this.doAddValueToHidden(choice.id, token.id, 'toValuesEdit', true);
                }
        ...
        doPushSpan: function(name, toID, newItem, hideLoc, list){
          ...
          if (newItem){
            ...
            myElement.innerHTML = name+" <a class='notclose' href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValueEdit(\""+toID+"\", \""+hideLoc+"\");'>x</a>";
          }
          else{
            myElement.id = "tospan_"+name+"_"+toID;
            myElement.innerHTML = name+" <a class='notclose' href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValueEdit(\""+toID+"\", \""+hideLoc+"\");'>x</a>";
          }
          scriptJquery('#toValues-wrapper').css('height', 'auto');
          if (list == null) list = "";
...
var OverlayFixEdit = new Class({
	...
	hide: function() {
		if (this.fix) this.fix.css('display', 'none');
		return this;
	},
...
AutocompleterEdit.Base = AutocompleterEdit;
