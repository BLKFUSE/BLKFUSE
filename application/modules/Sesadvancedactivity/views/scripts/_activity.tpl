 ...
 * @version    $Id: _activity.tpl 2017-01-12 00:00:00 socialnetworking.solutions $
...
<li id="activity-item-<?php echo $action->action_id ?>" data-activity-feed-item="<?php echo $action->action_id ?>" class="sesact_pinfeed sesact_pinfeed_hidden sesbasic_clearfix _photo<?php echo $this->userphotoalign; ?><?php if(!empty($fromActivityFeed)){ ?> sescommunityads_ad_id sescmads_ads_listing_item<?php } ?>" <?php if(!empty($fromActivityFeed)){ ?> rel="<?php echo $ad->getIdentity(); ?>"<?php } ?>>
  <?php endif; ?>
      ...
      <?php if(!$this->isOnThisDayPage && !engine_in_array('sesadvancedcomment',$this->enabledModuleNames)){ ?>
      ...
      <div class="sesact_feed_header_cont sesbasic_clearfix">
        ...
        <?php if(!empty($fromActivityFeed) && empty($hideOptions) && $ad->user_id != $this->viewer()->getIdentity()){ ?>
          <div class="sesact_feed_options sesact_pulldown_wrapper">
            <a href="javascript:void(0);" class="sesact_feed_options_btn"><i class="fa fa-ellipsis-h"></i></a>
        ...
        <?php if(empty($fromActivityFeed)  && empty($hideOptions) &&  $this->viewer()->getIdentity() && (empty($this->filterFeed) || $this->filterFeed != 'hiddenpost')){ ?>
          <?php if($this->subject() && $pintotop && $isPinned){ ?>
            <span title="<?php echo $this->translate("Pinned Post"); ?>" class="sesadv_tooltip sesadv_pin_post"><i class="fa fa-thumbtack"></i></span>
          <?php } ?>
          <div class="sesact_feed_options sesact_pulldown_wrapper">
            <a href="javascript:void(0);" class="sesact_feed_options_btn"><i class="fa fa-ellipsis-h"></i></a>
            <div class="sesact_pulldown">
              <div class="sesact_pulldown_cont">
                ...
                <?php if(@$action->type != 'facebookpostembed'): ?>
                  <li><a id="sesact_edit_<?php echo $action->getIdentity(); ?>" href="javascript:;" class="sesact_edit_feed sessmoothbox" data-url="<?php echo $this->url(array('module'=> 'sesadvancedactivity', 'controller'=> 'index', 'action' => 'edit-post', 'action_id' => $action->action_id),'default',true); ?>"><span><?php echo $this->translate("Edit Feed");?></span></a></li>
                <?php endif; ?>
               <?php endif; ?>
               <?php if( $this->viewer()->getIdentity()  && ((
                $this->activity_moderate || (
                  ...
                  <li><a class="sesact_delete_feed sessmoothbox" href="javascript:;" data-url="<?php echo $this->url(array('module'=> 'sesadvancedactivity', 'controller'=> 'index', 'action' => 'delete', 'action_id' => $action->action_id),'default',true); ?>"><span><?php echo $this->translate("Delete Feed");?></span></a></li>
             <?php endif; ?>
            ...
            <?php } ?>
             <?php if(!$detailAction->schedule_time){ ?>
               ...
               <?php if($this->viewer()->getIdentity() != $action->getSubject()->getIdentity()){ ?>
                  ...
                  if($reportEnable): ?>
                    <li>
                      <a href="javascript:;" onclick="openSmoothBoxInUrl('<?php echo $reportLink; ?>')" class="sesadv_report_feed" ><span><?php echo $this->translate("Report"); ?></span></a>
                    </li>
                  <?php endif; ?> 
               <?php } ?>
             <?php }else{ ?>
              ...
              <?php if($feelingposts) { ?>
                <?php
                  ...
                  if(empty($feelingposts->feeling_custom)) {
                    ...
                    ?>
                    <?php echo $this->translate("is "); ?><img title="<?php echo $feelingposts->feeling_customtext; ?>" class="sesfeeling_feeling_icon" src="<?php echo $feeling; ?>"> <?php echo strtolower($feelings->title); ?> <?php echo $feelingposts->feeling_customtext; ?>
                  <?php } 
                  }
                ?>
              <?php } ?>
...
?>
          ...
          <?php if(engine_in_array('sesiosapp',$this->enabledModuleNames) && !empty($actionDetails->posting_type) && $actionDetails->posting_type == 1){ ?>
            <span class="sesbasic_text_light">&middot;</span>
          ...
          <?php }else if(engine_in_array('sesandroidapp',$this->enabledModuleNames) && !empty($actionDetails->posting_type) && $actionDetails->posting_type == 2){ ?>
            <span class="sesbasic_text_light">&middot;</span>
      ...
      <div class='<?php if(defined('SESFEEDBGENABLED') && $actionDetails->feedbg_id && $photo && empty($location) && strlen(strip_tags($contentData[1])) <= $sesAdvancedactivitytextlimit) { ?> feed_background_image <?php } ?>' <?php if(defined('SESFEEDBGENABLED') && $actionDetails->feedbg_id && $photo && empty($location) && strlen(strip_tags($contentData[1])) <= $sesAdvancedactivitytextlimit) { ?> style="background-image:url(<?php echo $photo ?>);" <?php } ?>>
        ...
        <?php } ?>
        <?php if(!empty($detailAction) && $detailAction->reaction_id && engine_in_array('sesadvancedcomment',$this->enabledModuleNames)){ ?> 
        ...
        <?php if(defined('SESFEEDGIFENABLED') && isset($actionDetails->image_id)){ ?> 
          ...
          <?php } ?>
        <?php } ?>
        <?php if( ($action->getTypeInfo()->attachable || engine_in_array('sesfeelingactivity',$this->enabledModuleNames)) && $action->attachment_count > 0 ): // Attachments ?>
          <?php 
                ...
                ?>
                  ...
                  <?php elseif( $attachment->meta->mode == 1 ): // Thumb/text/title type actions ?>
                    ...
                    <div>
                      ...
                      <?php if( $attachment->item->getPhotoUrl() ): ?>
                        ...
                        if(engine_in_array('sesusercoverphoto',$this->enabledModuleNames) && $action->type == 'profile_photo_update') { ?>
                      ...
                      <div>
                        <div>
                          <?php 
                            if(engine_in_array('sesmember',$this->enabledModuleNames) && Engine_Api::_()->getApi('core', 'sesmember')->allowReviewRating() &&  $attachment && !empty($attachment->item->rating) && $attachment->item->rating){
                  ...
                  ?>
                    ...
                    <?php }else{$imageMoreText = '';} ?>
                      ...
                      <?php echo $this->htmlLink($linkHref, $this->itemPhoto($attachment->item, 'thumb.normal', $attachment->item->getTitle(), array('class' => '_sesactpinimg')).$imageMoreText, $imageAttribs) ?>
                      </div>
      ...
      <?php } ?>
          ...
          <?php if($this->viewer()->getIdentity() != 0){ ?>
           <?php if(!$buysell->is_sold){ ?>
            <?php if($action->subject_id != $this->viewer()->getIdentity()){ ?>
              <?php $url = $this->baseUrl().'/sesadvancedactivity/ajax/message/action_id/'.$action->getIdentity(); ?>
              <button onClick="openSmoothBoxInUrl('<?php echo $url; ?>');return false;"><i class="fa fa-comment"></i>Message Seller</button>
            <?php }else{ ?>
        ...
        <?php if(engine_count($getAllHashtags) > 0) { ?>
       ...
       <?php if(!empty($detailAction) && $detailAction->schedule_time && empty($previousAction)){ ?>
        <div class="sesact_feed_schedule_post_time">
        	<?php echo $this->translate("This post will be publish on"); ?> <b><?php echo date('Y-m-d H:i:s',strtotime($detailAction->schedule_time)); ?></b>.       
			   </div>
        <?php } ?>
     </div>
      ...
      <?php if(empty($fromActivityFeed) && empty($_SESSION['fromActivityFeed']) && !empty($detailAction) && !$detailAction->schedule_time && (empty($this->filterFeed) || $this->filterFeed != 'hiddenpost') && empty($previousAction) && $sescommunityads && is_array(Engine_Api::_()->sescommunityads()->allowedTypes($action)) && engine_in_array('boos_post',Engine_Api::_()->sescommunityads()->allowedTypes($action)) && Engine_Api::_()->sescommunityads()->getAllowedActivityType($action->type) && ($action->subject_id == $this->viewer()->getIdentity() && $action->subject_type == "user")){ 
