 ...
 * @version    $Id: _composetargetpost.tpl 2017-01-12 00:00:00 socialnetworking.solutions $
...
scriptJquery(document).on('click','.savevaluessel',function(e){
  if(scriptJquery('#sessmoothbox_main').length)
    var editElem = '_edit';
  else
    var editElem = '';
...
scriptJquery(document).on('click','.removevaluessel',function(e){
  removeTargetPostValues();  
});
scriptJquery(document).on('click','.selected_coun_val',function(e){
  if(scriptJquery('#sessmoothbox_main').length)
    var editElem = '_edit';
  else
    var editElem = '';
