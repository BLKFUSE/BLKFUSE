...
<script type="text/javascript">
  en4.core.runonce.add(function(){
    ...
    var anchor = scriptJquery('#anf_profile_links').parent();
    document.getElementById('anf_profile_links_previous').style.display = '<?php echo ( $this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
    document.getElementById('anf_profile_links_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';

    scriptJquery(document).on('click','#anf_profile_links_previous', function(){
      (scriptJquery.ajax({
    ...
    scriptJquery(document).on('click','#anf_profile_links_next', function(){
      (scriptJquery.ajax({
        url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
        data : {
          format : 'html',
