...
//         $like->save();
  ...
  public function commentAction()
  {
    ...
    if($typeC == 'activity_comment' && empty($sesadcRow)) {
      ...
      foreach($result[2] as $value) {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($item, !empty($guid) ? $guid : $viewer, $viewer, 'sesadvancedcomment_tagged_people', array("commentLink" => $commentLink,'resource_type'=>$action->getType(),'resource_id'=>$action->getIdentity()));
      }
      //Tagging People by status box
  ...
  public function getCommentAction(){
   ...
   public function previewCommentLink($url,$comment,$viewer){
      try{
         $contentLink = Engine_Api::_()->sesadvancedactivity()->previewHTML($url);   
  ...
  public function removepreviewAction() {
    ...
    if($type == 'core_comment') {
      Engine_Api::_()->getDbtable('corecomments', 'sesadvancedactivity')->update(array('showpreview' => 1), array('corecomment_id = ?' => $comment_id));
    } elseif($type == 'activity_comment') {
      Engine_Api::_()->getDbtable('activitycomments', 'sesadvancedactivity')->update(array('showpreview' => 1), array('activitycomment_id = ?' => $comment_id));
    }
    exit();
  }
}
