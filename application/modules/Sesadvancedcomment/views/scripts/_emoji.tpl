...
?>
<?php if(!$this->edit && !empty($show) && engine_in_array('stickers', $enableattachement)){ ?>
<!-- Sickers Search Box -->
<div class="ses_emoji_search_container sesbasic_clearfix emoji_content" id="only_stickers" <?php if(!empty($show) && engine_in_array('stickers', $enableattachement)):?><?php if(engine_count($getGallery) == 0 || empty($enablesearch)): ?> style="display:none;" <?php endif; ?> <?php else: ?> style="display:none;" <?php endif; ?>>
  <?php if(!empty($show) && engine_in_array('stickers', $enableattachement)): ?>
