...
class Sesalbum_Api_Core extends Core_Api_Abstract {
  ...
  public function getHref($albumId = '', $slug = '') {
    if (is_numeric($albumId)) {
			$album = Engine_Api::_()->getItem('album', $albumId);
			if($album)
				$slug = $this->getSlug($album->getTitle());
    }
  ...
  public function photoUrlGet($photo_id, $type = null,$string = '') {
        ...
        //paid content work
				if(Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.sesalbum',1) && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('epaidcontent')) && Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.allow',1) && Engine_Api::_()->epaidcontent()->isViewerPlanActive($photo)) {
				...
				if (Engine_Api::_()->getItem('photo', $photo_id)->getParent()->is_locked && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock'))  && engine_in_array('sesalbum',$sesprofilelock_enable_module) && $photo->owner_id != $viewer->getIdentity() && ($viewer->getIdentity() == 0 || $viewer->level_id != 1))
  ...
  function getPhotoUrl($photo_id = '', $type = 'album_photo') {
      ...
      $viewer = Engine_Api::_()->user()->getViewer();
			...
			if ($viewer->getIdentity() == 0)
				...
				$level = $viewer;
			$sesprofilelock_enable_module = is_string(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesprofilelock.enable.modules', 'a:2:{i:0;s:8:"sesvideo";i:1;s:8:"sesalbum";}')) ? unserialize(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesprofilelock.enable.modules', 'a:2:{i:0;s:8:"sesvideo";i:1;s:8:"sesalbum";}')) : Engine_Api::_()->getApi('settings', 'core')->getSetting('sesprofilelock.enable.modules', 'a:2:{i:0;s:8:"sesvideo";i:1;s:8:"sesalbum";}');
			if (Engine_Api::_()->getItem('photo', $photo_id) && Engine_Api::_()->getItem('photo', $photo_id)->getParent()->is_locked && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock'))  && engine_in_array('sesalbum',$sesprofilelock_enable_module) && $photo->owner_id != $viewer->getIdentity() && ($viewer->getIdentity() == 0 || $viewer->level_id != 1))
				return 'application/modules/Sesalbum/externals/images/locked-album.jpg';
		if( !$photo_id ) {
		 return 'application/modules/Sesalbum/externals/images/nophoto_album_thumb_normal.png';
		}
