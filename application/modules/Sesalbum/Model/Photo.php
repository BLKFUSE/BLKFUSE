...
class Sesalbum_Model_Photo extends Core_Model_Item_Abstract
{
  ...
  public function getRichContent($view = false, $params = array()) {
    ...
    if (!$view) {
      ...
      $view->photo = $this;
      $view->album = Engine_Api::_()->getItem('album', $this->album_id);
      $albumFeed = $view->render('application/modules/Sesalbum/views/scripts/_activityAlbum.tpl');
    }
    return $albumFeed;
  }
  ...
  { 
    ...
    $photo_id = $this->file_id;
		...
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.sesalbum',1) && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('epaidcontent')) && Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.allow',1) && Engine_Api::_()->epaidcontent()->isViewerPlanActive($this)) {
			return Zend_Registry::get('StaticBaseUrl').Engine_Api::_()->getApi('settings', 'core')->getSetting('sesalbum_album_default_adult', 'application/modules/Epaidcontent/externals/images/paidcontent.png');
		}
		...
		$valid = false;
		if(engine_in_array($this->album_id,explode(',',$cookieData))){
			$valid = true;
		}
		if(!$string && !$valid){
				...
				if ( Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock')) && $this->getParent()->is_locked  && engine_in_array('sesalbum',$sesprofilelock_enable_module) && $this->owner_id != $viewer->getIdentity() && ($viewer->getIdentity() == 0 || $viewer->level_id != 1))
