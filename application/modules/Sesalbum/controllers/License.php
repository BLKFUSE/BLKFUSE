...
if ($this->getRequest()->isPost()) {
  ...
  if ($server_output == "OK" && $error != 1) {
	...
	("sesalbum_photo_favourite", "sesalbum", \'{item:$subject} favourite your {item:$object:photo}.\', "0", "", "1"),
	("sesalbum_album_favourite", "sesalbum", \'{item:$subject} favourite your {item:$object:album}.\', "0", "", "1"),
	("sesalbum_album_rating", "sesalbum", \'{item:$subject} rate your {item:$object:album}.\', "0", "", "1"),
	("sesalbum_photo_rating", "sesalbum", \'{item:$subject} rate your {item:$object:photo}.\', "0", "", "1");');
            //email template
            $db->query("INSERT IGNORE INTO `engine4_core_mailtemplates` (`type`, `module`, `vars`) VALUES
('notify_sesalbum_photo_favourite', 'sesalbum', '[host],[email],[recipient_title],[recipient_link],[recipient_photo],[sender_title],[sender_link],[sender_photo],[object_title],[object_link],[object_photo],[object_description]'),('notify_sesalbum_album_favourite', 'sesalbum', '[host],[email],[recipient_title],[recipient_link],[recipient_photo],[sender_title],[sender_link],[sender_photo],[object_title],[object_link],[object_photo],[object_description]'),('notify_sesalbum_album_rating', 'sesalbum', '[host],[email],[recipient_title],[recipient_link],[recipient_photo],[sender_title],[sender_link],[sender_photo],[object_title],[object_link],[object_photo],[object_description]'),('notify_sesalbum_albumrated', 'sesalbum', '[host],[email],[recipient_title],[recipient_link],[recipient_photo],[sender_title],[sender_link],[sender_photo],[object_title],[object_link],[object_photo],[object_description]');");

         $db->query('DROP TABLE IF EXISTS `engine4_sesalbum_albums`');
         $db->query('CREATE TABLE IF NOT EXISTS `engine4_sesalbum_albums` (
...
pellentesque erat orci, non mollis purus ornare a. Ut a blandit dolor. Quisque ac pharetra ex. Aliquam pretium pharetra elementum. Phasellus nec mollis metus, non pellentesque purus. Vivamus in sem facilisis, dictum ex suscipit, imperdiet tortor. Sed varius massa ex, quis porta elit interdum non. Mauris at dictum nisi. Maecenas malesuada diam sit amet turpis porttitor, ut aliquam nibh facilisis. Ut sit amet ligula lacus.<br><br>In hac habitasse platea dictumst. Cras mollis sagittis feugiat. Nunc ac velit eu turpis congue lobortis. Pellentesque quam diam, feugiat vitae ipsum sit amet, aliquet vestibulum ligula. Sed nulla risus, malesuada blandit egestas vel, semper a risus. Pellentesque et tincidunt mauris. Nunc sodales diam dictum, sollicitudin leo nec, dapibus sapien. Suspendisse a fringilla urna. Quisque luctus neque tristique, cursus nulla ac, egestas felis. Proin dapibus condimentum posuere. Aenean lacinia volutpat convallis. In gravida, elit eu imperdiet venenatis, lacus risus venenatis quam, at consectetur tortor tortor malesuada enim. Nam hendrerit ipsum vel odio molestie rutrum. Vivamus vitae risus eget est vehicula consequat. In varius nec dolor eu aliquet.'));
            ...
            $db->query('DROP TABLE IF EXISTS `engine4_sesalbum_categories`');
                $db->query('CREATE TABLE IF NOT EXISTS `engine4_sesalbum_categories` (
          ...
          KEY `category_name` (`category_name`)
            ...
            if (empty($table_exist_categories)) {
                foreach ($catgoryData as $key => $value) {
                    ...
                    $db->query("INSERT IGNORE INTO `engine4_sesalbum_categories` (`category_name`,`subcat_id`,`subsubcat_id`,`slug`,`description`) VALUES ( '" . $value[0] . "',0,0,'','" . $value[3] . "')");
                    ...
                    if (is_file($PathFile . "banners" . DIRECTORY_SEPARATOR . $value[1]))
                        ...
                        $thumbnail_icon = 0;
                    $db->query("UPDATE `engine4_sesalbum_categories` SET `cat_icon` = '" . $cat_icon . "',`thumbnail` = '" . $thumbnail_icon . "' WHERE category_id = " . $catId);
                    $valueName = str_replace(array(' ', '&'), array('', ''), $value[0]);
                    if (isset(${$valueName})) {
                        foreach (${$valueName} as $value) {
                            $db->query("INSERT IGNORE INTO `engine4_sesalbum_categories` (`category_name`,`subcat_id`,`subsubcat_id`,`slug`,`description`) VALUES ( '" . $value[0] . "','" . $catId . "',0,'','" . $value[3] . "')");
                            ...
                            if (is_file($PathFile . "banners" . DIRECTORY_SEPARATOR . $value[1]))
                                ...
                                $thumbnail_icon = 0;
                            $db->query("UPDATE `engine4_sesalbum_categories` SET `cat_icon` = '" . $cat_icon . "',`thumbnail` = '" . $thumbnail_icon . "' WHERE category_id = " . $subId);
                            $valueSubName = str_replace(array(' ', '&'), array('', ''), $value[0]);
                            if (isset(${$valueSubName})) {
                                foreach (${$valueSubName} as $value) {
                                    $db->query("INSERT IGNORE INTO `engine4_sesalbum_categories` (`category_name`,`subcat_id`,`subsubcat_id`,`slug`,`description`) VALUES ( '" . $value[0] . "','0','" . $catId . "','','" . $value[3] . "')");
                                    ...
                                    if (is_file($PathFile . "banners" . DIRECTORY_SEPARATOR . $value[1]))
                                        ...
                                        $thumbnail_icon = 0;
                                    $db->query("UPDATE `engine4_sesalbum_categories` SET `cat_icon` = '" . $cat_icon . "',`thumbnail` = '" . $thumbnail_icon . "' WHERE category_id = " . $subsubId);
                                }
                            }
                        }
                    }
                ...
                $table_exist_categories = $db->query('SHOW TABLES LIKE \'engine4_album_categories\'')->fetch();
                if (!empty($table_exist_categories)) {
                    $db->query("INSERT IGNORE INTO engine4_sesalbum_categories (category_name)  SELECT category_name FROM engine4_album_categories");
                }
            }
                $db->query("UPDATE `engine4_sesalbum_categories` set `title` = category_name where title = ''");
                $db->query("UPDATE `engine4_sesalbum_categories` set `slug` = LOWER(REPLACE(REPLACE(REPLACE(category_name,'&',''),'  ',' '),' ','-')) where slug = ''");
                $db->query("UPDATE `engine4_sesalbum_categories` SET `order` = `category_id` WHERE `order` = 0");

            if (empty($runInstallCategory)) {
                foreach ($catgoryData as $key => $value) {
                    //Upload categories icon
                    $catId = $db->query("SELECT category_id,thumbnail,cat_icon FROM `engine4_sesalbum_categories` WHERE category_name = '" . $value[0] . "'")->fetchAll();
                            ...
                            if(isset(${$valueSubName})){
                                foreach(${$valueSubName} as $value){
                                    ...
                                    if (is_file($PathFile . "banners" . DIRECTORY_SEPARATOR . $value[1]))
                                        ...
                                        $thumbnail_icon = 0;
                                    $db->query("UPDATE `engine4_sesalbum_categories` SET `cat_icon` = '" . $cat_icon . "',`thumbnail` = '" . $thumbnail_icon . "' WHERE category_id = " . $subsubId);
                                    $db->query("UPDATE `engine4_sesalbum_categories` set `title` = category_name WHERE category_id = ".$subsubId );
                                    $db->query("UPDATE `engine4_sesalbum_categories` set `slug` = LOWER(REPLACE(REPLACE(REPLACE(category_name,'&',''),'  ',' '),' ','-'))  WHERE category_id = ".$subsubId);
                                    $db->query("UPDATE `engine4_sesalbum_categories` SET `order` = `category_id`  WHERE category_id = ".$subsubId);
                                }
                            }
                            $db->query("UPDATE `engine4_sesalbum_categories` set `title` = category_name WHERE category_id = ".$subId );
                            $db->query("UPDATE `engine4_sesalbum_categories` set `slug` = LOWER(REPLACE(REPLACE(REPLACE(category_name,'&',''),'  ',' '),' ','-'))  WHERE category_id = ".$subId);
                            $db->query("UPDATE `engine4_sesalbum_categories` SET `order` = `category_id`  WHERE category_id = ".$subId);
                        }
                    }
                    $db->query("UPDATE `engine4_sesalbum_categories` set `title` = category_name WHERE category_id = ".$catId );
                    $db->query("UPDATE `engine4_sesalbum_categories` set `slug` = LOWER(REPLACE(REPLACE(REPLACE(category_name,'&',''),'  ',' '),' ','-'))  WHERE category_id = ".$catId);
                    $db->query("UPDATE `engine4_sesalbum_categories` SET `order` = `category_id`  WHERE category_id = ".$catId);
                }
            }
            //check 0 id category.
            $db->query("DELETE FROM `engine4_sesalbum_categories` WHERE category_id = 0 AND `category_name` = 'All Categories'");
      ...
      ("comment_album_photo", "sesalbum", \'{item:$subject} commented on {item:$owner}\'\'s {item:$object:photo}: {body:$body}\', 1, 1, 1, 1, 1, 0),
			("sesalbum_album_rating", "sesalbum", \'{item:$subject} rated album {item:$object}:\', 1, 5, 1, 1, 1, 1),
			("sesalbum_photo_rating", "sesalbum", \'{item:$subject} rated photo {item:$object}:\', 1, 5, 1, 1, 1, 1),
			("sesalbum_album_favourite", "sesalbum", \'{item:$subject} added album {item:$object} to favourite:\', 1, 5, 1, 1, 1, 1),
			("sesalbum_photo_favourite", "sesalbum", \'{item:$subject} added photo {item:$object} to favourite:\', 1, 5, 1, 1, 1, 1)');
            $db->query('INSERT IGNORE INTO `engine4_core_menus` (`name`, `type`, `title`) VALUES
      ("sesalbum_main", "standard", "SNS - Advanced Photos - Album Main Navigation Menu"),
    ...
    $domain_name = @base64_encode(str_replace(array('http://','https://','www.'),array('','',''),$_SERVER['HTTP_HOST']));
		...
		Engine_Api::_()->getApi('settings', 'core')->setSetting('sesalbum.sesdomainauth', $domain_name);
		Engine_Api::_()->getApi('settings', 'core')->setSetting('sesalbum.seslkeyauth', $licensekey);
  } else {
