        ...
        function _helper(lastRow){
            if (o.maxRows && rows > o.maxRows || o.truncate && lastRow) {
							if(getX == 0){
								...
								if(isComplete)
									return;
								scriptJquery('#sesalbum_slideshow'+identitySlideshow).append('<div id="idDiv_'+idDiv+'" style="float:left"></div>');
							 }
							scriptJquery('#idDiv_'+idDiv).append(scriptJquery('#'+row[x][0].attr('id'))[0].outerHTML);
							getX++;
							if(row[x][0].attr('id') == 'sesalbum_slideshow_id_'+itemCountSlideshow){
								getX = 0;
								row[x][0].remove();
									scriptJquery("#idDiv_"+idDiv).flexImagesSlideshow({maxRows:maxRowsSlideshow});
									return;
							}
							row[x][0].remove();
						}else {
