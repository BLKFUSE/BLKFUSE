  ...
  <?php foreach( $this->paginatorCategory as $item): ?>
    ...
    <?php if($this->view_type == 1){ ?> 
      ...
      <div class="sesalbum_categories_albums_conatiner clear sesbasic_clearfix sesbm">
        <div class="sesalbum_categories_albums_item sesbasic_clearfix clear <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.sesalbum',1) && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('epaidcontent')) && Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.allow',1) && Engine_Api::_()->epaidcontent()->isViewerPlanActive($albumData)) { ?> paid_content <?php } ?>">
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.sesalbum',1) && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('epaidcontent')) && Engine_Api::_()->getApi('settings', 'core')->getSetting('epaidcontent.allow',1) && Engine_Api::_()->epaidcontent()->isViewerPlanActive($albumData)) { ?>
      <?php echo $this->partial('application/modules/Epaidcontent/views/scripts/_paidContent.tpl', 'epaidcontent', array('item' => $albumData)); ?>
    <?php } ?>
              ...
              <?php if(isset($this->titleActive)){ ?>
                ...
                <div class="sesalbum_cat_album_list_stats sesalbum_list_stats sesbasic_text_light">
                  <?php if(isset($this->likeActive) && isset($itemAlbum->like_count)) { ?>
                    <span title="<?php echo $this->translate(array('%s like', '%s likes', $itemAlbum->like_count), $this->locale()->toNumber($itemAlbum->like_count)); ?>"><i class="sesbasic_icon_like_o"></i><?php echo $itemAlbum->like_count; ?></span>
                  <?php } ?>
                  <?php if(isset($this->commentActive) && isset($itemAlbum->comment_count)) { ?>
                    <span title="<?php echo $this->translate(array('%s comment', '%s comments', $itemAlbum->comment_count), $this->locale()->toNumber($itemAlbum->comment_count))?>"><i class="sesbasic_icon_comment_o"></i><?php echo $itemAlbum->comment_count;?></span>
                  <?php } ?>
                  <?php if(isset($this->viewActive) && isset($itemAlbum->view_count)) { ?>
                    <span title="<?php echo $this->translate(array('%s view', '%s views', $itemAlbum->view_count), $this->locale()->toNumber($itemAlbum->view_count))?>"><i class="sesbasic_icon_view"></i><?php echo $itemAlbum->view_count; ?></span>
                  <?php } ?>
                   <?php  if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesalbum.allowfavourite', 1) && isset($this->favouriteActive) && isset($itemAlbum->favourite_count)) { ?>
                    <span title="<?php echo $this->translate(array('%s favourite', '%s favourites', $itemAlbum->favourite_count), $this->locale()->toNumber($itemAlbum->favourite_count))?>"><i class="sesbasic_icon_favourite_o"></i><?php echo $itemAlbum->favourite_count; ?></span>
                  <?php } ?>
                  <?php  if(isset($this->albumCountActive)) { ?>
...
scriptJquery (document).on('click','.seschanel_slideshow_prev',function(e){
		...
		scriptJquery (this).parent().parent().parent().find('.sesalbum_categories_albums_listing_thumbnails').children().eq(changeIndex).find('a').click();
});
...
scriptJquery (document).on('click','.slideshow_album_data',function(e){
	e.preventDefault();
	var album_id = scriptJquery (this).attr('data-url');
	 ...
	 scriptJquery.ajax({
      ...
      'data': {
        format: 'html',
				...
				album_id : scriptJquery (this).attr('data-url'),
      },
