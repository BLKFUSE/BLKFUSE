...
<script type="application/javascript">
	...
	scriptJquery('#sesalbum_title').html(scriptJquery('#tab-widget-sesalbum-<?php echo $randonNumber; ?>').find('.sesbasic_tab_selected').find('a').html()).show();
</script>
<?php } ?>
...
<?php if($this->load_content == 'auto_load' && $this->show_limited_data == 'no'){ ?>
        ...
        if(fromtop_<?php echo $randonNumber; ?> > heightOfContentDiv_<?php echo $randonNumber; ?> - 100 && scriptJquery('#view_more_<?php echo $randonNumber; ?>').css('display') == 'block' && scriptJquery('.tab_layout_sesalbum_profile_albums').hasClass('active')){
...
<script type="text/javascript">
	...
	function changeTabSes_<?php echo $randonNumber; ?>(valueTab){
      ...
      if(document.getElementById("loading_image_<?php echo $randonNumber; ?>"))
			...
			if(scriptJquery("#sesTabContainer_<?php echo $randonNumber ?>_"+valueTab).hasClass('sesbasic_tab_selected'))
				return;
