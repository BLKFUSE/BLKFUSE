...
$module_name = 'sesandroidapp';

//product title and module title.
$module_title = 'Android Native Mobile App Plugin';
...
if ($this->getRequest()->isPost()) {
  ...
  //license key
  $postdata['licenseKey'] = @base64_encode($_POST['sesandroidapp_licensekey']);
  ...
  if ($server_output == "OK" && $error != 1) {
    ...
    if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesandroidapp.pluginactivated')) {
      ...
      Engine_Api::_()->getApi('settings', 'core')->setSetting('sesandroidapp.licensekey', $_POST['sesandroidapp_licensekey']);
    }
    $domain_name = @base64_encode(str_replace(array('http://','https://','www.'),array('','',''),$_SERVER['HTTP_HOST']));
		$licensekey = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesandroidapp.licensekey');
		$licensekey = @base64_encode($licensekey);
		Engine_Api::_()->getApi('settings', 'core')->setSetting('sesandroidapp.sesdomainauth', $domain_name);
		Engine_Api::_()->getApi('settings', 'core')->setSetting('sesandroidapp.seslkeyauth', $licensekey);
		$error = 1;
  } else {
