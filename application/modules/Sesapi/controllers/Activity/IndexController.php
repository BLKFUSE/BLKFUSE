...
class Activity_IndexController extends Sesapi_Controller_Action_Standard {
  ...
  public function gifAction() {
	...
	function coreactivity(){
      ...
      foreach($composePartials as $partial ):
        if(false !== strpos($partial[0], '_composeTag') && !engine_in_array('userTags', $activitycomposerOptions)) {
          continue;
        }
        // For core modules
        ...
        if(false !== strpos($partial[0], '_composePhoto')) {
         ...
         // For core  music
        if(false !== strpos($partial[0], '_composeMusic') && _SESAPI_VERSION_IOS == 0) {
        ...
        // For sesmusic
        if(false !== strpos($partial[0], '_composeMusic') && _SESAPI_VERSION_IOS == 0 && Engine_Api::_()->sesapi()->isModuleEnable('sesmusic')) {
          $composerOptions['addMusic'] = $this->view->translate('Add Music');
        }
        // For sesvideo
        if(false !== strpos($partial[0], '_composeVideo') && Engine_Api::_()->sesapi()->isModuleEnable('sesvideo') && (!$subject || ($subject->getType() != 'sesgroup_group' && $subject->getType() != 'businesses'))) {
          $composerOptions['addVideo'] = $this->view->translate('Add Video');
        }

		if($subject && $subject->getType() == 'sesgroup_group' && Engine_Api::_()->sesapi()->isModuleEnable('sesgroupvideo')){
			$composerOptions['addVideo'] = $this->view->translate('Add Video');
		}

		if($subject && $subject->getType() == 'businesses' && Engine_Api::_()->sesapi()->isModuleEnable('sesbusinessvideo')){
			$composerOptions['addVideo'] = $this->view->translate('Add Video');
		}

        // For sespagepoll
        if ($subject && $subject->getType() == 'sespage_page' && (false !== strpos($partial[0], '_composeSespagepoll')) && Engine_Api::_()->sesapi()->isModuleEnable('sespagepoll') && ( !defined('_SESAPI_VERSION_IOS') || _SESAPI_VERSION_IOS >= 9.3)) {
          $composerOptions['addPoll'] = $this->view->translate("Add Poll");
        }
        // For sesgrouppoll
        if ($subject && $subject->getType() == 'sesgroup_group' && (false !== strpos($partial[0], '_composeSesgrouppoll')) && Engine_Api::_()->sesapi()->isModuleEnable('sesgrouppoll')  && ( !defined('_SESAPI_VERSION_IOS') || _SESAPI_VERSION_IOS >= 9.3)) {
          $composerOptions['addPoll'] = $this->view->translate("Add Poll");
        }
        if ($subject && $subject->getType() == 'businesses' && (false !== strpos($partial[0], '_composeSesbusinesspoll')) && Engine_Api::_()->sesapi()->isModuleEnable('sesbusinesspoll')  && ( !defined('_SESAPI_VERSION_IOS') || _SESAPI_VERSION_IOS >= 9.3)) {
          $composerOptions['addPoll'] = $this->view->translate("Add Poll");
        }
      endforeach;
			// for live streaming.
      if ((_SESAPI_VERSION_IOS < 1.8 && _SESAPI_PLATFORM_SERVICE == 1) || (_SESAPI_VERSION_ANDROID < 3.1 && _SESAPI_PLATFORM_SERVICE == 2)) {
			...
			foreach ($composerOptions as $key => $option) {
				...
				$contentResponse['composerOptions'][$counter]['from_device'] = true; 
  ...
  function sesadvancedactivity()
  {
    ...
    if ($emojiPluginEnable && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesfeelingactivity.enablefeeling', 1) && Engine_Api::_()->authorization()->isAllowed('sesfelngactvity', null, 'enablefeeling')) {
      ...
      if (engine_in_array('1', $enableFeelingsCategories)) {
      $feelingEnable = true;
    ...
    if($allownetworkprivacy == 1){
      ...
      $enableComposers = (array) $levelAdapter->getAllowed('sesadvactivity', $this->view->viewer(), 'composeroptions');
	  //check module enable
	  $listsArray = array();
	...
	if ($viewer->getIdentity() && !$this->_getParam('action_id')) {
      ...
      if ((defined('_SESAPI_VERSION_IOS') > 2.1 && SESAPI_PLATFORM_SERVICE == 1 && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('eioslivestreaming')) || (_SESAPI_VERSION_ANDROID < 3.1 && _SESAPI_PLATFORM_SERVICE == 2)) {
    ...
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedactivity') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesfeedgif.giphyapi', '')){
	  ...
	  if (engine_in_array('locationses', $getEnableComposers)) {
		$composerOptions['checkIn'] = $this->view->translate("Check In");
	  }
	  if (engine_in_array('sesadvancedactivitylink', $getEnableComposers)) {
	  ...
	  if (engine_in_array('tagUseses', $getEnableComposers)) {
		$composerOptions['tagPeople'] = $this->view->translate("Tag People");
	  }
	  if (engine_in_array('emojisses', $getEnableComposers) || engine_in_array('feelingssctivity', $getEnableComposers)) {
      ...
      $enableComposers = (array) $levelAdapter->getAllowed('sesadvactivity', $this->view->viewer(), 'composeroptions');
       ...
       if(!engine_in_array('sesadvancedactivitytargetpost', $enableComposers)) {
            //unset($composerOptions['scheduledPost']);
       }
	...
	if (engine_count($textColorChange)) {
    ...
    if(engine_in_array('enablefeedbg', $enablefeedbg)) {
	  ...
	  if ($sesfeedbg_enablefeedbg) {
		...
		$getBackgrounds = Engine_Api::_()->getDbTable('backgrounds', 'sesfeedbg')->getBackgrounds(array('admin' => 1, 'fetchAll' => 1, 'sesfeedbg_limit_show' => $sesfeedbg_limit_show, 'feedbgorder' => '', 'featuredbgIds' => $featured));
