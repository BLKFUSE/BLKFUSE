    ...
    foreach ($notifications as $notification) {
      try {
        ...
        if (strpos($notification->type, '_tagged_') !== false || strpos($notification->type, 'commented') !== false || strpos($notification->type, 'liked') !== false || strpos($notification->type, '_like') !== false || strpos($notification->type, '_reacted_') !== false)
          ...
          $isCommentLike = false;
        $result['notification'][$counterLoop] = array('notification_id' => $notification->getIdentity(),'subject_type'=>$notification->subject_type, 'object_id' => $notification->object_id, 'subject_id' => $notification->subject_id, 'notification_icon' => $notificationIcon, 'title' => strip_tags($title), 'body' => '', 'user_image' => $image, 'href' => $href, 'object_type' => $notification->object_type, 'read' => $notification->read, 'date' => strip_tags((($notification->date))), 'isCommentLike' => $isCommentLike);
        ...
        }else if ($notification->object_type == "activity_action" || $notification->object_type == "sesadvancedactivity_action") {
