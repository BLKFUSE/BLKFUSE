  ...
  public function browseAction(){
    ...
    if ($page == 1 && @$_POST['filter_sort'] == 'classroom_main_browse') { 
        ...
        if (!isset($params['category_id'])) {
          ...
          if (engine_count($this->getPopularClassrooms($paginator))) {
  ...
  try {
    ...
    if(!$classroom){
	 ...
	 // check for claim already exist or not
		...
		if(engine_count($claimedClassroom->toArray()) >0){
...
//           }
  ...
  public function getInformation($classroom){
        ...
        if (is_countable($likeMembers) && engine_count($likeMembers)) {
            $likeCounter = 0;
  ...
  public function moreMembersAction(){
    ...
    if(engine_count($Members) && $classroomcheck) {
    ...
    if (engine_count($Members) && !$classroomcheck && $this->_getParam('type',null) != 'like') {
