...
class Courses_IndexController extends Sesapi_Controller_Action_Standard {
  ...
  try {
    ...
    if($form->getElement('course_list')){
      $form->removeElement('course_list');
    }
      ...
      if (!empty($_FILES['image'])) {
        ...
        $wishlist->setPhoto($_FILES["image"], 'mainPhoto');
      }
...
// 		$form->getElement('productstyle')->setValue($course->style);
  ...
  public function overviewAction(){
	...
	public function profileUpsellAction(){
		...
		$paginator->setItemCountPerPage($this->_getParam('limit', 10));
    $paginator->setCurrentPageNumber($this->_getParam('page', 1));
		$data['upsell_course'] = engine_count($paginator) ? $this->getCourses($paginator) : array();
		$extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
		$extraParams['pagging']['total'] = $paginator->getTotalItemCount();
		$extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
		$extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
  ...
  public function crosssellCoursesAction(){
    $cartData = Engine_Api::_()->courses()->cartTotalPrice();
		...
		$paginator->setItemCountPerPage($this->_getParam('limit', 10));
    $paginator->setCurrentPageNumber($this->_getParam('page', 1));
		$data['crosssell_course'] = engine_count($paginator) ? $this->getCourses($paginator) : array();
		$extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
		$extraParams['pagging']['total'] = $paginator->getTotalItemCount();
		$extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
		$extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
