...
class Elivestreaming_IndexController extends Core_Controller_Action_Standard
{
  public function cancelAction()
  {
    ...
    public function commentsAction(){
        ...
        $comments['host']['image'] = Engine_Api::_()->sesapi()->getBaseUrl(true,$user->getPhotoUrl() ? $user->getPhotoUrl() : "/application/modules/User/externals/images/nophoto_user_thumb_profile.png");
        $comments['host']['name'] = $user->getTitle();
        $comments['host']['title'] = "is live now.";
        $comments['host']['href'] = Engine_Api::_()->sesapi()->getBaseUrl(true,$user->getHref());


        $time = strtotime($elivehostItem->datetime);
        $oldTimezone = date_default_timezone_get();
        date_default_timezone_set($user->timezone);
        
        $comments['host']['creation_date'] = date("Y-m-d H:i:s",$time);
        ...
        if($viewer->getIdentity()) {
            $comments['user']['image'] = Engine_Api::_()->sesapi()->getBaseUrl(true,$viewer->getPhotoUrl());
            $comments['user']['name'] = $viewer->getTitle();
            $comments['user']['href'] = Engine_Api::_()->sesapi()->getBaseUrl(true,$viewer->getHref())
            ;
        }
  ...
  public function getPermissionAction()
  {
    ...
    if (engine_in_array("sesadvancedactivity", $permission))
      $result['canPost'] = true;
    if (engine_in_array("sesstories", $permission))
