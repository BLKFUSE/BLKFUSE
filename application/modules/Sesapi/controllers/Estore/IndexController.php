...
//           }
  ...
  public function getstateAction(){ 
      ...
      Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error'=>'0', 'result' => array("states"=>$results)));      
  }
  ...
  public function billingAction() {
    ...
    if(engine_count($billingAddressArray)){
...
//     }
  ...
  public function shippingAction() {
    ...
    $this->view->form = $form = new Estore_Form_Billing();
    if(engine_count($shippingAddressArray))
  ...
  public function myorderAction() {
    ...
    public function browseAction(){
        ...
        if ($page == 1 && $_POST['filter_sort'] == 'estore_main_browse') {
            ...
            if (!isset($params['category_id'])) {
                ...
                if (engine_count($this->getPopularStores($paginator))) {
   ...
   public function getContentFollowCount($subject,$tableName = "",$modulename = "",$resources_type = "",$column_name = "resource_id"){
    ...
    public function likeAction()
    {
        ...
        if (!empty($result)) {
            ...
            if ($type == 'estore_store') {
                ...
                if (engine_count($storeFollowers) > 0) {
    ...
    public function getstore($store)
    {
        ...
        $storedata['currency'] = Engine_Api::_()->sesproduct()->getCurrencySymbol(Engine_Api::_()->estore()->getCurrentCurrency());
...
function getNavigation($store,$viewer){
  ...
  public function userAge($member){
    ...
    public function getInformation($stores)
    {
        ...
        if (engine_count($likeStoreResult)) {
            $likeStoresCounter = 0;
            $result['total_page_liked_by_this_store'] = engine_count($likeStoreResult) > 4 ? engine_count($likeStoreResult) - 4 : 0;
...
//         $member->userImage($member->getIdentity(),'thumb.profile');
    ...
    public function servicesAction()
    {
        ...
        $servicesCounter = 0;
        $currency = Engine_Api::_()->sesproduct()->getCurrencySymbol(Engine_Api::_()->estore()->getCurrentCurrency());
  ...
  function likeVideoAction() {
   ...
   function favouriteVideoAction() {
    ...
    //Process form
      ...
      $paginator->setItemCountPerPage(100);
      $paginator->setCurrentPageNumber($page);
      $result = $this->getStoreReview($paginator);
      $extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
      $extraParams['pagging']['total'] = $paginator->getTotalItemCount();
      $extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
      $extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
    ...
    protected function getStoreReview($paginator)
    {
    ...
    public function filtersearchAction() {
      ...
      $form->removeElement('product_title');
      if ($this->_getParam('getForm')) {

        $formFields = Engine_Api::_()->getApi('FormFields', 'sesapi')->generateFormFields($form);
        $this->generateFormFields($formFields, array('resources_type' => 'store'));
      } else {
        Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array_merge(array('error' => '', 'error_message' => '', 'result' => $result), $extraParams));
      }
    }
