       ...
       // $form->setDescription('Are you sure that you want to delete this review? It will not be recoverable after being deleted.');
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                if (is_countable($parameters) && engine_count($parameters) > 0) {
...
//       }
	...
	/* 	if(!isset($_POST['type']) || !$_POST['type'])
	  ...
	  /* 
		...
		try
		{
			...
			if( is_countable($action->toArray()) && engine_count($action->toArray()) <= 0 && $values['draft'] == '0' && (!$sesproduct->publish_date || strtotime($sesproduct->publish_date) <= time())) {
	...
	public function getWishlists($paginator){
        ...
        $result[$counter]['images'] = array('main' =>  $this->getBaseUrl(false,Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct_wishlist_default_image', "application/modules/Sesproduct/externals/images/nophoto_wishlist_thumb_profile.png")));
			}
...
//                 }
    ...
    public function viewAction(){
		...
		} else if(!empty($sesproduct->photo_id)) {
      $file = Engine_Api::_()->getItem('storage_file',$sesproduct->photo_id);	
      $result['slider_images'][$photoCounter]['type'] = 'image';
      $result['slider_images'][$photoCounter]['value'] = $this->getBaseUrl(true,$file->map());
		} else {
			$result['slider_images'][$photoCounter]['type'] = 'image';
    ...
    $optionCounter++;
		...
		if(engine_in_array('paypal',$paymentMethods)){
		...
		if(engine_in_array('stripe',$paymentMethods)){
			$productdata["payment_methods"][$paymentMethodsCounter]['label'] = $this->view->translate('Pay With Stripe');
			$productdata["payment_methods"][$paymentMethodsCounter]['image'] = $this->getBaseUrl(true,'/application/modules/Sesproduct/externals/images/stripe.png');
			$paymentMethodsCounter++;
			
		}
		if(engine_in_array('paytm',$paymentMethods)){
			$productdata["payment_methods"][$paymentMethodsCounter]['label'] = $this->view->translate('Pay With Paytm');
			$productdata["payment_methods"][$paymentMethodsCounter]['image'] = $this->getBaseUrl(true,'/application/modules/Sesproduct/externals/images/paytm.png');
			$paymentMethodsCounter++;
			
		}
		if(engine_in_array('cashfree',$paymentMethods)){
			$productdata["payment_methods"][$paymentMethodsCounter]['label'] = $this->view->translate('Pay With Cash Free');
			$productdata["payment_methods"][$paymentMethodsCounter]['image'] = $this->getBaseUrl(true,'/application/modules/Sesproduct/externals/images/cashfree.png');
			$paymentMethodsCounter++;
			
		}
		if(engine_in_array('0',$paymentMethods)){
		...
		if(engine_in_array('1',$paymentMethods)){
      ...
      $productLink = Engine_Api::_()->sesproduct()->productPurchaseable($product);
	...
	public function claimAction(){
	 ...
	 // check for claim already exist or not
		...
		if(engine_count($claimedStores->toArray()) >0){
  ...
  /*if ($canJoin) {
	...
	*/
	...
	public function moreMembersAction(){
	   ...
	   if(engine_count($Members) && $storecheck) {
			$likeStoresCounter = 0;
	 ...
	 if (engine_count($Members) && !$storecheck && $this->_getParam('type',null) != 'like') {
	...
	public function getInformation($product){
		...
		if(engine_in_array('stripe',$paymentMethods)){
			$result['product_info'][$counter]["payment_methods"][$paymentMethodsCounter]['label'] = $this->view->translate('Pay With Stripe');
			$result['product_info'][$counter]["payment_methods"][$paymentMethodsCounter]['image'] = $this->getBaseUrl(true,'/application/modules/Sesproduct/externals/images/stripe.png');
			$paymentMethodsCounter++;
			
		}
		if(engine_in_array('paytm',$paymentMethods)){
			$result['product_info'][$counter]["payment_methods"][$paymentMethodsCounter]['label'] = $this->view->translate('Pay With Paytm');
			$result['product_info'][$counter]["payment_methods"][$paymentMethodsCounter]['image'] = $this->getBaseUrl(true,'/application/modules/Sesproduct/externals/images/paytm.png');
			$paymentMethodsCounter++;
			
		}
		if(engine_in_array('cashfree',$paymentMethods)){
			$result['product_info'][$counter]["payment_methods"][$paymentMethodsCounter]['label'] = $this->view->translate('Pay With Cash Free');
			$result['product_info'][$counter]["payment_methods"][$paymentMethodsCounter]['image'] = $this->getBaseUrl(true,'/application/modules/Sesproduct/externals/images/cashfree.png');
			$paymentMethodsCounter++;
...
//          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>'permission_error', 'result' => array()));
	...
	public function profileUpsellAction(){
		...
		$data['upsell_product'] = engine_count($paginator) ? $this->getProducts($paginator) : array();
    ...
    public function viewVideoAction(){
		...
		if($viewer->getIdentity() != 0){
			...
			if(engine_count($resourceItem)>0)
  ...
  //item liked as per item tye given
	...
	public function cancelAction(){
     ...
     public function myproductreviewAction()
 {
    $searchArray['title'] = isset($_POST['title']) ? $_POST['title'] : '';
      $searchArray['rating_star'] = isset($_POST['rating_star']) ? $_POST['rating_star'] : '';
      $searchArray['product_title'] = isset($_POST['product_title']) ? $_POST['product_title'] : '';
    $viewer = Engine_Api::_()->user()->getViewer();
    $viewer_id = $viewer->getIdentity();
   ...
   $productTable = Engine_Api::_()->getDbTable('sesproducts', 'sesproduct');
    ...
    $paginator->setCurrentPageNumber($page);
     $result = $this->getProductReview($paginator);
    $extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
    $extraParams['pagging']['total'] = $paginator->getTotalItemCount();
    $extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
    $extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
  ...
  protected function getProductReview($paginator)
  {
    ...
    foreach ($paginator as $review) {
      ...
      $result['reviews'][$counter]= $review->toArray();
      $result['reviews'][$counter]['owner']['id'] = $owner->getIdentity();
      $result['reviews'][$counter]['owner']['Guid'] = $owner->getGuid();
      $result['reviews'][$counter]['owner']['title'] = $owner->getTitle();
      $result['reviews'][$counter]['owner']['images'] = $this->getBaseUrl(true, $owner->getPhotoUrl());
   ...
   public function filtersearchAction() {
      $viewer = Engine_Api::_()->user()->getViewer();
      $form = new Estore_Form_Admin_Review_Filter();
       $form->removeElement('store_title');
