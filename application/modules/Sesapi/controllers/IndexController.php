...
class Sesapi_IndexController extends Sesapi_Controller_Action_Standard
{
  ...
  function appDefaultDataAction(){
      ...
      if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedactivity')){
        $coreModules["sesadvancedactivity"] = "sesadvancedactivity";
      }
      if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedactivity')){
        $coreModules["sesadvancedactivity"] = "sesadvancedactivity";
     ...
     // Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'0','error_message'=>'', 'result' => "Item Liked Successfully"));
       ...
       //if($subject->getType() == "album_photo"){
        ...
        $resultData =  $table->fetchAll($select);
            ...
            $reactionData = array();
            $reactionCounter = 0;
            if(engine_count($resultData) && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedcomment')){
  ...
  protected function _likes($resource_id,$resource_type){
    ...
    if($sesAdv){
      ...
      //get Comments
      $reverseOrder = false;
      $canComment = $subject->getType() == "activity_action" || $subject->getType() == "sesadvancedactivity_action" ? true : $subject->authorization()->isAllowed($viewer, 'comment');
      ...
      $commentSettings = unserialize(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesadvancedcomment.enableattachement', 'a:3:{i:0;s:6:"photos";i:1;s:6:"videos";i:2;s:8:"emotions";}'));
      if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedactivity') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesfeedgif.giphyapi', '') && engine_in_array('gif', $commentSettings)) {
        $comments['enable']['is_gif'] = true;
      } else {
        $comments['enable']['is_gif'] = false;
      }
...
$text .= '&nbsp;<img src="https://blkfuse.com/application/modules/Sesbasic/externals/images/verify.png" />';
      ...
      if($activitycomments && isset($activitycomments->gif_url) && $activitycomments->gif_url) {
        $array[$counter]['gif_url'] = $activitycomments->gif_url;
  ...
  public function createAction()
  {
    ...
    try
    {
      ...
      if(!empty($file_id) && $file_id && $file_id != '' && $sesAdcCommentRow){
        $counter = 1;
      ...
      if(!empty($emoji_id) && $emoji_id && $sesAdcCommentRow){
      ...
      if(isset($_POST['image_id']) && $_POST['image_id']) {
        $sesAdcCommentRow->gif_id = 1;
        $sesAdcCommentRow->gif_url = $_POST['image_id'];
        $sesAdcCommentRow->save();
      ...
      foreach($result[2] as $value) {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($item, !empty($guid) ? $guid : $viewer, $viewer, 'sesadvancedcomment_tagged_people', array("commentLink" => $commentLink,'resource_type'=>$subject_type,'resource_id'=>$subject_id));
      }
      //Tagging People by status box
      // Stats
    ...
    try
    {
      ...
      foreach($result[2] as $value) {
       ...
       if(intval($user_id)>0){
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($item, !empty($guid) ? $guid : $viewer, $viewer, 'sesadvancedcomment_taggedreply_people', array("commentLink" => $commentLink,'resource_type'=>$action->getType(),'resource_id'=>$action->getIdentity()));
      }
      //Tagging People by status box
      $db->commit();
	 ...
	 public function editAction()
  {
   ...
   function removeLocationAction(){
    $location = $this->_getParam('sesbasic_location_data');
    $lat = $this->_getParam('sesbasic_location_lat');
    $lng = $this->_getParam('sesbasic_location_lng');
    $_SESSION["sesbasic_location_data"] = "";
    setcookie('sesbasic_location_data', $location, time() - (30 * 24*60*60*1000), "/");
    setcookie('sesbasic_location_lat', $lat, time() - (30 * 24*60*60*1000), "/");
    setcookie('sesbasic_location_lng', $lng, time() - (30 * 24*60*60*1000), "/");
  ...
  function setLocationAction(){
    ...
    $_SESSION["sesbasic_location_data"] = $location;
    $_SESSION["sesbasic_location_lat"] = $lat;
    $_SESSION["sesbasic_location_lng"] = $lng;

    setcookie('sesbasic_location_datatetete', $location, time() + (86400 * 365), "/");
    setcookie('sesbasic_location_data', $location, time() + (86400 * 365), "/");
    setcookie('sesbasic_location_lat', $lat, time() + (86400 * 365), "/");
    setcookie('sesbasic_location_lng', $lng, time() + (86400 * 365), "/");
