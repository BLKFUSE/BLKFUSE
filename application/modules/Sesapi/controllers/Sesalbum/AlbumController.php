...
class Sesalbum_AlbumController extends Sesapi_Controller_Action_Standard {
 ...
 public function rotateAction() {
  ...
  $angle = (int) $this->_getParam('direction', 90);
  if (!$angle || !($angle % 360)) {
    Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=> $this->view->translate('Invalid angle, must not be empty'), 'result' => array()));
  }
  if (!engine_in_array((int) $angle, array(90, 270))) {
    Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=> $this->view->translate('Invalid angle, must be 90 or 270'), 'result' => array()));
  }
  // Get file
  ...
  if (!($file instanceof Storage_Model_File)) {
    Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=> $this->view->translate('Could not retrieve file'), 'result' => array()));
  }
  ...
  Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'0','error_message'=> "", 'result' => array('url'=>$photo->getPhotoUrl())));
}
...
public function flipAction() {
  ...
  if (!engine_in_array($direction, array('vertical', 'horizontal'))) {
    Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=> $this->view->translate('Invalid direction'), 'result' => array()));
  }
  // Get file
  $file = Engine_Api::_()->getItem('storage_file', $photo->file_id);
  if (!($file instanceof Storage_Model_File)) {
  ...
  public function lightboxAction() {
    ...
    if($viewer->getIdentity() > 0){
      ...
      if($canEdit){
        ...
        $rotateCounter = 0;
        $photoRotates[$rotateCounter]['rotateAngle'] = "90";
        $rotateCounter++;
        $photoRotates[$rotateCounter]['rotateAngle'] = "270";
        $rotateCounter++;
        $photoRotates[$rotateCounter]['rotateAngle'] = "horizontal";
        $rotateCounter++;
        $photoRotates[$rotateCounter]['rotateAngle'] = "vertical";
        $rotateCounter++;
        $albumData['rotate'] = $photoRotates;
