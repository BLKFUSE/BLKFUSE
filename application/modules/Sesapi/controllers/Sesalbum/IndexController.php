  ...
  public function getAlbums($paginator,$return = false){
    ...
    foreach($paginator as $albums){
        ...
        if ($albums->category_id) {
          $category = Engine_Api::_()->getItem('sesalbum_category', $albums->category_id);
          if ($category) {
              $album['category_title'] = $category->category_name;
              if ($albums->subcat_id) {
                  $subcat = Engine_Api::_()->getItem('sesalbum_category', $albums->subcat_id);
                  if ($subcat) {
                      $album['subcategory_title'] = $subcat->category_name;
                      if ($albums->subsubcat_id) {
                          $subsubcat = Engine_Api::_()->getItem('sesalbum_category', $albums->subsubcat_id);
                          if ($subsubcat) {
                              $album['subsubcategory_title'] = $subsubcat->category_name;
                          }
                      }
                  }
              }
