 ...
 protected function isSesblogEnable(){
  ...
  function blogResult($paginator){
      ...
      foreach($paginator as $blogs){
        ...
        try{
          ...
          $blog["comment_count"] = !empty($blogs->comment_count) ? $blogs->comment_count : 0;
        }
  ...
  public function viewAction() {
    ...
    if($viewer->getIdentity() > 0){
      ...
      if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesblog.enable.report', 1) && $this->view->viewer()->getIdentity()){
    ...
    if(is_null($result['blog']["share"]["title"]))
      ...
      $owner = $sesblog->getOwner();
      if( $owner->getIdentity() == $viewer->getIdentity() || !Engine_Api::_()->getApi('settings', 'core')->getSetting('sesblog.subscription', 1)){}else{
  ...
  public function createAction() {     
    ...
    // Prepare form
    $this->view->form = $form = new Sesblog_Form_Index_Create(array('defaultProfileId' => $defaultProfileId,'fromApi'=>true));
    $form->removeElement('lat');
  ...
  public function editAction(){
    ...
    if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesblogcre.ecust.url', 0)) {
      // Custom Url
      $form->removeElement("custom_url_blog");
    ...
    if($this->_getParam('getForm')) {
      ...
      if($form->custom_url_blog)
      $form->custom_url_blog->setValue($sesblog->custom_url);
      
      $formFields = Engine_Api::_()->getApi('FormFields','sesapi')->generateFormFields($form);
      //set subcategory
      $newFormFieldsArray = array();
      
          if(is_countable($formFields) && engine_count($formFields) &&  $sesblog->category_id){
    ...
    if( !$form->isValid($_POST) ) { 
     ...
     //check custom url
     if (isset($_POST['custom_url']) && !empty($_POST['custom_url'])) {
      $custom_url = Engine_Api::_()->getDbtable('blogs', 'sesblog')->checkCustomUrl($_POST['custom_url'],$sesblog->getIdentity());
      if ($custom_url) {
				$form->addError($this->view->translate("Custom Url is not available. Please select another URL."));
        $validateFields = Engine_Api::_()->getApi('FormFields','sesapi')->validateFormFields($form);
        if(is_countable($validateFields) && engine_count($validateFields))
    ...
    { 
      ...
      $values['custom_url'] = $_POST['custom_url'];
	...
	Engine_Db_Table::getDefaultAdapter()->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $this->_getParam('blog_id') . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","sesblog_blog") ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      ...
      if( is_countable($action->toArray()) && engine_count($action->toArray()) <= 0 && $values['draft'] == '0' && (!$sesblog->publish_date || strtotime($sesblog->publish_date) <= time())) {
        $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($viewer, $sesblog, 'sesblog_new');
          // make sure action exists before attaching the sesblog to the activity
        if( $action != null ) {
          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $sesblog);
...
Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'0','error_message'=>'', 'result' => array('blog_id'=>$sesblog->getIdentity(),'message'=>$this->view->translate('Blog Edit successfully.'))));
  ...
  {
    $blog = Engine_Api::_()->getItem('sesblog_blog', $this->_getParam('blog_id'));
    if( !$this->_helper->requireUser()->isValid() ) {   
