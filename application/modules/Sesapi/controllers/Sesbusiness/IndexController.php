...
class Sesbusiness_IndexController extends Sesapi_Controller_Action_Standard
{
        ...
        if ($page == 1 && $_POST['filter_sort'] == 'sesbusiness_main_browse') {
            ...
            if (!isset($params['category_id'])) {
                ...
                if ($paginator->getTotalItemCount() > 0) {
                    ...
                    if (engine_count($popularBusinesses)) {
    ...
    public function likeAction()
    {
        ...
        if (!empty($result)) {
            ...
            if ($type == 'businesses') {
                ...
                if (engine_count($businessFollowers) > 0) {
    ...
    public function getbusiness($business)
    {
        ...
        if(Engine_Api::_()->authorization()->getPermission($levelId, 'businesses', 'auth_subbusiness') ) {
            ...
            $result['menus'][$tabcounter]['label'] = $this->view->translate('Associated businesses');
            $tabcounter++;
        }
		...
		if($viewer->getIdentity() > 0 && !$business->isOwner($viewer) && Engine_Api::_()->authorization()->getPermission($viewer, 'businesses', 'auth_claim')){
            ...
            $result['menus'][$tabcounter]['label'] = $this->view->translate('Claim businesses');
			 $tabcounter++;
		}
	...
	public function claimAction(){
	 ...
	 // check for claim already exist or not
		...
		if(engine_count($claimedbusinesses->toArray()) >0){
  ...
  $class = end(explode(' ', $link->class));
   ...
   if ($class != "sesbusiness_profile_addtoshortcut") {
            ...
            if ($class == 'sesbusiness_profile_dashboard') {
                ...
                $pluralurl = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness.businesses.manifest', 'business-directories');
                if ($pluralurl) {
  ...
  /*if ($canJoin) {
	...
	*/
	...
	public function moreMembersAction(){
	   ...
	   if(engine_count($Members) && $businesscheck) {
	 ...
	 if (engine_count($Members) && !$businesscheck && $this->_getParam('type',null) != 'like') {
    ...
    public function getInformation($businesses)
    {
        ...
        if (engine_count($likeBusinessResult)) {
            $likebusinessesCounter = 0;
            $result['total_business_liked_by_this_business'] = engine_count($likeBusinessResult) > 4 ? engine_count($likeBusinessResult) - 4 : 0;
...
//          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>'permission_error', 'result' => array()));
    ...
    try {
      if ($approve) {
        ...
        if($parent_id && $parent_type) {
	        ...
	        $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $video, 'sesbuvido_crte');
	        if ($action != null) {
	          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $video);
	        }
        }
   ...
   // }
    ...
    $dbTable = 'videos';
    $resorces_id = 'video_id';
    $notificationType = 'sesbuvido_fav';
	...
	function getButtonMenus($businesses){
        ...
        $showLoginformFalse = false;
         if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness.enable.contact.details', 1) && $viewer->getIdentity() == 0) {
            $showLoginformFalse = true;
        }
        $shareType = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness.allow.share', 0);
        $i = 0;
