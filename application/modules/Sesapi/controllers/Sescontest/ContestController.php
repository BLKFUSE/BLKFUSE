...
class Sescontest_ContestController extends Sesapi_Controller_Action_Standard {
    ...
    public function viewAction() {
       ...
       $feesEdit = Engine_Api::_()->getDbtable('dashboards', 'sescontest')->getDashboardsItems(array('type' => 'entry_fees_paid_sespaidext'));
       $juryEdit = Engine_Api::_()->getDbtable('dashboards', 'sescontest')->getDashboardsItems(array('type' => 'jury_member'));
    ...
    public function likeAction() {
        ...
        if (!empty($result)) {
            ...
            if ($type == 'contest') {
                ...
                if (engine_count($contestFollowers) > 0) {
      ...
      //Start Package Work
       if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sescontestpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sescontestpackage.enable.package', 0) && (_SESAPI_VERSION_ANDROID >= 2.5 || _SESAPI_VERSION_IOS >= 1.7)) {
         ...
         if (!$package && !$existingpackage) {
           ...
           if (engine_count($packageMemberLevel)) {
    ...
    public function contestPackage() {
        ...
        if (!engine_count($packageMemberLevel) || !Engine_Api::_()->getApi('settings', 'core')->getSetting('sescontestpackage.enable.package', 0))
            return true;
        ...
        if(engine_count($existingleftpackages)){
            ...
            foreach ($existingleftpackages as $leftpackages) {
                ...
                $enableModules = json_decode($package->params, true);
                    ...
                    $paramscounter ++;
                    if(engine_in_array('featured',$showinfo)){
                    ...
                    if(engine_in_array('sponsored',$showinfo)){
                    ...
                    if(engine_in_array('hot',$showinfo)){
                    ...
                    $result['existingleftpackages'][$counterleft]['params'][$paramscounter]['action'] = 'image';
                    $paramscounter ++;
                    if(engine_in_array('description',$showinfo)){
        ...
        $counter = 0;
        if(is_countable($packages) && engine_count($packages)){
    ...
    if($_GET['sesapi_platform'] == 1){
      ...
      $contest_start_date = date('Y-m-d', strtotime($contestStartDate));
      $contest_start_time = date('H:i:s', strtotime($contestStartDate));
      $contestEndDate = date('Y-m-d h:i:s', strtotime($previous_endtime));
      $contest_end_date = date('Y-m-d', strtotime($contestEndDate));
      $contest_end_time = date('H:i:s', strtotime($contestEndDate));
      // join time
      $joinStartDate = date('Y-m-d h:i:s', strtotime($previous_joinstarttime));
      $join_start_date = date('Y-m-d', strtotime($joinStartDate));
      $join_start_time = date('H:i:s', strtotime($joinStartDate));
      $joinEndDate = date('Y-m-d h:i:s', strtotime($previous_joinendtime));
      $join_end_date = date('Y-m-d', strtotime($joinEndDate));
      $join_end_time = date('H:i:s', strtotime($joinEndDate));
      // voting time
      $votingStartDate = date('Y-m-d h:i:s', strtotime($previous_votingstarttime));
      $voting_start_date = date('Y-m-d', strtotime($votingStartDate));
      $voting_start_time = date('H:i:s', strtotime($votingStartDate));
      $votingEndDate = date('Y-m-d h:i:s', strtotime($previous_votingendtime));
      $voting_end_date = date('Y-m-d', strtotime($votingEndDate));
      $voting_end_time = date('H:i:s', strtotime($votingEndDate));
      // result time
      $resultDate = date('Y-m-d h:i:s', strtotime($result_time));
      $result_start_date = date('Y-m-d', strtotime($resultDate));
      $result_start_time = date('H:i:s', strtotime($resultDate));
    }else{
      $contestStartDate = date('Y-m-d h:i:s', strtotime($previous_starttime));
   ...
   $contestId = $this->_getParam('contest_id',null);
    ...
    public function contestInfoAction() {
      ...
      $result['information'] = $this->getInformation($subject);
      Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array_merge(array('error' => '0', 'error_message' => '', 'result' => $result)));
    }
    
    
    public function contestAwardsAction() {
      ...
      if($subject->award) {
        $result['awardInformation'][$awardCounter]['name'] = 'award';
        $result['awardInformation'][$awardCounter]['value'] = $subject->award;
        $result['awardInformation'][$awardCounter]['label'] = '1st';
        $awardCounter++;
      }
      
      if($subject->award2) {
        $result['awardInformation'][$awardCounter]['name'] = 'award2';
        $result['awardInformation'][$awardCounter]['value'] = $subject->award2;
        $result['awardInformation'][$awardCounter]['label'] = '2nd';
        $awardCounter++;
      }
      if($subject->award3) {
        $result['awardInformation'][$awardCounter]['name'] = 'award3';
        $result['awardInformation'][$awardCounter]['value'] = $subject->award3;
        $result['awardInformation'][$awardCounter]['label'] = '3rd';
        $awardCounter++;
      }
      if($subject->award4) {
        $result['awardInformation'][$awardCounter]['name'] = 'award4';
        $result['awardInformation'][$awardCounter]['value'] = $subject->award4;
        $result['awardInformation'][$awardCounter]['label'] = '4th';
        $awardCounter++;
      }
      if($subject->award5) {
        $result['awardInformation'][$awardCounter]['name'] = 'award5';
        $result['awardInformation'][$awardCounter]['value'] = $subject->award5;
        $result['awardInformation'][$awardCounter]['label'] = '5th';
        $awardCounter++;
      }
    ...
    public function getInformation($contests) {
      ...
      $result['basicInformation'][$basicInformationCounter]['value'] = $contests->creation_date;
      ...
      $state[$statsCounter]['value'] = $contests->like_count;
      $state[$statsCounter]['label'] = 'Likes';
      $statsCounter++;
      
      $state[$statsCounter]['name'] = 'comment';
      $state[$statsCounter]['value'] = $contests->comment_count;
      $state[$statsCounter]['label'] = 'Comments';
      $statsCounter++;

      $state[$statsCounter]['name'] = 'view';
      $state[$statsCounter]['value'] = $contests->view_count;
      $state[$statsCounter]['label'] = 'Views';
      $statsCounter++;
      
      $state[$statsCounter]['name'] = 'favourite';
      $state[$statsCounter]['value'] = $contests->favourite_count;
      $state[$statsCounter]['label'] = 'Favourites';
      $statsCounter++;
      
      $state[$statsCounter]['name'] = 'follow';
      $state[$statsCounter]['value'] = $contests->follow_count;
      $state[$statsCounter]['label'] = 'Follows';
      $statsCounter++;
      
      $state[$statsCounter]['name'] = 'join';
      $state[$statsCounter]['value'] = $contests->join_count;
      $state[$statsCounter]['label'] = 'Entries';
      $statsCounter++;
      ...
      $fieldStructure = Engine_Api::_()->fields()->getFieldsStructurePartial($contests);
      if (engine_count($fieldStructure)) { // @todo figure out right logic
          $content = $this->view->fieldSesapiValueLoop($contests, $fieldStructure);;
          $counter = 0;
      ...
      $result['Detail'] = $contests->description;
      if($contests->overview) {
        $result['Overview'] = $contests->overview;
      }
      return $result;
    }
    ...
    public function contestRulesAction() {
      ...
      $subject = Engine_Api::_()->core()->getSubject();
      $edit = $subject->authorization()->isAllowed($viewer, 'edit');

      if (!$edit && (!$subject->rules || is_null($subject->rules))) {
          Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('There are no results that match your search. Please try again.'), 'result' => array()));
      }

      if ($edit) {
        if ($subject->rules) {
            $result['button'][0]['name'] = "editrule";
            $result['button'][0]['lable'] = $this->view->translate("Change Rules");
        } else {
            $result['button'][0]['name'] = "editoverview";
            $result['button'][0]['lable'] = $this->view->translate("Add Rules");
        }
      }
      if ($subject->rules) {
          $result['rules'] = $subject->rules;
      } else {
          $result['rules'] = $this->view->translate("There is currently no rules.");
      }
    ...
    public function contactParticipantsAction() {
	 ...
	 $contestId = $this->_getParam('contest_id',null);
	   ...
	   $result = array();
	   if(engine_count($participants) > 0){
		   $counter = 0;
