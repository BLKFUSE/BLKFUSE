        ...
        foreach ($paginator as $contests) {
            ...
            if($sesshortcut){
                ...
                if (empty($isShortcut)) {
                    ...
                    $shortMessage['resource_id'] = $contests->getIdentity();
...
//            $image = Engine_Api::_()->storage()->get($category->colored_icon)->getPhotoUrl();
    ...
    public function browsecategoriesAction() {
        ...
        $extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
        $extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
        if (engine_count($result['category']) <= 0)
        ...
        foreach ($paginator as $entries) {
            ...
            if($sesshortcut){
                ...
                if (empty($isShortcut)) {
                    ...
                    $shortMessage['resource_id'] = $entries->getIdentity();
                    $shortMessage['is_saved'] = false;
                  } else {
                    $shortMessage['title'] = $this->view->translate('Remove From Shortcuts');
                    $shortMessage['resource_type'] = $entries->getType();
