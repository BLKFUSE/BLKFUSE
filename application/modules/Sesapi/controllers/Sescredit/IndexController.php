...
class Sescredit_IndexController extends Sesapi_Controller_Action_Standard {
	...
	public function getTransaction($paginator){
      ...
      elseif($transaction->point_type == 'reward'):
        $result[$counter]['language'] = $this->view->translate("Reward");
      else:
      $result[$counter]['language'] = $transaction->language;
      endif;
      
      if(engine_in_array($transaction->point_type, array('credit', 'affiliate', 'receive_friend', 'purchase','reward'))) {
        $result[$counter]['point_type'] = $this->view->translate("credit");;
      } else if(engine_in_array($transaction->point_type, array('deduction', 'transfer_friend', 'sesproduct_order', 'upgrade_level')))  {
        $result[$counter]['point_type'] = $this->view->translate("deduction");;
      }
			
			if( 1 !== engine_count($languageNameList))
