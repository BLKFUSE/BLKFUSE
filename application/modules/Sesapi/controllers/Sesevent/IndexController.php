...
class Sesevent_IndexController extends Sesapi_Controller_Action_Standard
{
    ...
    public function getevents($paginator, $editdelete){
        ...
        foreach ($paginator as $events) {
            ...
            if(!engine_count($result[$counter]['images']))
    ...
    public function browsesearchAction(){
        ...
        // Populate options
        if (isset($formFilter->category_id) && engine_count($formFilter->category_id->getMultiOptions()) <= 1)
    ...
    public function browselistAction(){
        ...
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $result['lists'] = $this->getlists($paginator,false);
        $extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
        $extraParams['pagging']['total'] = $paginator->getTotalItemCount();
        $extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
        $extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
        ...
        foreach ($paginator as $items) {
            ...
            foreach ($events as $eventItems) {
                ...
                if (!empty($event)) {
                    ...
                    if (engine_count($events) > 3) {
                        $moreNumber = engine_count($events) - 3;
    ...
    public function getProfile($event){
        ...
        $defaultParams['endtime'] = true;
        $defaultParams['timezone'] = true;
        $defaultParams['eventview'] = true;
        $strtime = $this->view->eventStartEndDates($event, $defaultParams);
        list($result['calanderStartTime'], $result['calanderEndTime']) = explode('ENDDATE', strip_tags($strtime));
        ...
        //if ($row) {
            ...
            $result['RSVP'][$counterrsvp]['value'] = $rsvp == 10;
            $counterrsvp++;
        //}
    ...
    public function createAction(){
        ...
        $paginator = engine_count(Engine_Api::_()->getDbTable('events', 'sesevent')->fetchAll($select));
    ...
    public function viewhostAction(){
        ...
        $followCount = engine_count($results);
...
//         }
    ...
    public function addtolistAction(){
		...
		if (!($quota > engine_count($listCount) || $quota == 0))
