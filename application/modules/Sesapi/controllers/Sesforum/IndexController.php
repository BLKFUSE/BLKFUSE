...
//     }
    ...
    foreach( $paginator as $i => $post ) {
      ...
      $signature_body = $this->view->BBCode($signature_body, array('link_no_preparse' => true));
      $signature_body = $this->replaceSrc($signature_body);
      $signature_body = "<link href=\"".$this->getBaseUrl(true,'application/modules/Sesapi/externals/styles/tinymce.css')."\" type=\"text/css\" rel=\"stylesheet\">".($signature_body);
  
      $signature_body = $signature_body."<link href='".$this->getBaseUrl(true, 'application/modules/Sesapi/externals/styles/style_forum.css')."' media='all' rel='stylesheet' type='text/css' />";
      ...
      if($signature_body) {
        $post_content['signature'] = Engine_Text_Emoji::decode($signature_body);
      }
      ...
      //if( !$this->decode_html && $this->decode_bbcode ) {
        $post_content['body'] = $this->view->BBCode( $post_content['body'], array('link_no_preparse' => true));
      //}
      ...
      $post_content['body'] = Engine_Text_Emoji::decode($post_content['body'])."<link href='".$this->getBaseUrl(true, 'application/modules/Sesapi/externals/styles/style_forum.css')."' media='all' rel='stylesheet' type='text/css' />";
...
/*
  ...
  public function indexAction() {
    ...
    foreach($categories as $category) {
      ...
      $result['categories'][$counter]['category_name'] = $this->view->translate($category->title);
      ...
      $result['categories'][$counter]['topic_count'] = $this->view->translate(array('%s Topic', '%s Topics', $category->getTopics()), $category->getTopics());
      $result['categories'][$counter]['post_count'] = $this->view->translate(array('%s Post ', '%s Posts ', $category->getPosts()), $category->getPosts());      
      ...
      foreach($subCategories as $subCategorie) {  
        ...
        $subcat[$subCatCounter]['category_name'] = $this->view->translate($subCategorie->title);
  ...
  public function categoryviewAction() {
    ...
    foreach($subCategories as $subCategorie) {  
      ...
      $subcat['subcat'][$subCatCounter]['category_name'] = $this->view->translate($subCategorie->title);
      ...
      foreach($subsubCategories as $subsubCategorie) {  
        ...
        $subsubcat[$subsubCatCounter]['category_name'] = $this->view->translate($subCategorie->title);
    ...
    if(engine_count($forumIds) > 0) {
      ...
      foreach( $topicPaginator as $i => $topic ) {
        ...
        $lastPostCount = 0;
        if( $lastPoster && !empty($lastPoster->user_id)) {
    ...
    $breadcrumb['breadcrumb'][$breadcrumbCounter]['category_name'] = $this->view->translate($category->title);
  ...
  public function subcategoryviewAction() {
    ...
    foreach($subsubCategories as $subsubCategorie) {  
      ...
      $subsubcat[$subsubCatCounter]['category_name'] = $this->view->translate($subsubCategorie->title);
      $subsubcat[$subsubCatCounter]['category_id'] = $subsubCategorie->getIdentity();
      $subsubcat[$subsubCatCounter]['type'] = 'subsubcat';
      if($subsubCategorie->cat_icon != '' && !is_null($subsubCategorie->cat_icon) && intval($subsubCategorie->cat_icon)) {
        $cat_icon = Engine_Api::_()->storage()->get($subsubCategorie->cat_icon, '');
    ...
    if(engine_count($forumIds) > 0) {
      ...
      foreach( $topicPaginator as $i => $topic ) {
        ...
        $lastPoster = Engine_Api::_()->getItem('user', $topic->lastposter_id);
        $lastPostCount = 0;
        if( $last_post && !empty($lastPoster->user_id) ) {
          $topics['topics'][$topicCounter]['last_post'][$lastPostCount]['user_images'] = $this->userImage($lastPoster->user_id,"thumb.icon");
          $topics['topics'][$topicCounter]['last_post'][$lastPostCount]['user_id'] = $lastPoster->user_id;
          $topics['topics'][$topicCounter]['last_post'][$lastPostCount]['user_title'] = $lastPoster->getTitle();
          $topics['topics'][$topicCounter]['last_post'][$lastPostCount]['creation_date'] = $topic->modified_date;
    ...
    if(!empty($category->subcat_id)) {
      ...
      $breadcrumb['breadcrumb'][$breadcrumbCounter]['category_name'] = $this->view->translate($maincategory->title);
