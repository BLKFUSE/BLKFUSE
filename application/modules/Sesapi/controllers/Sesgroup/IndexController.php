...
class Sesgroup_IndexController extends Sesapi_Controller_Action_Standard
{
        ...
        if ($paginator) {
            ...
            foreach ($paginator as $groups) {
                ...
                if ($groups->category_id) {
                    ...
                    if ($category) {
                        $result[$groupCounter]['category_title'] = $category->category_name;
                        if ($groups->subcat_id) {
                            $subcat = Engine_Api::_()->getItem('sesgroup_category', $groups->subcat_id);
                            if ($subcat) {
                                $result[$groupCounter]['subcategory_title'] = $subcat->category_name;
                                if ($groups->subsubcat_id) {
                                    $subsubcat = Engine_Api::_()->getItem('sesgroup_category', $groups->subsubcat_id);
                                    if ($subsubcat) {
                                        $result[$groupCounter]['subsubcategory_title'] = $subsubcat->category_name;
                                    }
                                }
                            }
                        }
    ...
    function getFriendsData($params){
        ...
        $select = $table->select()
                ...
                ->where('`' . $subtableName . '`.`resource_id` = ?', $params['group_id'])
                ->where('resource_approved =?', 1)
                ->where('user_approved =?', 1)
                ->where($tableName.'.user_id IN (?)' , $params['friends'])
    ...
    public function getGroups($paginator){
        ...
        $canShare = Engine_Api::_()->getApi('settings', 'core')->getSetting('mobile_sesgroup_share', 1);
        ...
        foreach ($paginator as $groups) {
            ...
            if($sesshortcut){
                ...
                if (empty($isShortcut)) {
                    ...
                    $shortMessage['resource_id'] = $groups->getIdentity();
                  ...
                  $result[$counter]['shortcut_save'] = $shortMessage;
            }
            $result[$counter]['can_share'] = $canShare;
            if($viewer->getIdentity() && engine_count($friendsIds)){
                $friends = $this->getFriendsData(array("group_id"=>$groups->getIdentity(),'friends'=>$friendsIds));
                if(engine_count($friends)){
                    $result[$counter]['friends']['counts'] = engine_count($friends);
                    $result[$counter]['friends']['text'] = $this->view->FluentListUsers($friends,false,false,'','');
                    $usersFriends = array();
                    $cnt = 0;
                    ...
                    $result[$counter]['friends']['users'] = $usersFriends;
                }
            }
   ...
   public function getContentFollowCount($subject,$tableName = "",$modulename = "",$resources_type = "",$column_name = "resource_id"){
    ...
    public function likeAction()
    {
        ...
        if (!empty($result)) {
            ...
            if ($type == 'sesgroup_group') {
                ...
                if (engine_count($groupFollowers) > 0) {
    ...
    public function followAction()
    {
        ...
        if (!empty($Fav)) {
            ...
            Engine_Api::_()->sesapi()->deleteFeed(array('type' => 'sesgroup_group_follow', "subject_id" => $viewer->getIdentity(), "object_type" => $item->getType(), "object_id" => $item->getIdentity()));
    ...
    public function getgroup($group)
    {
        ...
        $joinedMembers = Engine_Api::_()->sesgroup()->getallJoinedMembers($item);
        $memberCount = engine_count($joinedMembers);
        ...
        $showLoginformFalse = false;
        if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.contact.details', 1) && $viewer_id == 0) {
            $showLoginformFalse = true;
        }
        $l = 0;
        ...
        if (Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesgroupforum')) {
          ...
          $canview =  Engine_Api::_()->sesgroupforum()->isAllowed('sesgroupforum', $levelId, 'view')->value;
          if(!empty($canview)) {
            $result['menus'][$tabcounter]['name'] = 'forum';
            $result['menus'][$tabcounter]['label'] = $this->view->translate('Topics');
            $tabcounter++;
          }
        }
        if(Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'auth_subgroup') || Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'view')) {
            ...
            $result['menus'][$tabcounter]['label'] = $this->view->translate('Associated Groups');
            $tabcounter++;
        }
		...
		if($viewer->getIdentity() > 0 && !$group->isOwner($viewer) && Engine_Api::_()->authorization()->getPermission($viewer, 'sesgroup_group', 'auth_claim')){
			...
			$result['menus'][$tabcounter]['label'] = $this->view->translate('Claim Group');
			$tabcounter++;
		}
	...
	public function claimAction(){
		...
		if(engine_count($claimedGroups->toArray()) >0){
...
function getNavigation($group,$viewer){
    ...
    foreach ($navigation as $link) {
        ...
        if ($class != "sesgroup_profile_addtoshortcut") {
            ...
            if ($class == 'sesgroup_profile_dashboard') {
                ...
                $pluralurl = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.groups.manifest', 'group-communities');
                if ($pluralurl) {
  ...
  $result[$navigationCounter]['url'] =  $this->getBaseUrl(false,$group->getHref());
  $navigationCounter++;
  ...
  /*if ($canJoin) {
	...
	*/
    ...
    public function photo($groupid)
    {
        ...
        foreach ($paginator as $photos) {
            ...
            if (!engine_count($images)) {
                $images['main'] = $this->getBaseUrl(true, $photos->getPhotoUrl()) . 'application/modules/Sesgroup/externals/images/nophoto_group_thumb_profile.png';
                $images['normal'] = $this->getBaseUrl(true, $photos->getPhotoUrl()) . 'application/modules/Sesgroup/externals/images/nophoto_group_thumb_profile.png';
	...
	public function moreMembersAction(){
	   ...
	   if(engine_count($Members) && $groupcheck) {
			$likeGroupsCounter = 0;
	 ...
	 if (engine_count($Members) && !$groupcheck && $this->_getParam('type',null) != 'like') {
  ...
  public function userAge($member){
    ...
    public function getInformation($groups)
    {
        ...
        if (engine_count($likeGroupResult)) {
            $likeGroupsCounter = 0;
            $result['total_group_liked_by_this_group'] = engine_count($likeGroupResult) > 4 ? engine_count($likeGroupResult) - 4 : 0;
        ...
        if (engine_count($likeMembers)) {
            $likeCounter = 0;
            $result['total_people_who_liked'] = engine_count($likeMembers) > 4 ? engine_count($likeMembers) - 4 : 0;
        ...
        if (engine_count($favMembers) && $canFavourite) {
            $favCounter = 0;
            $result['total_people_who_favourited'] = engine_count($favMembers) > 4 ? engine_count($favMembers) - 4 : 0;
        ...
        if (engine_count($followMembers) && $canFollow) {
            $followCounter = 0;
            $result['total_people_who_follow_this'] = engine_count($followMembers) > 4 ? engine_count($followMembers) - 4 : 0;
            foreach ($followMembers as $member) {
                if($followCounter > 3)
                    break;
                $item = Engine_Api::_()->getItem('user', $member['owner_id']);

                if(engine_count($item->toArray() == 0)){
...
//          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>'permission_error', 'result' => array()));
    ...
    try {
      if ($approve) {
        ...
        if($parent_id && $parent_type) {
		      $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $parentItem, 'sesgroupvideo_new');
	        if ($action != null) {
	          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $video);
	        }
        } else {
	        $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $video, 'sesgroupvideo_new');
	        if ($action != null) {
	          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $video);
	        }
        }
  ...
  //item liked as per item tye given
    ...
    $dbTable = 'videos';
    $resorces_id = 'video_id';
    $notificationType = 'sesgpvido_fav';
  ...
  public function getUserAction() {
    ...
    Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => $sesdata));
  }
  ...
  public function discussionsAction() {
    ...
    // Get subject and check auth
    $subject = $sesgroup = Engine_Api::_()->core()->getSubject('sesgroup_group');
    if (!$subject->authorization()->isAllowed($viewer, 'view')) {
      Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => 'user_not_autheticate', 'result' => array()));
    }
    
    $sesgroupforum = Engine_Api::_()->getItem('sesgroupforum', 1);
    $params['forum_id'] = 1;
