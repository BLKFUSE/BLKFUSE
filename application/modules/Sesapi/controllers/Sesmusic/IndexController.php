  ...
  function getAlbums($paginator,$manage = ""){
      ...
      foreach($paginator as $albums){
        ...
        if($manage){
            ...
            $counterMenu++;
            if($albums->favourite_count == 0){
              $menuoptions[$counterMenu]['name'] = "add to favourite";
              $menuoptions[$counterMenu]['label'] = $this->view->translate("Add to Favourite");
              $counterMenu++;
            }
            else{
              $menuoptions[$counterMenu]['name'] = "Remove to favourite";
              $menuoptions[$counterMenu]['label'] = $this->view->translate("Remove to Favourite"); 
              $counterMenu++;
            }
            $album['menus'] = $menuoptions;
        }
        if($viewer->getIdentity() != 0){
          $album['is_content_like'] = Engine_Api::_()->sesapi()->contentLike($albums,'like','sesmusic','sesmusic_albums');
...
('registered', 0, 'user', $viewer_id, 'sesmusic_artist', $resource_id, 'sesmusic_artist_rating', $action->action_id);");
    ...
    if (!empty($_FILES['image']['size']))
      ...
      if(!empty($_FILES['musicupload'])){
        ...
        foreach($_FILES['musicupload']['name'] as $key => $upload){
            ...
            $file_ids[] = $fileUpload->getIdentity();
  ...
  public function createAction(){
    ...
    //}
		if(isset($track) && engine_count($track)){
