...
class Sesmusic_SongController extends Sesapi_Controller_Action_Standard
{
  ...
  public function playlistViewAction(){
    ...
    $sesshortcut = Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesshortcut') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesshortcut.enableshortcut', 1);
      ...
      if($sesshortcut){
          ...
          if (empty($isShortcut)) {
              ...
              $shortMessage['resource_id'] = $playlist->getIdentity();
            ...
            $response['playlist']['shortcut_save'] = $shortMessage;
      }
    ...
    if($this->_getParam('page', 1) == 1){
      //related songs
      $paginator = Engine_Api::_()->getDbTable('albumsongs', 'sesmusic')->widgetResults(array("popularity"=>"view_count","paginator"=>true));
      $paginator->setItemCountPerPage(10);
      $paginator->setCurrentPageNumber(1);
      $songs = $this->getAlbums($paginator);
      $result["related_songs"] = $songs["songs"];
    }
  ...
  function getAlbums($paginator,$getItem = false){
      ...
      $sesshortcut = Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesshortcut') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesshortcut.enableshortcut', 1);
      $canShare = Engine_Api::_()->getApi('settings', 'core')->getSetting('mobile_sesmusic_share', 1);
      ...
      foreach($paginator as $albums){
        ...
        if($parentAlbum)
          ...
          if($sesshortcut){
              ...
              if (empty($isShortcut)) {
                  ...
                  $shortMessage['resource_id'] = $albums->getIdentity();
                  $shortMessage['is_saved'] = false;
                } else {
                  $shortMessage['title'] = $this->view->translate('Remove From Shortcuts');
                  $shortMessage['resource_type'] = $albums->getType();
                ...
                $album['shortcut_save'] = $shortMessage;
          }
          $album['can_share'] = $canShare;
  
          if ($canShare) {
            $album["share"]["imageUrl"] = $this->getBaseUrl(false, $albums->getPhotoUrl());
        ...
        if ($albums->artists) {
          ...
          $artistArray = array();
          if ($artists){
            $artists_array = Engine_Api::_()->getDbTable('artists', 'sesmusic')->getArtists(array_merge(array('all'=>' ')),$artists);
  ...
  public function viewAction(){
    ...
    if($viewer->getIdentity()){
      ...
      if($canEdit){
        $menuoptions[$counterMenu]['name'] = "edit";
        $menuoptions[$counterMenu]['label'] = $this->view->translate("Edit Song");
        $counterMenu++;
      }
      $canDelete = $this->_helper->requireAuth()->setAuthParams($song, null, 'delete')->isValid();
      if($canDelete){
        $menuoptions[$counterMenu]['name'] = "delete";
        $menuoptions[$counterMenu]['label'] = $this->view->translate("Delete Song");
        $counterMenu++;
      }
      if(!$song->isOwner($viewer)){
        $menuoptions[$counterMenu]['name'] = "report";
        $menuoptions[$counterMenu]['label'] = $this->view->translate("Report");
        $counterMenu++;
      }
      $downloadPublic = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmusic.download.publicuser', 0);
      if($downloadPublic || $this->view->viewer()->getIdentity()){
        $menuoptions[$counterMenu]['name'] = "download";
        $menuoptions[$counterMenu]['label'] = $this->view->translate("Download");
        $counterMenu++;
      }
      $menuoptions[$counterMenu]['name'] = "add to playlist";
      $menuoptions[$counterMenu]['label'] = $this->view->translate("Add to playlist");
      $counterMenu++;
    ...
    if(is_null($response["share"]["title"]))
      ...
      if($sesshortcut){
          ...
          if (empty($isShortcut)) {
              ...
              $shortMessage['resource_id'] = $song->getIdentity();
              $shortMessage['is_saved'] = false;
            } else {
              $shortMessage['title'] = $this->view->translate('Remove From Shortcuts');
              $shortMessage['resource_type'] = $song->getType();
            ...
            $response['shortcut_save'] = $shortMessage;
      }
    ...
    $paginator = Engine_Api::_()->getDbTable('albumsongs', 'sesmusic')->widgetResults(array("popularity"=>"view_count","paginator"=>true,'category_id'=>$song->category_id));
    $paginator->setItemCountPerPage(10);
    $paginator->setCurrentPageNumber(1);
    $songs = $this->getAlbums($paginator);
    $result["related_songs"] = $songs["songs"];
   ...
   public function albumViewAction(){
      ...
      if(!$this->_getParam('getAlbum')){
        ...
        if($viewer->getIdentity()){
         ...
         if($canEdit){
            $menuoptions[$counterMenu]['name'] = "edit";
            $menuoptions[$counterMenu]['label'] = $this->view->translate("Edit Music Album");
            $counterMenu++;
          }
          $canDelete = $this->_helper->requireAuth()->setAuthParams($albums, null, 'delete')->isValid();
          if($canDelete){
            $menuoptions[$counterMenu]['name'] = "delete";
            $menuoptions[$counterMenu]['label'] = $this->view->translate("Delete Music Album");
            $counterMenu++;
          }
          if(!$albums->isOwner($viewer)){
            $menuoptions[$counterMenu]['name'] = "report";
            $menuoptions[$counterMenu]['label'] = $this->view->translate("Report");
            $counterMenu++;
          }
          $menuoptions[$counterMenu]['name'] = "add to playlist";
          $menuoptions[$counterMenu]['label'] = $this->view->translate("Add to all song in to my playlist");
          $counterMenu++;
       ...
       if(isset($canEdit) && $canEdit){
        ...
        if($sesshortcut){
            ...
            if (empty($isShortcut)) {
              ...
              $result['albums']['shortcut_save'] = $shortMessage;
        }
...
('registered', 0, 'user', $viewer_id, 'sesmusic_playlist', $playlistID, 'sesmusic_addplaylist', $action->action_id);");
  ...
  public function editAction() {
    ...
    if ($albumsong->artists) {
      ...
      if (engine_count($artists_array) > 0)
   ...
   if($this->_getParam('getForm')) {
      ...
      if(is_countable($formFields) && engine_count($formFields) &&  $albumsong->category_id){
