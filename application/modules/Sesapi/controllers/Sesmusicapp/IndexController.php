  ...
  public function homeAction() {
		...
		foreach($paginator  as $widgetdata){
			...
			if($widgetdata->name == 'sesmusicapp.category'){
				...
				if(is_countable($data) && engine_count($data)){
					if($widgetCount < 3){
    ...
    if ($contentType == 'album') {
      ...
      if (empty($album))
        return array();
			...
			$canLike = Engine_Api::_()->authorization()->isAllowed('sesmusic_album', $viewer, 'comment');
				...
				if($canAddFavourite && !empty($viewer_id) && $information && engine_in_array('favourite', $information))
					$result['items']["is_content_favourite"] = $isFavourite ? true : false;
				if($canLike && $information && engine_in_array('addLikeButton', $information))
				...
				if ($showRating && !empty($information) && engine_in_array('ratingCount', $information))
					$result['items']["show_rating"] = true;
				
				if($canAddPlaylist && !empty($information) && engine_in_array('addplaylist', $information))
					$result['items']["canadd_playlist"] = true;
				
				if($albumlink && engine_in_array('share', $albumlink) && !empty($information) && engine_in_array('share', $information)){
      ...
      if (empty($song))
        return array();
			...
			$isLike = Engine_Api::_()->getDbTable('likes', 'core')->isLike($value, $viewer);
			if($addfavouriteAlbumSong && !empty($viewer_id) && $information && engine_in_array('favourite', $information) )
			...
			if ($showAlbumSongRating && !empty($information) && engine_in_array('ratingCount', $information))
				$result['items'][$counter]["show_rating"] = true;
		
			if($canAddPlaylistAlbumSong && !empty($information) && engine_in_array('addplaylist', $information))
			...
			if($songlink && engine_in_array('share', $songlink) && !empty($information) && engine_in_array('share', $information)){
      ...
      if (empty($artist))
        return array();
			...
			$result['settings']['canadd_fav'] = $isFavourite;
				...
				if($artistlink && engine_in_array('favourite', $artistlink)){
					$result['items'][$counter]['is_content_favourite'] = $isFavourite ? true :  false;
				}
				...
				if($canLike && $information && engine_in_array('addLikeButton', $information))
				$result['items'][$counter]["is_content_like"] = $isLike ? true : false;
				if ($showArtistRating && !empty($information) && engine_in_array('ratingCount', $information)){
        ...
        $result['items'][$counter]['images'] = $this->getBaseUrl(false,Engine_Api::_()->sesapi()->getPhotoUrls($value->artist_photo, '', ""));
				if($artistlink && engine_in_array('share', $artistlink) && !empty($information) && engine_in_array('share', $information)){
					$result['items'][$counter]["can_share"] = true;
					$result['items'][$counter]["share"]["imageUrl"] = $this->getBaseUrl(false, $value->getPhotoUrl());
					$result['items'][$counter]["share"]["url"] = $this->getBaseUrl(false,$value->getHref());
					$result['items'][$counter]["share"]["title"] = $value->getTitle();
      ...
      $showAlbumSongRating = $showRating;
			...
			// playlist
			$result['items'][$counter] = $value->toArray();
			if(!empty($information) && engine_in_array('postedby', $information))
