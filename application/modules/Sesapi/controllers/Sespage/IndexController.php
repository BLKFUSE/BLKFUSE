...
class Sespage_IndexController extends Sesapi_Controller_Action_Standard
{
            ...
            foreach ($paginator as $pages) {
                ...
                if($category_id){
                    ...
                    if($category){
                        ...
                        $result[$pageCounter]['category_images'] = Engine_Api::_()->sesapi()->getPhotoUrls($category->thumbnail, '', "");
                        if ($category->cat_icon)
                            $result[$pageCounter]['category_icon'] = Engine_Api::_()->sesapi()->getPhotoUrls($category->cat_icon, '', "");
                        if ($category->colored_icon)
                            $result[$pageCounter]['category_icon_colored'] = Engine_Api::_()->sesapi()->getPhotoUrls($category->colored_icon, '', "");
                        $result[$pageCounter]['category_slug'] = $category->slug;
                        $result[$pageCounter]['category_name'] = $category->category_name;
                    }
                }
                if ($pages->category_id) {
                    $category = Engine_Api::_()->getItem('sespage_category', $pages->category_id);
                    if ($category) {
                        $result[$pageCounter]['category_title'] = $category->category_name;
                        if ($pages->subcat_id) {
                            $subcat = Engine_Api::_()->getItem('sespage_category', $pages->subcat_id);
                            if ($subcat) {
                                $result[$pageCounter]['subcategory_title'] = $subcat->category_name;
                                if ($pages->subsubcat_id) {
                                    $subsubcat = Engine_Api::_()->getItem('sespage_category', $pages->subsubcat_id);
                                    if ($subsubcat) {
                                        $result[$pageCounter]['subsubcategory_title'] = $subsubcat->category_name;
                                    }
                                }
                            }
                        }
    ...
    public function browseAction(){
        ...
        if ($page == 1 && !isset($params['search'])) {
            ...
            if (!isset($params['category_id'])) {
                ...
                if (engine_count($this->getPopularPages($paginator))) {
    ...
    public function getPages($paginator){
        ...
        $canShare = Engine_Api::_()->getApi('settings', 'core')->getSetting('mobile_sespage_share', 1);
        foreach ($paginator as $pages) {
            $page = $pages->toArray();
            $result[$counter] = $page;
            ...
            if($sesshortcut){
                ...
                if (empty($isShortcut)) {
                    ...
                    $shortMessage['resource_id'] = $pages->getIdentity();
            ...
            $result[$counter]['user_image']['main']= $this->getBaseUrl(true, $owner->getPhotoUrl());
   ...
   public function getContentFollowCount($subject,$tableName = "",$modulename = "",$resources_type = "",$column_name = "resource_id"){
    ...
    public function likeAction()
    {
        ...
        if (!empty($result)) {
            ...
            if ($type == 'sespage_page') {
                ...
                if (engine_count($pageFollowers) > 0) {
	...
	public function claimAction(){    
	 ...
	 // check for claim already exist or not
		...
		if(engine_count($claimedPages->toArray()) >0){
  ...
  $class = end(explode(' ', $link->class));
   ...
   if ($class != "sespage_profile_addtoshortcut") {
            ...
            if ($class == 'sespage_profile_dashboard') {
                ...
                $pluralurl = Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.pages.manifest', 'page-directories');
                if ($pluralurl) {
  ...
  /*if ($canJoin) {
	...
	*/
	...
	public function moreMembersAction(){
	   ...
	   if(engine_count($Members) && $pagecheck) {
			$likePagesCounter = 0;
	 ...
	 if (engine_count($Members) && !$pagecheck && $this->_getParam('type',null) != 'like') {
  ...
  public function userAge($member){
    ...
    public function getInformation($pages)
    {
        ...
        if ($pages->page_contact_website){
            ...
            $result['contactInformation'][$contactInformationCounter]['label'] = $pages->page_contact_website;
        ...
        if ($pages->page_contact_facebook){
            ...
            $result['contactInformation'][$contactInformationCounter]['label'] = $pages->page_contact_facebook;        
        ...
        if ($pages->page_contact_linkedin){
            ...
            $result['contactInformation'][$contactInformationCounter]['label'] = $pages->page_contact_linkedin;
        ...
        if ($pages->page_contact_twitter){
            ...
            $result['contactInformation'][$contactInformationCounter]['label'] = $pages->page_contact_twitter;
        ...
        if ($pages->page_contact_instagram){
            ...
            $result['contactInformation'][$contactInformationCounter]['label'] = $pages->page_contact_instagram;
        ...
        if ($pages->page_contact_pinterest){
            ...
            $result['contactInformation'][$contactInformationCounter]['label'] = $pages->page_contact_pinterest;
        ...
        if (is_countable($likePageResult) && engine_count($likePageResult)) {
            $likePagesCounter = 0;
            $result['total_page_liked_by_this_page'] = engine_count($likePageResult) > 4 ? engine_count($likePageResult) - 4 : 0;
...
//         $member->userImage($member->getIdentity(),'thumb.profile');
    ...
    public function servicesAction()
    {
        ...
        $servicesCounter = 0;
        $currency = Engine_Api::_()->sespage()->getCurrencySymbol(Engine_Api::_()->sespage()->getCurrentCurrency());
        ...
        foreach ($paginator as $item) {
            ...
            if ($item->price) {
                ...
                $result['services'][$servicesCounter]['service_price'] = $currency.$item->price;
            }
            if ($item->description) {
...
//    $this->_helper->layout->setLayout('default-simple');
    ...
    public function addmorephotosAction()
    {
        ...
        $uploadSource = engine_count($_FILES['file_type_attachmentImage']) ? $_FILES['file_type_attachmentImage'] :  $_FILES['attachmentImage'];
...
//    die;
  ...
  public function editDescriptionAction() {
    ...
    public function deletephotoAction() {
        ...
        Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('success_message' => $this->view->translate('You have successfully deleted this photo.'),'status'=>1)));
    }
  ...
  function likeVideoAction() {
   ...
   function favouriteVideoAction() {
	...
	function getButtonMenus($pages){
        ...
        $showLoginformFalse = false;
         if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.enable.contact.details', 1) && $viewer->getIdentity() == 0) {
            $showLoginformFalse = true;
        }
        $shareType = Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.allow.share', 0);
        $i = 0;
