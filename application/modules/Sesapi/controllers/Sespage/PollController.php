...
class Sespage_PollController extends Sesapi_Controller_Action_Standard
{
        ...
        foreach ($paginator as $poll){
            ...
            if($sesshortcut){
                ...
                if (empty($isShortcut)) {
                    ...
                    $shortMessage['resource_id'] = $poll->getIdentity();
            ...
            $result[$counter]["share"]["imageUrl"] = $this->getBaseUrl(false, $poll->getPhotoUrl());
            $result[$counter]["share"]["url"] = $this->getBaseUrl(false,$poll->getHref());
            $result[$counter]["share"]["title"] = $poll->getTitle();
            $result[$counter]["share"]["description"] = strip_tags($poll->getDescription());
    ...
    public function createAction(){
        ...
        if($module){
            $gifCount = engine_count(Engine_Api::_()->getDbTable('images', 'sesfeedgif')->getImages(array('fetchAll' => 1, 'limit' => 10)));
        }
        $isGifModuleEnable=$gifCount > 0 ? true : false;
        if($form->getElement("errror_dummy"))
            $form->removeElement('errror_dummy');
        if ($this->_getParam('getForm')) {
            $formFields = Engine_Api::_()->getApi('FormFields', 'sesapi')->generateFormFields($form);
