...
class Sespage_ProfileController extends Sesapi_Controller_Action_Standard
{
    ...
    public function createAction(){
      ...
      $levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
        ...
        if ($categoryShowAble && !$category_id && engine_count($categories) && $checkCategoriesForMemberLevel) {
       ...
       if($_GET['sesapi_platform'] == 1){
        ...
        try {
            ...
            if (isset($values['tags']) && engine_count($values['tags'])>0) {
    ...
    public function pagePackage() {
        ...
        if (!engine_count($packageMemberLevel) || !Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagepackage.enable.package', 0))
            return true;
    ...
    public function editAction(){
        ...
        //if (!$this->getRequest()->isPost()) {
            ...
            foreach ($roles as $role) {
              ...
              if (isset($form->auth_forum->options[$role]) && $auth->isAllowed($page, $role, 'forum'))
                $form->auth_forum->setValue($role);
                
              if (isset($form->auth_poll->options[$role]) && $auth->isAllowed($page, $role, 'poll'))
                $form->auth_poll->setValue($role);
        ...
        try {
            ...
            foreach ($roles as $i => $role) {
              ...
              $auth->setAllowed($page, $role, 'album', ($i <= $albumMax));
              $auth->setAllowed($page, $role, 'video', ($i <= $videoMax));
              $auth->setAllowed($page, $role, 'forum', ($i <= $forumMax));
              $auth->setAllowed($page, $role, 'poll', ($i <= $pollMax));
              //Note
