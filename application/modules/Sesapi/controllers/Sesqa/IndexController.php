...
class Sesqa_IndexController extends Sesapi_Controller_Action_Standard{
	...
	public function getQuestions($paginator,$manage){
		...
		foreach($paginator as $question){
			...
			$pollOptions = engine_count($question->getOptions());
  ...
  public function favouriteAction() {
    ...
    $data['current_count'] = $current_count = engine_count(Engine_Api::_()->getDbtable('questions', 'sesqa')->getQuestions($values));
    ...
    $data['isPollDisable'] = $isPollDisable = $question->vote_count && engine_count($pollOptions);
    $options = array_filter(array_map('trim', $options));
    $options = array_slice($options, 0, $max_options);
    $data['options'] = $options;
    if($form->video)
    ...
    if(!empty($_POST['is_poll']) &&  (empty($options) || !is_array($options) || engine_count($options) < 2 )) {
  ...
  public function editAction(){
   ...
   // $sesqa_create = Zend_Registry::isRegistered('sesqa_create') ? Zend_Registry::get('sesqa_create') : null;
    ...
    $data['isPollDisable'] = $question->vote_count && engine_count($pollOptions);
    $options = array_filter(array_map('trim', $options));
    $options = array_slice($options, 0, $max_options);
    $data['options'] = $options;
    if($form->video)
    ...
    try {
      ...
      if(!empty($_POST['is_poll'])){
         if(($isPollOptionUpdate && !$question->vote_count) || !engine_count($pollOptions)){
          $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
    ...
    $paginator->setCurrentPageNumber($this->_getParam('page', 1));
		...
		foreach($questionOptions as $options){
			...
			if( engine_in_array($options->poll_option_id,$hasVoted))
