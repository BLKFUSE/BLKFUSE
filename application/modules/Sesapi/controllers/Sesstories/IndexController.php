...
class Sesstories_IndexController extends Sesapi_Controller_Action_Standard
{
    ...
    public function alltextbackgroundsAction() {
      
      $backgrounds = Engine_Api::_()->getDbTable('backgrounds', 'sesstories')->getBackgrounds(array('fetchAll' => 1, 'admin' => 1));
      
      $story_backgrounds = $result = array();
      $counterLoop =  0;
      ...
      foreach ($backgrounds as $background) {
          ...
          $story_backgrounds[$counterLoop]['image'] = $this->getBaseUrl(true, $photo);
          $story_backgrounds[$counterLoop]['background_id'] = $background->getIdentity();
          $counterLoop++;
      }
      $result['backgrounds'] = $story_backgrounds;
      Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => $result));
    }
            ...
            if (engine_count($viewerresults) > 0) {
                foreach ($viewerresults as $item) {
                    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('elivestreaming')) {
                        ...
                        if ($elivehost && ((_SESAPI_VERSION_IOS < 2.0 && _SESAPI_PLATFORM_SERVICE == 1))) {
                            continue;
                        }
                    }
                    // for live streaming.
                    ...
                    if(isset($item->file_id) && !empty($item->file_id) && empty($item->background_id)) {
                    ...
                    $images['story_content'][$counter]['comment'] = Engine_Api::_()->core()->decode($item->title);
                    $images['story_content'][$counter]['story_type'] = $item->story_type;
                    if (!empty($item->type)) {
        ...
        if (engine_count($friendArra) > 0) {
            foreach ($friendArra as $key => $friend_id) {
                ...
                foreach ($results as $item) {
                    ...
                    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('elivestreaming')) {
                        $elivehost = Engine_Api::_()->getDbtable('elivehosts', 'elivestreaming')->getHostId(array('story_id' => $item->story_id));
                        if($elivehost && ((_SESAPI_VERSION_IOS > 2.0 && _SESAPI_PLATFORM_SERVICE == 1) || (_SESAPI_VERSION_ANDROID > 3.0 && _SESAPI_PLATFORM_SERVICE == 2))) {
                        ...
                        foreach ($images['story_content'] as $key => $item) {
                            if ($key == 0) {
                                ...
                                if ($elivehost && $elivehost['status'] == 'started') {
                                    ...
                                    $result['stories'][$counterLoop]['elivehost_id'] = $elivehost->getIdentity();
                                } else {
    ...
    public function createAction()
    {
        ...
        if(!empty($_POST['background_id'])) {
          $values['background_id'] = $_POST['background_id'];
        }
        //if(!empty($_POST['story_type'])) {
          $values['story_type'] = $_POST['story_type'] ? $_POST['story_type'] : 'imagevideo';
        //}
            ...
            foreach ($_FILES['attachmentVideo']['name'] as $key => $files) {
                ...
                if (!empty($_FILES['attachmentVideo']['name'][$key])) {
                    try {
                        ...
                        $item->title = $_POST['comment'][$key] ? Engine_Api::_()->core()->encode($_POST['comment'][$key]) : "";
                        $item->save();
                                    ...
                                    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('elivestreaming')) {
                                        if (!empty($postData['elivehost_id'])) {
                                            ...
                                            if (!empty($elivehostItem)) {
                                                if ($postData['canPost'] == "true" || $postData['canPost'] == true) {
                                                    $elivehostItem->status = 'completed';
                                                    //$elivehostItem->delete();
                                                } else {
                                                    if ($postData['canPost'] == "false" || $postData['canPost'] == false)
                ...
                if (!empty($_FILES['attachmentImage']['name'][$key])) {
                    try {
                        ...
                        if (isset($_POST['comment'][$key]))
                            $item->title = Engine_Api::_()->core()->encode($_POST['comment'][$key]);
                        else
                    ...
                    $images['story_content'][$counter]['media_url'] = $this->getBaseUrl(true, $storageObject->map());
                    $images['story_content'][$counter]['comment'] = Engine_Api::_()->core()->encode($item->title);
                    $images['story_content'][$counter]['is_video'] = false;
                    $images['story_content'][$counter]['highlight'] = $item->highlight;
                    $images['story_content'][$counter]['like_count'] = $item->like_count;
                    $images['story_content'][$counter]['comment_count'] = $item->comment_count;
        ...
        //code for text story
        if(isset($_POST['background_id']) && !empty($_POST['background_id'])) {
          try {
            $item = $table->createRow();
            $values['plateform'] = 3;
            $item->setFromArray($values);
            $item->title = $_POST['title'] ? Engine_Api::_()->core()->encode($_POST['title']) : '';
            $item->background_id = $_POST['background_id'] ? $_POST['background_id'] : '0';
            $item->view_privacy = $auth_view;
            $item->save();
