...
class Sesthought_IndexController extends Sesapi_Controller_Action_Standard
{
  ...
  public function categoryAction($getCategory = false){
    ...
    foreach($paginator as $category){
      ...
      if($category->cat_icon != '' && !is_null($category->cat_icon) && intval($category->cat_icon)):
        $catgeoryArray[$counter]["cat_icon"] = Engine_Api::_()->storage()->get($category->cat_icon) ? $this->getBaseUrl(false,Engine_Api::_()->storage()->get($category->cat_icon)->getPhotoUrl('thumb.icon')) : "";
      endif;
  ...
  public function getThoughts($paginator,$manage = false){
    ...
    foreach($paginator as $thoughts){ 
        ...
        if(is_countable($images) && engine_count($images))
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
     ...
     if( !$form->isValid($_POST) ) {
       ...
       if($mediaType == 2){
          ...
          if(!$valid){
            $count = engine_count($validateFields) - 1;
            $video[$count]["type"] = "";
  ...
  public function editAction()
  {
    ...
    if( !$this->_helper->requireAuth()->setAuthParams($thought, $viewer, 'edit')->isValid() ) 
      ...
      if($this->_getParam('getForm')) {  
        ...
        if(is_countable($formFields) && engine_count($formFields) &&  $thought->category_id){
