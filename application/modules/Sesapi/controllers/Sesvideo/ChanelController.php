...
class Sesvideo_ChanelController extends Sesapi_Controller_Action_Standard {
  ...
  public function indexAction() {
      ...
      if(!engine_count( $response["channel"]['images']))
  ...
  public function browseAction() {
   ...
   $search_for = $this-> _getParam('search_for', 'chanel');
    ...
    if($manage == "manage"){
      if($this->_getParam("channel_owner_id")){
        $value['user_id'] = $this->_getParam("channel_owner_id");
        $value["manageChannel"] = "manageVideo";
      }else if($this->view->viewer()->getIdentity()){
  ...
  protected function getChannels($paginator,$manage = ""){
    ...
    foreach($paginator as $channel){
        ...
        $item['user_title'] = $channel->getOwner()->getTitle();
        $item['user_image'] = $this->userImage($channel->getOwner()->getIdentity(),"thumb.profile");
        $item['user_username'] = $channel->getOwner()->username;
        if($sesmember){
          $select = $table->select()
                    ->from($table->info('name'))
                    ->setIntegrityCheck(false)
                    ->joinLeft($userinfoTableName, "$userinfoTableName.user_id = $tableName.user_id",array('userinfo_id', 'follow_count'))
                    ->where($table->info("name").'.user_id = ?',$channel->getOwner()->getIdentity());

          $userObj = $table->fetchAll($select);
          if($userObj && engine_count($userObj)){
            $item["user_follow_count"] = $userObj[0]["follow_count"];
          }else{
            $item["user_follow_count"] = 0;
          }
        }
      //$item['user_follow_count'] = $channel->getOwner()->follow_count;
         if($manage){
            $menuoptions= array();
  ...
  public function createAction() {
    ...
    if (isset($values['video_ids']) && $values['video_ids'] != '' && isset($chanel->chanel_id)) {
      ...
      //Activity Feed work
      $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity(Engine_Api::_()->user()->getViewer(), $chanel, 'sesvideo_chanel_new', null, array('count' => engine_count(explode(',', rtrim($values['video_ids'], ',')))));
...
//      }
  ...
  public function deleteVideoAction(){
    ...
    $dbObject = Engine_Db_Table::getDefaultAdapter();
    $queryVideos = $dbObject->query('DELETE FROM engine4_sesvideo_chanelvideos WHERE  `chanel_id` = ' . $id .' && video_id ='.$video_id);
  ...
  public function editAction() {
    ...
    if($this->_getParam('getForm')) {
      ...
      if(is_countable($formFields) && engine_count($formFields) &&  $chanel->category_id){
    ...
    try {
      ...
      if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
        $ThumbnailIconId = $this->setPhoto($_FILES["image"], $chanel->chanel_id);
        if (!empty($ThumbnailIconId)) {
          if ($previousThumbnail && $deleteTh) {
            $chanelThub = Engine_Api::_()->getItem('storage_file', $previousThumbnail);
            $chanelThub->delete();
    ...
    if (isset($values['video_ids']) && $values['video_ids'] != '' && isset($chanel->chanel_id)) {
      ...
      if ($runQuery) {
        ...
        $stmt = $dbObject->query($query . rtrim($queryString, ','));
			...
			$totalNewVideos = engine_count($newVideos);
...
<td><img src="'.$chanel->getPhotoUrl().'" width="30" height="30" style="display:block" border="0" /></td><td style="padding-left:10px;"><a href="'.$siteUrl.$chanel->getHref().'" style="font-family:arial,Arial,sans-serif;font-size:12px;color:#222222;line-height:15px;text-decoration:none" target="_blank">'.$chanel->getTitle().'</a></td></tr></table>
								...
								if(engine_count($recommendedVideos)){
...
<span style="display:block;"><a href="'.$siteUrl.$user->getHref().'" style="font-family:arial,Arial,sans-serif;font-size:12px;color:#999999;line-height:15px;letter-spacing:0px;text-decoration:none" target="_blank">by '.$user->getTitle().'</a>
			...
			if(engine_count($followerChannel)){
  ...
  function channelFollowersAction(){
      $channel_id = $this->_getParam('channel_id');
      if(!$channel_id){
          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>'parameter_missing','result'=>''));
      }
      $channel = Engine_Api::_()->getItem('sesvideo_chanel',$channel_id);
      $paginator = Engine_Api::_()->getDbtable('chanelfollows', 'sesvideo')->getChanelFollowers($channel->chanel_id,true);
      ...
      $extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page']+1;
      if($result <= 0)
          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'0','error_message'=>'No followers found yet.', 'result' => array()));
      else
          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array_merge(array('error'=>'0','error_message'=>'', 'result' => $result),$extraParams));
  }
  ...
  public function chanelDataAction() {
    ...
    if ($chanel_id) {
      ...
      if (engine_in_array('chanelVideo', $show_criterias)) {
