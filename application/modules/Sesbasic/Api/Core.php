...
class Sesbasic_Api_Core extends Core_Api_Abstract {
  ...
  public function isModuleEnable($name = array()) {
...
//middle left
  ...
  //upload photo
  public function setPhoto($photo, $isURL = false, $isUploadDirect = false, $modulename = null, $memberlevelType = false, $photoParams = array(), $item = false, $package = false, $sameThumbWatermark = false,$watermarkLabel = 'watermark') {
    if (!$isURL) {
      ...
      if($PhotoExtension == '.') {
				$PhotoExtension = '.jpg';
			} else if(strpos($PhotoExtension,"?")) {
				$PhotoExtension = explode('?', $PhotoExtension);
				if($PhotoExtension[0])
					$PhotoExtension = $PhotoExtension[0];
			}
    ...
    if ($enableWatermark == 1) {
      ...
      if (is_file($watermarkImage)) {
        if (isset($extension))
          ...
          $type = $PhotoExtension;
        $fileNew = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary' . DIRECTORY_SEPARATOR . time() . '_' . $fileName . ".jpg";
        $watemarkImageThumbResult = $this->watermark_image($normalMainPath, $fileNew, $type, $watermarkImage, $modulename);
