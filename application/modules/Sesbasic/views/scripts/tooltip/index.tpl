<?php $subject = $this->subject; ?>
  <?php if(engine_in_array('coverphoto',$this->globalEnableTip) && engine_in_array('coverphoto',$this->moduleEnableTip) && isset($this->subject->cover_photo) && $this->subject->cover_photo != 0 && $this->subject->cover_photo != ''){ 
...
<div class="sesbasic_tooltip sesbasic_clearfix sesbasic_bxs<?php if($cover){?> sesbasic_tooltip_cover_wrap<?php } ?>">
  ...
  <?php if(engine_in_array('mainphoto',$this->globalEnableTip) && engine_in_array('mainphoto',$this->moduleEnableTip)){ ?>
   ...
   <?php } ?>
    <div class="sesbasic_tooltip_info">
      ...
      <?php } ?>
      <?php if( engine_in_array('hostedby',$this->moduleEnableTip)){ 
      ...
      <?php } ?>
      <?php } ?>
     <?php if( engine_in_array('startendtime',$this->moduleEnableTip)){ ?>
      <p class="sesbasic_tooltip_info_stats sesevent_list_stats sesevent_list_time">
      	<span class="widthfull">
     ...
     <?php } ?>
     <?php
     if( engine_in_array('category',$this->moduleEnableTip) && isset($subject->category_id) && $subject->category_id != '' && intval($subject->category_id) && !is_null($subject->category_id)){
     ...
     ?>
     <?php if( engine_in_array('location',$this->moduleEnableTip) && isset($subject->location) &&  $subject->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesevent.enable.location', 1)){ ?>
  ...
  ?>
	<?php  if(engine_in_array('buybutton',$this->moduleEnableTip) && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seseventticket') && Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventticket.pluginactivated')) { 
