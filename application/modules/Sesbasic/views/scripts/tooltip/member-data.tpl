...
<?php else:?>
  <?php $levelId = $viewer;?>
<?php endif;?>
<?php if(engine_in_array('coverphoto',$this->globalEnableTip) && engine_in_array('coverphoto',$this->moduleEnableTip) && isset($this->subject->coverphoto) && $this->subject->coverphoto != 0 && $this->subject->coverphoto != ''){
...
<div class="sesbasic_tooltip sesbasic_clearfix sesbasic_bxs<?php if($cover){?> sesbasic_tooltip_cover_wrap<?php } ?>">
  ...
  <div class="sesbasic_tooltip_cover">
    ...
    <div class="sesbasic_tooltip_cover_info">
      <?php if(engine_in_array('title',$this->globalEnableTip) && engine_in_array('title',$this->moduleEnableTip)){ ?>
        <div class="sesbasic_tooltip_info_title">  
          <a href="<?php echo $subject->getHref(); ?>"><?php echo $subject->getTitle(); ?></a></a>
        </div>
      <?php } ?>
      <?php if(engine_in_array('rating',$this->moduleEnableTip) && Engine_Api::_()->getApi('core', 'sesmember')->allowReviewRating()):?>
  ...
  <?php if(engine_in_array('mainphoto',$this->globalEnableTip) && engine_in_array('mainphoto',$this->moduleEnableTip)){ ?>
    <div class="sesbasic_tooltip_photo sesbd">
      <a href="<?php echo $subject->getHref(); ?>"><img src="<?php echo $imageURL; ?>"></a>
      <?php
      if(engine_in_array('socialshare',$this->moduleEnableTip)){
   ...
   <?php } ?>
    <div class="sesbasic_tooltip_info">
        ...
        <?php } ?>
        <?php if(isset($getUserInfoItem->rating) && $getUserInfoItem->rating && engine_in_array('rating',$this->moduleEnableTip) && Engine_Api::_()->getApi('core', 'sesmember')->allowReviewRating() && isset($getUserInfoItem->rating)):?>
          <div class="sesmember_list_rating">
            <?php echo $this->partial('_userRating.tpl', 'sesmember', array('rating' => $getUserInfoItem->rating)); ?>
          </div>
        <?php endif;?>
      <?php } ?>
	<?php if(engine_in_array('profileType',$this->moduleEnableTip)):?>
	  <div class="sesbasic_tooltip_stats sesmember_list_membertype "> 
        ...
        <?php endif;?>
        <?php if(engine_in_array('age',$this->moduleEnableTip)):?>
        ...
        <?php if(engine_in_array('friendCount',$this->moduleEnableTip) && Engine_Api::_()->getApi('settings', 'core')->user_friends_eligible && $cfriends && !$viewer->isSelf($subject)):?>
          ...
          <?php endif;?>
          <?php if(engine_in_array('mutualFriendCount',$this->moduleEnableTip) && ($viewer->getIdentity() && !$viewer->isSelf($subject)) && $mcount =  Engine_Api::_()->sesmember()->getMutualFriendCount($subject, $viewer) ): ?> 
	    <div class="sesbasic_tooltip_stats">
          ...
          <?php endif;?>
          <?php if(engine_in_array('email',$this->moduleEnableTip)):?>
	    <div class="sesbasic_tooltip_stats">
	    <span class="widthfull"><i class="fa fa-envelope"></i><span><?php echo $subject->email;?></span></span>
	  </div>
        <?php endif;?>
	<div class="sesbasic_tooltip_stats">
	  <?php if(engine_in_array('like',$this->moduleEnableTip)):?>
	  ...
	  <?php endif;?>
	  <?php if(engine_in_array('view',$this->moduleEnableTip)):?>
	  ...
	  <?php endif;?>
	  <?php if(engine_in_array('rating',$this->moduleEnableTip)):?>
	...
	<?php if( engine_in_array('location',$this->moduleEnableTip) && isset($getUserInfoItem->location) &&  $getUserInfoItem->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmember.enable.location', 1)){ ?>
  ...
  <div class="sesbasic_tooltip_footer sesbasic_clearfix sesbm clear">
    <div class="floatR">
      <?php if(engine_in_array('friendButton',$this->moduleEnableTip) && $viewer->getIdentity() != 0):?>
	<?php echo '<span>'.$this->partial('_addfriend.tpl', 'sesbasic', array('subject' => $subject)).'</span>'; ?>
      <?php endif;?>
      <?php if(engine_in_array('likeButton',$this->moduleEnableTip) && $viewer->getIdentity() != 0):?>
	...
	<?php echo "<span><a href='javascript:;' data-url='".$subject->getIdentity()."' class='sesbasic_btn sesmember_add_btn sesmember_button_like_user sesmember_button_like_user_". $subject->user_id."'><i class='fa ".$likeClass."'></i><span><i class='fa fa-caret-down'></i>$likeText</span></a></span>";?>
      <?php endif;?>
      <?php if(engine_in_array('followButton',$this->moduleEnableTip) && $viewer->getIdentity() != 0 &&  Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmember.follow.active',1) && !$viewer->isSelf($subject)){
	...
	echo "<span><a href='javascript:;' data-url='".$subject->getIdentity()."' class='sesbasic_btn sesmember_add_btn sesmember_follow_user sesmember_follow_user_".$subject->getIdentity()."'><i class='fa ".$followClass."'  title='$followText'></i> <span><i class='fa fa-caret-down'></i> Follow</span></a></span>"; 
      ...
      ?>
      <?php if (engine_in_array('message',$this->moduleEnableTip) && Engine_Api::_()->sesbasic()->hasCheckMessage($subject)): ?>
