...
class Sescommunityads_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($params['state'] == 'cancel') {
      ...
      Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($adsOwner, $adsOwner, $item, 'sescommunityads_paymentcancel', array("adsLink" => $notificationlink));
    ...
    if ($package->isOneTime()) {
      ...
      if ($paymentStatus == 'okay' || $paymentStatus == 'active' ||
        ...
        //For Credit 
        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sescredit') && !empty($params['creditCode'])) {
      ...
      switch ($rawData['txn_type']) {
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Expired': // Not sure about this one
                ...
                $link = '<a href="/ads/view/ad_id/'.$item->sescommunityad_id.'">'.$item->getTitle().'</a>';
        ...
        case 'recurring_payment_outstanding_payment':
          ...
          // send notification
            ...
            $link = '/ads/view/ad_id/'.$item->sescommunityad_id;
						$notificationlink = '<a href="' . $link . '">' . $item->getTitle() . '</a>';
            Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $owner, $item, 'sescommunityads_adsoverdue', array("adsLink" => $notificationlink));
