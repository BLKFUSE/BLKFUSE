...
<script type="text/javascript">
  if('<?php echo $enableDescription;?>' == 1)
    scriptJquery('#sescontest_show_description_create-wrapper').show();
  else
    scriptJquery('#sescontest_show_description_create-wrapper').hide();
  if('<?php echo $enableTag;?>' == 1)
    scriptJquery('#sescontest_show_tag_create-wrapper').show();
  else
    scriptJquery('#sescontest_show_tag_create-wrapper').hide();
  showViewPrivacy('<?php echo $enableViewPrivacy;?>');
  function showViewPrivacy(value) {
    if(value == 1)
    scriptJquery('#sescontest_default_viewprivacy-wrapper').hide();
    else
    scriptJquery('#sescontest_default_viewprivacy-wrapper').show();
  }
  showCommentPrivacy('<?php echo $enableCommentPrivacy;?>');
  function showCommentPrivacy(value) {
    if(value == 1)
    scriptJquery('#sescontest_default_commentprivacy-wrapper').hide();
    else
    scriptJquery('#sescontest_default_commentprivacy-wrapper').show();
  }
</script>
