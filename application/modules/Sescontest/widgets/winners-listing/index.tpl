...
<?php if(!$this->is_ajax){ ?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sescontest_winners_listing sesbasic_clearfix sesbasic_bxs">
    <div id="tabbed-widget_<?php echo $randonNumber; ?>" class="<?php if($this->view_type == 'grid'):?>row<?php endif;?>  sescontest_winners_list sesbasic_clearfix">
<?php } ?>
        ...
        requestViewMore_<?php echo $randonNumber; ?> = scriptJquery.ajax({
          ...
          success: function(responseHTML) {
              ...
              scriptJquery('#loading_images_browse_<?php echo $randonNumber; ?>').remove();
							 scriptJquery('#tabbed-widget_<?php echo $randonNumber; ?>').find('.sesbasic_loading_container').remove();
              ...
              document.getElementById('loading_image_<?php echo $randonNumber; ?>').style.display = 'none';
							if(scriptJquery('#sescontest_pinboard_view_<?php echo $randonNumber; ?>').length > 0){
								if(scriptJquery('#sescontest_pinboard_view_<?php echo $randonNumber; ?>').hasClass('active')){
									pinboardLayout_<?php echo $randonNumber ?>();
								}
							}
