 ...
 */
class Sescontestjoinfees_Plugin_Gateway_Cashfree extends Engine_Payment_Plugin_Abstract
{
  ...
  public function orderTicketTransactionReturn($order,$params){
    ...
    if( $paymentStatus == 'okay' ||
      ...
      $orderTicket->gateway_transaction_id = $params['referenceId'];
      ...
      $order->gateway_profile_id = $params['referenceId'];
			$order->save();
      // Order pending
  ...
  public function createContestOrderTransaction($params = array()) {
    ...
    $params['orderCurrency'] = $currentCurrency;
    ksort($params);
    $signatureData = "";
