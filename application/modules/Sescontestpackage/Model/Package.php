...
class Sescontestpackage_Model_Package extends Core_Model_Item_Abstract {
  ...
  public function getItemModule($modulename = 'photo') {
    ...
    if ($modulesEnable && array_key_exists($modulename, $modulesEnable))
      return 1;
    return 0;
  }
  ...
  public function allowUploadVideo($item, $left = false) {
    if (is_int($item))
      ...
      return 0;
    $modulesEnable = json_decode($this->params, true);
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && engine_in_array('video', $modulesEnable['modules'])) {
  ...
  public function allowUploadMusic($item, $left = false) {
    if (is_int($item))
      ...
      return 0;
    $modulesEnable = json_decode($this->params, true);
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && engine_in_array('music', $modulesEnable['modules'])) {
  ...
  public function allowUploadPhoto($item, $left = false) {
    if (is_int($item))
      ...
      return 0;
    $modulesEnable = json_decode($this->params, true);
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && engine_in_array('photo', $modulesEnable['modules'])) {
