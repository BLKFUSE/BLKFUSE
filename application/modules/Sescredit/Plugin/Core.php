...
class Sescredit_Plugin_Core extends Zend_Controller_Plugin_Abstract {
  ...
  public function onActivityActionCreateAfter($event) {
    ...
    $levelId = $activityOwner->getIdentity() ? $activityOwner->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
    Engine_Api::_()->getDbTable('credits', 'sescredit')->insertUpdatePoint(array('type' => $itemObject->type, 'owner_id' => $activityOwner->getIdentity(), 'action_id' => $itemObject->action_id, 'object_id' => $itemObject->object_id, 'point_type' => 'credit', 'level_id' => $levelId));
  }
  ...
  public function onItemDeleteBefore($event) {
    ...
    if ($itemObject->getType() == 'activity_action') {
      $activityOwner = $itemObject->getOwner();
      $levelId = $activityOwner->getIdentity() ? $activityOwner->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
      Engine_Api::_()->getDbTable('credits', 'sescredit')->insertUpdatePoint(array('type' => $itemObject->type, 'owner_id' => $activityOwner->getIdentity(), 'action_id' => $itemObject->action_id, 'object_id' => $itemObject->object_id, 'point_type' => 'deduction', 'level_id' => $levelId));
    } else {
      ...
      foreach ($actions as $action) {
        ...
        $levelId = $activityOwner->getIdentity() ? $activityOwner->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
        Engine_Api::_()->getDbTable('credits', 'sescredit')->insertUpdatePoint(array('type' => $action->type, 'owner_id' => $activityOwner->getIdentity(), 'action_id' => $action->action_id, 'object_id' => $action->object_id, 'point_type' => 'deduction', 'level_id' => $levelId));
      }
    }
  }
  ...
  public function onUserLoginAfter($event) {
    ...
    if (!$typeInfo->enabled && isset($user) && !empty($user->user_id)) {
