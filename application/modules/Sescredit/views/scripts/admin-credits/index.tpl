        ...
        <?php foreach($this->moduleBaseActionTypes as $key => $value):?>
          <?php foreach($value as $typeKey =>$type): ?>
            ...
            <div class="sescredit_points_info_table_item">
              ...
              <div class="_activitytype"><?php echo str_replace(array('(subject)','(object)',' .'),array('','','.'),$this->translate($type));?></div>
              <div class="_status admin_table_centered">
...
<script type="text/javascript">
  ...
  function changeStatus(value_id,element) {
    scriptJquery(element).find('img').attr('src','application/modules/Core/externals/images/loading.gif');
    console.log(value_id);
    if(value_id == 0) {
      var value = scriptJquery(element).parent().find('input').val();
      if(value == 1)
        value = 0;
      else
        value = 1;
      scriptJquery(element).parent().find('input').val(value);
      console.log(value,scriptJquery(element).parent().find('input'));
      updateImage(value,element);
      return;
    }
    var sendAjaxRequest  = scriptJquery.ajax({
      dataType: 'html',
      method: 'post',
  ...
  function updateImage(value,element) {
    if(value == 1){
      scriptJquery(element).find('img').attr('src','application/modules/Sesbasic/externals/images/icons/check.png');
    }else{
      scriptJquery(element).find('img').attr('src','application/modules/Sesbasic/externals/images/icons/error.png');
    }
  }
</script>
