 ...
 * @version    $Id: create.tpl 2016-05-25 00:00:00 socialnetworking.solutions $
...
<?php $this->headScript()->appendFile(Zend_Registry::get('StaticBaseUrl'). 'application/modules/Sesmember/externals/scripts/core.js'); ?>
  ...
  <li class="sesbasic_clearfix sesmember_reviews">
        ...
        <div class='sesmember_review_listing_top_info_cont'>
          ...
          <p class="sesmember_review_listing_stats sesbasic_text_light">
            <?php if(is_array($this->stats) && engine_in_array('likeCount', $this->stats)): ?>
              <span title="<?php echo $this->translate(array('%s like', '%s likes', $this->review->like_count), $this->locale()->toNumber($this->review->like_count)); ?>"><i class="sesbasic_icon_like_o"></i><?php echo $this->review->like_count; ?></span>
            <?php endif; ?>
            <?php if(is_array($this->stats) && engine_in_array('commentCount', $this->stats)): ?>
              <span title="<?php echo $this->translate(array('%s comment', '%s comments', $this->review->comment_count), $this->locale()->toNumber($this->review->comment_count))?>"><i class="sesbasic_icon_comment_o"></i><?php echo $this->review->comment_count;?></span>
            <?php endif; ?>
            <?php if(is_array($this->stats) && engine_in_array('viewCount', $this->stats)): ?>
              <span title="<?php echo $this->translate(array('%s view', '%s views', $this->review->view_count), $this->locale()->toNumber($this->review->view_count))?>"><i class="sesbasic_icon_view"></i><?php echo $this->review->view_count; ?></span>
            <?php endif; ?>
          </p>
        </div>
    </div>
    ...
    <div class="sesmember_review_listing_desc sesbasic_clearfix">
      ...
      <?php endif; ?>
      <?php if(engine_in_array('description', $this->stats) && $this->review->description): ?>
        <p class='sesmember_review_listing_body'>
