...
<script type="text/javascript">
  ...
  function importMusicPlaylists() {
    ...
    document.getElementById('musicplaylist_import').style.display = 'none';
    en4.core.request.send(scriptJquery.ajax({
      ...
      success: function(responseJSON) {
        if (responseJSON.error_code) {
          ...
          document.getElementById('musicplaylist_message').innerHTML = "<span>Some error might have occurred during the import process. Please refresh the page and click on “Start Importing Music” again to complete the import process.</span>";
        } else {
          document.getElementById('loading_image').style.display = 'none';
          document.getElementById('musicplaylist_message').style.display = 'none';
          document.getElementById('musicplaylist_message1').innerHTML = "<span>" + '<?php echo $this->string()->escapeJavascript($this->translate("Playlists from SE Music have been successfully imported.")) ?>' + "</span>";
        }
      }
    }));
  }
...
<div class='settings'>
  <form class="global_form">
    <div>
      ...
      <?php if(engine_count($this->playlistresults) > 0): ?>
