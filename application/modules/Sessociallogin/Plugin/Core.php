...
class Sessociallogin_Plugin_Core extends Zend_Controller_Plugin_Abstract {
    ...
    public function onUserCreateAfter($event) {
      ...
      }else if($_SESSION['telegram_signup']) {
        $hotmailsignup = $settings->getSetting('sessociallogin.telegramsignup');
        $hotmail_signup = $hotmailsignup + 1;
        Engine_Api::_()->getDbTable('settings', 'core')->update(array('value' => $hotmail_signup), array('name = ?' => 'sessociallogin.telegramsignup'));
    ...
    public function onRenderLayoutDefault($event) {
        ...
        $script = '';
        if ($controllerName == 'signup' && $actionName == 'index' && !engine_count($_POST) && !empty($_SESSION['signup_fields']['email'])) {
            $email = $_SESSION['signup_fields']['email'];
            $script .= "scriptJquery(document).ready(function() {
        var elum = scriptJquery('.form-elements').find('input[type=email]');
        elum.each(function(){
        if(scriptJquery(this).closest('form').attr('action').indexOf('signup') >= 0) {
            scriptJquery(this).val('" . $email . "')
}
        });})";
        }
        $script .= "function onTelegramLogin(user){
