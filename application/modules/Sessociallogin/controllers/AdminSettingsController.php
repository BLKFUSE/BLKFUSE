...
class Sessociallogin_AdminSettingsController extends Core_Controller_Action_Admin {
    ...
    public function telegramAction() {
      $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sessociallogin_admin_main', array(), 'sessociallogin_admin_main_telegram');
      $this->view->form = $form = new Sessociallogin_Form_Admin_Settings_Telegram();
      if( _ENGINE_ADMIN_NEUTER ) {
        $form->populate(array(
            'sessociallogin_telegram_username' => '******',
            'sessociallogin_telegram_token' => '******',
        ));
      }
      if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
          $values = $form->getValues();
