 ...
 * @version    $Id: AdminManageController.php 2017-01-12  00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Sesstories_AdminManagestoriesController extends Core_Controller_Action_Admin {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $background = Engine_Api::_()->getItem('sesstories_background', $value);
          if($background)
          $background->delete();
        }
      }
    }
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesstories_admin_main', array(), 'sesstories_admin_main_bgtextstories');

    $this->view->paginator = Engine_Api::_()->getDbTable('backgrounds','sesstories')->getPaginator(array('admin' => 0));
  ...
  public function enabledAction() {
    ...
    if (!empty($id)) {
      $item = Engine_Api::_()->getItem('sesstories_background', $id);
      $item->enabled = !$item->enabled;
      $item->save();
    }
    $this->_redirect('admin/sesstories/managestories');
  }
  ...
  public function orderAction() {
    ...
    $backgroundsTable = Engine_Api::_()->getDbtable('backgrounds', 'sesstories');
  ...
  public function createAction() {
    ...
    $this->view->max_file_upload_in_bytes = $max_file_upload_in_bytes = Engine_Api::_()->sesstories()->max_file_upload_in_bytes();

    $this->view->form = $form = new Sesstories_Form_Admin_Background_Create();
    if($id){
      $item = Engine_Api::_()->getItem('sesstories_background',$id);
  ...
  public function deleteAction() {
    ...
    // Check post
    if ($this->getRequest()->isPost()) {
      $background = Engine_Api::_()->getItem('sesstories_background', $id);
      $background->delete();
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => 10,
          'parentRefresh' => 10,
  ...
  public function uploadZipFileAction(){
    ...
    $this->view->form = $form = new Sesstories_Form_Admin_Background_Zipupload();
    $this->view->upload_max_size = $upload_max_size = ini_get('upload_max_filesize');
