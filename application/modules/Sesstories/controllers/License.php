...
if ($this->getRequest()->isPost()) {
  ...
  //license key
  $postdata['licenseKey'] = @base64_encode($_POST['sesstories_licensekey']);
  ...
  if ($server_output == "OK" && $error != 1) {
    ...
    if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesstories.pluginactivated')) {
      ...
      $db->query('DROP TABLE IF EXISTS `engine4_sesstories_stories`;');
      $db->query('CREATE TABLE `engine4_sesstories_stories` (
      ...
      $db->query('DROP TABLE IF EXISTS `engine4_sesstories_mutes`;');
      $db->query('CREATE TABLE `engine4_sesstories_mutes` (
      ...
      $db->query('DROP TABLE IF EXISTS `engine4_sesstories_recentlyviewitems`;');
      ...
      $db->query('DROP TABLE IF EXISTS `engine4_sesstories_usersettings`;');
      ...
      include_once APPLICATION_PATH . "/application/modules/Sesstories/controllers/defaultsettings.php";

      Engine_Api::_()->getApi('settings', 'core')->setSetting('sesstories.pluginactivated', 1);
      Engine_Api::_()->getApi('settings', 'core')->setSetting('sesstories.licensekey', $_POST['sesstories_licensekey']);
    }
    ...
    Engine_Api::_()->getApi('settings', 'core')->setSetting('sesstories.sesdomainauth', $domain_name);
    Engine_Api::_()->getApi('settings', 'core')->setSetting('sesstories.seslkeyauth', $licensekey);
    $error = 1;
  } else {
