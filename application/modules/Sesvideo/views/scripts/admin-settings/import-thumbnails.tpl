...
<script type="text/javascript">
  ...
  function importVideoThumbnails() {
    ...
    document.getElementById('video_import').style.display = 'none';
    en4.core.request.send(scriptJquery.ajax({
      ...
      success: function(responseJSON) {
        if (responseJSON.error_code) {
          ...
          document.getElementById('video_message').innerHTML = "<span>Some error might have occurred during the import process. Please refresh the page and click on 'Start Importing' again to complete the import process.</span>";
        } else {
          document.getElementById('loading_image').style.display = 'none';
          document.getElementById('video_message').style.display = 'none';
          document.getElementById('video_message1').innerHTML = "<span>" + '<?php echo $this->string()->escapeJavascript($this->translate("Video thumbnails have been imported successfully.")) ?>' + "</span>";
        }
      }
    }));
  }
