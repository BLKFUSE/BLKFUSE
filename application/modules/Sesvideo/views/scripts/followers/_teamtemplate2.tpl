    ...
    <?php $user = Engine_Api::_()->getItem('user', $user->owner_id); ?>
      <div class="team_box" style="width:<?php echo $this->width ?>px;">
        <div class="team_box_inner">
          ...
          <?php if(!empty($this->content_show) && ($user->status || engine_in_array('email', $this->content_show) || engine_in_array('message', $this->content_show) || engine_in_array('addFriend', $this->content_show) || Engine_Api::_()->sesteam()->hasCheckMessage($user) || $row == NULL)): ?>
            <div class="sesteam-social-links team_member_info">
              <?php if($viewer->getIdentity() == $user->user_id): ?>
              <?php if(engine_in_array('email', $this->content_show)): ?>
