...
<script type="text/javascript">
  ...
  function set_rating() {
    ...
    for(var x=1; x<=parseInt(rating); x++) {
        scriptJuery('#rate_'+x).attr('class', 'fa fa-star');
      }

      for(var x=parseInt(rating)+1; x<=5; x++) {
        scriptJuery('#rate_'+x).attr('class', 'far fa-star star-disable');
      }

      var remainder = Math.round(rating)-rating;
      if (remainder <= 0.5 && remainder !=0){
        var last = parseInt(rating)+1;
        scriptJuery('#rate_'+last).attr('class', 'far fa-star-half');
      }
  }
  ...
  <div class="sesvideo_external_video_btn_cont clear sesbasic_clearfix">   
    ...
    <div class="sesvideo_view_meta sesvideo_list_stats sesbasic_text_light clear sesbasic_clearfix">
      <?php if( $this->category ): ?>
        <span><i class="far fa-folder-open"></i> <?php echo $this->htmlLink(array('route' => 'sesvideo_general', 'QUERY' => array('category_id' => $this->category->category_id)), $this->translate($this->category->category_name)) ?></span>
      <?php endif; ?>
    ...
    <?php
      ...
      if(engine_count($customMetaFields)>0){
