 ...
 */
class Sitead_Form_Admin_Create extends Engine_Form {
    ...
    public function init() {
        ...
        $site_ad_modules = Engine_Api::_()->getDbtable('modules', 'sitead')->getModuleName();
        // Getting the list of plugins enabled on the site
        $enabledModuleNames = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();

        // Which widget are enable we are tacking in array.
        $moduleArray = array_intersect($site_ad_modules, $enabledModuleNames);
        $moduleArray = array_unique($moduleArray);
        foreach ($moduleArray as $module) {
            $getInfo = Engine_Api::_()->getDbtable('modules', 'sitead')->getModuleInfo($module);
            if (!empty($getInfo)) {
        ...
        $catigoriesAvilable = array(
            'content' => 'Content Ad',
            'boost' => 'Boost Post',
            'page' => 'Page Ad',
            'website' => 'Website Ad'
        );
        // remove content option if no content module is installed
        if(empty($urloption_prepared)) {
            unset($catigoriesAvilable['content']);
            unset($catigoriesAvilable['page']);
        } else if( !(array_key_exists("sitepage", $urloption_prepared)) ) {
                unset($catigoriesAvilable['page']);
        }

        $this->addElement('Multiselect', 'add_categories', array(
            'label' => 'Ad Category',
            'description' => 'Select the Ad Category to which this Ad Package should be available.',
            'attribs' => array('style' => 'max-height:100px; '),
            'multiOptions' => $catigoriesAvilable,
            'value' => array_keys($catigoriesAvilable),
            'required' => true
        ));
        ...
        $this->addElement('Multiselect', 'urloption', array(
            ...
            'value' => array_keys($urloption_prepared)
        ));
        ...
        // Allowed ad in this Package
        $this->addElement('Text', 'allow_ad', array(
            'label' => 'Maximum Allowed Ads',
            'value' => 0,
            'description' => 'Enter the maximum number of Ads that a user can create. This field must contain an integer. (0 for unlimited ads) Note: A change in this setting, later on, will only apply on new ads that are created in this package.',
            'filters' => array(
                new Engine_Filter_Censor(),
            ),
        ));
        $this->allow_ad->getDecorator("Description")->setOption("placement", "append");

        //Boost
        $this->addElement('Checkbox', 'carousel', array(
            'label' => 'Carousel Ad format, you want to activate in this package.',
            'value' => 1,
        ));

        //Single Image
        $this->addElement('Checkbox', 'image', array(
            'label' => 'Image Ad format, you want to activate in this package.',
            'value' => 1,
        ));

        //Video
        $this->addElement('Checkbox', 'video', array(
            'label' => 'Video Ad format, you want to activate in this package. ',
            'value' => 1,
        ));
        ...
        $this->addElement('Checkbox', 'sponsored', array(
            'label' => 'Mark ads of this package as Sponsored.',
            'value' => 0,
        ));

        //featured
        $this->addElement('Checkbox', 'featured', array(
            'label' => 'Mark ads of this package as Featured.',
            'value' => 0,
        ));
        ...
        $this->addElement('Checkbox', 'public', array(
            'label' => 'Show ads of this package to non-logged-in visitors.',
            'value' => 1,
        ));
