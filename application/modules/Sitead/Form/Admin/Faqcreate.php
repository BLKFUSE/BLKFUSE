...
class Sitead_Form_Admin_Faqcreate extends Engine_Form
{
	public function init() {
		...
		$faq_type = 0;
		if ( !empty($faqId) ) {
			$faqObj = Engine_Api::_()->getItem('sitead_faq', $faqId);
		...
		if( empty($faq_default) ) {
			...
			if( $faq_type == 1 ) {
				$language_variable = '_sitead_help_generalfaq_';
			}else if( $faq_type == 2 ){
				$language_variable = '_sitead_help_designfaq_';
			}else if( $faq_type == 3 ){
				$language_variable = '_sitead_help_targetingfaq_';
			}
