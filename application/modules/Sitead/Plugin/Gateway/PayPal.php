 ...
 */
class Sitead_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
   ...
   * @param Sitead_Model_Package $package
   * @param array $params
  ...
  public function createUserSiteadTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $userad, Sitead_Model_Package $package, array $params = array()) {
    ...
    $params['driverSpecificParams']['PayPal'] = array(
        ...
        'ITEMS' => array(
            array(
                'NAME' => $view->translate("Ad on") . " " . Engine_Api::_()->getApi('settings', 'core')->getSetting('core.general.site.title') . ": " . ucfirst($userad->web_name),
                'DESC' => $package->getPackageDescription(),
                'AMT' => $package->price,
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($params['state'] == 'cancel') {
      ...
      if ($userad->didStatusChange()) {
        ...
        Engine_Api::_()->sitead()->sendMail("OVERDUE", $userad->userad_id);
      }
      // Error
    ...
    // Get benefit setting
    $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sitead')
    ...
    if ($paymentStatus == 'okay' ||
      ...
      if ($userad->didStatusChange()) {
        ...
        Engine_Api::_()->sitead()->sendMail("ACTIVE", $userad->userad_id);
      }
      ...
      if ($userad->didStatusChange()) {
        ...
        Engine_Api::_()->sitead()->sendMail("PENDING", $userad->userad_id);
      }
        ...
        case 'express_checkout':
          ...
          switch ($rawData['payment_status']) {
            ...
            case 'Pending':
              ...
              // Get benefit setting
              $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sitead')->getBenefitStatus($user);
              if ($giveBenefit) {
                $userad->onPaymentSuccess();
              } else {
                $userad->onPaymentPending();
            ...
            case 'Refunded':
              ...
              if ($userad->didStatusChange()) {
                // SEND OVERDUE MAIL HERE
                Engine_Api::_()->sitead()->sendMail("REFUNDED", $userad->userad_id);
              }
              break;
