 ...
 */
class Sitead_AdminHelpsController extends Core_Controller_Action_Admin {
  ...
  public function helpAndLearnmoreAction() {
    ...
    $pagesettingsTable = Engine_Api::_()->getItemTable('sitead_infopage');
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $contentObject = Engine_Api::_()->getItem('sitead_infopage', $value);
  ...
  public function statusAction() {
    ...
    // Check post
    if ($this->getRequest()->isPost()) {
      $pagesettingsTable = Engine_Api::_()->getDbTable('infopages', 'sitead');
  ...
  public function helpPageCreateAction() {
    ...
    if (!empty($page_id)) {
      $this->view->faqCheck = $faqCheck = Engine_Api::_()->getItem('sitead_infopage', $page_id)->faq;
    } else {
    ...
    if (empty($faqCheck)) {
      ...
      $this->view->form = $form = new Sitead_Form_Admin_Helppagecreate();


		$textFlag = $this->_getParam('textFlag', 0);
		if( empty( $textFlag ) ) {
			$form->removeElement('text_description');
			$textLinkFlag = $this->view->url(array('module' => 'sitead', 'controller' => 'helps', 'action' => 'help-page-create', 'textFlag' => 1, 'page_id' => $page_id), 'admin_default', true);
			...
			$textLinkFlag = $this->view->url(array('module' => 'sitead', 'controller' => 'helps', 'action' => 'help-page-create', 'textFlag' => 0, 'page_id' => $page_id), 'admin_default', true);
      ...
      if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
        ...
        $contentTable = Engine_Api::_()->getDbTable('infopages', 'sitead');
        if( empty($values['is_opration']) ) {
          $widgetDisplay = $contentTable->createRow();
          $widgetDisplay->title = $values['title'];
          $widgetDisplay->description = $values['description'];
          $widgetDisplay->status = 1;
          $widgetDisplay->delete = 1;
          $widgetDisplay->faq = 0;
          $widgetDisplay->save();
        } else {
          $contentTable->update(array('title' => $values['title'], 'description' => $values['description']), array('infopage_id =?' => $values['is_opration']));
        }
        $this->_helper->redirector->gotoRoute(array('module' => 'sitead', 'controller' => 'helps', 'action' => 'help-and-learnmore'), 'admin_default', true);
      }
    } else { // Condition: Faq page come here.
      // Show the navigation bar.
      $page = $this->_getParam('page', 1); // Page id: Controll pagination.
			$this->view->page_title = Engine_Api::_()->getItem('sitead_infopage', $page_id)->title;
      $socialengineTable = Engine_Api::_()->getItemTable('sitead_faq');
      ...
      if ($this->getRequest()->isPost()) {
        ...
        foreach ($values as $key => $value) {
          if ($key == 'delete_' . $value) {
            Engine_Api::_()->getItem('sitead_faq', $value)->delete();
          }
        }
      }
    }
  ...
  public function helpPageDeleteAction() {
    ...
    // Check post
    if ($this->getRequest()->isPost()) {
      $pagesettingTable = Engine_Api::_()->getItem('sitead_infopage', $pagesetting_id);
	...
	public function defaultHelpMsgAction(){
		...
		if( !empty($pagesetting_id) ) {
			$this->view->item = $item = Engine_Api::_()->getItem('sitead_infopage', $pagesetting_id);
		}
	}
