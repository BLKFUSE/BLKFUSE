 ...
 */
class Sitead_AdminPaymentController extends Core_Controller_Action_Admin {

    public function indexAction() {

        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('sitead_admin_main', array(), 'sitead_admin_payment_history');
        // Test curl support
        ...
        // Make form
        $this->view->formFilter = $formFilter = new Sitead_Form_Admin_Transaction_Filter();
        ...
        // Initialize select
        $transactionsTable = Engine_Api::_()->getDbtable('transactions', 'sitead');
        $transactionsName = $transactionsTable->info('name');
        $orderssName = Engine_Api::_()->getDbtable('orders', 'payment')->info('name');
        $useradsName = Engine_Api::_()->getDbtable('userads', 'sitead')->info('name');
        $transactionSelect = $transactionsTable->select()
                ...
                ->join($useradsName, $useradsName . '.userad_id=' . $orderssName . '.source_id', array($useradsName . '.web_name', $useradsName . '.userad_id'));
        ...
        if (!empty($filterValues['query'])) {
            $transactionSelect
                    ->joinRight('engine4_users', 'engine4_users.user_id=engine4_sitead_transactions.user_id', null)
        ...
        $this->view->sitecreditEnabled = $sitecreditEnabled = Engine_Api::_()->hasModuleBootstrap('sitecredit');
        if (!empty($sitecreditEnabled)) {
            $transactionSelect->joinLeft('engine4_sitecredit_credits', 'engine4_sitecredit_credits.type_id=engine4_sitead_transactions.order_id', "engine4_sitecredit_credits.credit_point");
        }

        if (($user_id = $this->_getParam('user_id', @$filterValues['user_id']))) {
            $this->view->filterValues['user_id'] = $user_id;
            $transactionSelect->where('engine4_sitead_transactions.user_id = ?', $user_id);
        }
    ...
    public function detailAction() {
        ...
        if (!($transaction_id = $this->_getParam('transaction_id')) ||
                !($transaction = Engine_Api::_()->getItem('sitead_transaction', $transaction_id))) {
            return;
        }
        ...
        $this->view->title = Engine_Api::_()->getItem('userads', $this->view->order->source_id)->web_name;
        $this->view->user = Engine_Api::_()->getItem('user', $transaction->user_id);

        if (Engine_Api::_()->hasModuleBootstrap('sitecredit')) {
            $this->view->credits = Engine_Api::_()->getDbtable('credits', 'sitecredit')->fetchRow(array('type =?' => 'sitead_package', 'type_id =?' => $transaction->order_id));
        }
    }
    ...
    public function rawOrderDetailAction() {
        ...
        if (null != ($transaction_id = $this->_getParam('transaction_id')) &&
                null != ($transaction = Engine_Api::_()->getItem('sitead_transaction', $transaction_id))) {
            $gateway = Engine_Api::_()->getItem('payment_gateway', $transaction->gateway_id);
