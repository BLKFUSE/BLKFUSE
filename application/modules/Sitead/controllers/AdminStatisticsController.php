 ...
 */
class Sitead_AdminStatisticsController extends Core_Controller_Action_Admin {

    public function exportReportAction() {
        Engine_Api::_()->getDbTable('adstatistics', 'sitead')->removeOldStatistics();
        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('sitead_admin_main', array(), 'sitead_admin_main_statistics');
        ...
        $this->view->reportform = $reportform = new Sitead_Form_Admin_Report();
        ...
        if (!empty($_GET['type'])) {
            ...
            if (empty($values['format_report'])) {
                $url = $urlProtocol . $_SERVER['HTTP_HOST'] . $this->view->url(array('module' => 'sitead', 'controller' => 'statistics', 'action' => 'export-webpage', 'start_daily_time' => $start_tm, 'end_daily_time' => $end_tm), 'admin_default', true) . '?' . $url_values[1];
            } else {
                $url = $urlProtocol . $_SERVER['HTTP_HOST'] . $this->view->url(array('module' => 'sitead', 'controller' => 'statistics', 'action' => 'export-excel', 'start_daily_time' => $start_tm, 'end_daily_time' => $end_tm), 'admin_default', true) . '?' . $url_values[1];
            }
            // Session Object
    ...
    /**
     * Genterate ad statistics report excel file
     */
    public function exportExcelAction() {
        ...
        if (!empty($_GET)) {
            ...
            if ($values['type'] == 'Advertising Performance') {
                $stat_object = Engine_Api::_()->getDbTable('adstatistics', 'sitead')->getStats($values);
                $rawdata = Engine_Api::_()->getDbTable('adstatistics', 'sitead')->fetchAll($stat_object);
            }
            ...
            $url_values = explode('?', $url_string);
            $urlProtocol = _ENGINE_SSL ? 'https://' : 'http://';
            $url = $urlProtocol . $_SERVER['HTTP_HOST'] . $this->view->url(array('module' => 'sitead', 'controller' => 'statistics', 'action' => 'export-report', 'empty' => '1'), 'admin_default', true) . '?' . $url_values[1];
    ...
    /**
     * Genterate ad statistics report on WebPage 
     */
