...
<script type="text/javascript">
  ...
  window.location.href='<?php echo $this->url(array('module' => 'sitead', 'controller' => 'packagelist', 'action' => 'index'), 'admin_default', true); ?>'+'/index/type/'+value;
  }  
</script>
...
Below, you can manage existing ad packages on your site, or create new ones. You can create different packages for the different Ad Types. You can also set the sequence of ad packages in the order in which they should appear to advertisers on the package listing page in the first step of ad creation. To do so, drag-and-drop the packages vertically and click on "Save Order" to save the sequence.') ?>
 ...
 <br style="clear:both;" />
<div>
  <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'packagelist', 'action' => 'create'), $this->translate('Create New Ad Package'), array('class'=>'cmad_icon_package_add buttonlink')) ?> 
...
<?php $currency = Engine_Api::_()->getApi('settings', 'core')->getSetting('payment.currency', 'USD');
    ...
    <table class='admin_table admin_table_packagelist' style="width:100%;">
      <thead>
        <tr>
	          ...
	          <div style="width:13%;" class="admin_table_centered"><?php echo $this->translate("Name") ?></div>
	          <div style="width:12%;" class="admin_table_centered"><?php echo $this->translate("Model") ?></div>
	          <div style="width:10%;" class="admin_table_centered"><?php echo $this->translate("Detail") ?></div>        
	          <div style="width:10%;" class="admin_table_centered"><?php echo $this->translate("Price") ?></div>
            ...
            <?php endif; ?>
	          ...
	          <div style="width:10%;" class="admin_table_centered"><?php echo $this->translate("Total Ads") ?></div>
	          <div style="width:8%;" class="admin_table_centered"><?php echo $this->translate("Enabled") ?></div>
	          <div style="width:10%;" class="admin_table_centered"><?php echo $this->translate("Created On") ?></div>
	          <div style="width:15%;" class="admin_table_centered"><?php echo $this->translate("Options") ?></div>
	        </th>  
        </tr>
      </thead>
   </table>
   <form id='saveorder_form' method='post' action='<?php echo $this->url(array('action' =>'update')) ?>'>
    ...
    <div id='order-element' class="seaocore_admin_order_list">
      <ul id='menu_list'>
      <?php foreach ($this->paginator as $item) :
      ?>
        <li id="content_<?php echo $item->package_id; ?>" class="package-list">
          <input type='hidden'  name='order[]' value='<?php echo $item->package_id; ?>'>
                	...
                	<div style="width:13%;" title= "<?php echo ucfirst($item->title) ?>">
                		<?php echo ucfirst(Engine_Api::_()->sitead()->truncation($item->title,30)) ?>
                	</div>
	                ...
	                <?php if($item->model_detail != -1): ?>
	                	<div style="width:10%;" class="admin_table_centered"><?php echo $item->model_detail ?></div>
	                <?php else:?>
	                  <div style="width:10%;" class="admin_table_centered" title="UNLIMITED"><?php echo $this->translate('UNLIMITED'); ?></div>
	                <?php endif; ?>
	                
	                <div style="width:10%;"><?php if ($item->isFree()) {
	                echo "Free"; ?>
	                <?php
	                  } else {
                 ...
                 <?php if($this->type == "default") : ?>   
	              ...
	              <?php if ($item->enabled == 1) {?>
	                <div style="width:8%;" class="admin_table_centered"> 
	                	<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'packagelist', 'action' => 'enabled', 'id' => $item->package_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitead/externals/images/enabled1.gif', '', array('title' => $this->translate('Disable Package')))) ?>
	                </div>
	              <?php } else {?>
	                <div style="width:8%;" class="admin_table_centered"> 
	                	<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'packagelist', 'action' => 'enabled', 'id' => $item->package_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitead/externals/images/enabled0.gif', '', array('title' => $this->translate('Enable Package')))) ?>
	                </div>
              	<?php } ?>
			
			
              	<div style="width:10%;"><?php echo $this->translate('%s', gmdate("M d, Y", strtotime($item->creation_date))); ?></div>
			
	              <div style="width:15%;" class="admin_table_centered">
	                <?php
	                echo $this->htmlLink(
	                        array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'packagelist', 'action' => 'edit', 'id' => $item->package_id),
	                        $this->translate('Edit')
	                ) ?>
	                |
  ...
  <form id='view_selected' method='post'  action='<?php  echo $this->url(array('module' => 'sitead', 'controller' => 'viewad', 'action' => 'index'),'admin_default',true);         
?>'>
