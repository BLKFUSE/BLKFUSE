    ...
    <thead>
      <tr>
        ...
        <th class="<?php echo $class ?>">
         ...
         <?php $class = ( $this->order == 'web_name' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->direction) : '' ) ?>
        <th class="<?php echo $class ?>">
          <a href="javascript:void(0);" onclick="javascript:changeOrder('web_name', 'ASC');">
        ...
        <?php if (!empty($this->sitecreditEnabled)): ?>
          <th style='width: 1%;' class='admin_table_centered'>
            <?php echo $this->translate("Credits Redeemed") ?>
          </th>
        <?php endif; ?>
      ...
      <?php foreach( $this->paginator as $item ):
        ...
        <tr>
          <td><?php echo $item->transaction_id ?></td>
           ...
           <td><?php  echo $this->htmlLink(
			array('route' => 'sitead_userad', 'ad_id' => $item->userad_id),
			 Engine_Api::_()->sitead()->truncation(ucfirst($item->web_name),25), array('target' => '_blank', 'title' => $this->translate(ucfirst($item->web_name)))); ?></td>
          ...
          ?>
            <td class='admin_table_centered'>
              <?php 
              echo $this->locale()->toCurrency((abs($item->credit_point)/$creditValue),Engine_Api::_()->getApi('settings', 'core')->getSetting('payment.currency', 'USD')); 
              echo $this->translate('(%s)', $item->currency); 
              ?>
            </td>
          <?php endif; ?>
          <td class='admin_table_centered'>
