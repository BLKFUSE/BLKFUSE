...
<?php
	...
	{
		echo '<ul class="form-notices" style="margin:0px;"><li style="float:left;">' . $this->translate('Successfully created') . ' ' . ucfirst($this->success_message) . ' ' . $this->translate('Sitead.') . '</li></ul>';
	}
  ...
  <table class='admin_table'>
      ...
      <tr>
        ...
        <th class='admin_table_short'>
					<?php 
						...
						echo "<a href=" . $this->url(array('module' => 'sitead', 'controller' => 'widgets', 'action' => 'adreports', 'idSorting' => $orderby), 'admin_default', true) . ">" . $this->translate("ID") . '</a>';
					?>
        </th>
        <th>
    ...
    <tbody>
        ...
        <tr>
          ...
          <td><?php if( !empty($item->web_name) ){ echo $item->web_name; } else { echo '-'; } ?></td>
          <td><?php echo $item->displayname; ?></td>
          <td><?php echo date('M d, Y', strtotime($item->creation_date)) ?></td>
					<td><?php if($item->report_type == 'Other'){ echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'widgets', 'action' => 'reportdiscription', 'reportId' => $item->adcancel_id), $this->translate($item->report_type), array('class' => 'smoothbox')); } else { echo $this->translate($item->report_type); } ?></td>
          <td>
          <?php 
						echo $this->htmlLink(array('route' => 'sitead_userad', 'ad_id' => $item->ad_id), $this->translate("view ad"), array('target' => '_blank')) . ' | '; 
						echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'viewad', 'action' => 'editad', 'id' => $item->ad_id), $this->translate("take action")) . ' | '; 
						echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitead', 'controller' => 'widgets', 'action' => 'deletereports', 'reportId' => $item->adcancel_id), $this->translate("dismiss"), array('class' => 'smoothbox'));
					?>
          </td>
        </tr>
      <?php endforeach; ?>
