	...
	<div class="tip" id ='tip2' style='display:none;'>
		...
		<span>
			<?php if (Engine_Api::_()->sitead()->enableCreateLink()): ?>
						<?php echo $this->translate('You have not created any Ad yet. %1$sCreate an ad%2$s to advertise on %3$s.', '<a href="'.$this->url(array(), 'sitead_listpackage', true). '">', '</a>', $site_title); ?>
					<?php endif; ?>
			
		</span>
	</div>
  <h3><?php echo $this->translate('Advertising Performance Reports') ?></h3>
  <p><?php echo $this->translate('You can view performance reports of your campaigns and ads over different time intervals.The generated reports include statistics like views, clicks and click through rate (CTR). You can also export and save the reports.') ?></p>
