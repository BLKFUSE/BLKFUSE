...
class Sitead_Widget_GetconnectionLinkController extends Engine_Content_Widget_Abstract 
{
  public function indexAction() 
  {
      ...
      return $this->setNoRender();
		...
		$enable_ads = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitead');
		if(!$enable_ads) {
			return $this->setNoRender();
		}
	  ...
	  $module_type = strtolower($module_name);
		...
		$ismoduleads_enabled = Engine_Api::_()->getDbtable('modules', 'sitead')->ismoduleads_enabled($module_type);
		if(!$ismoduleads_enabled) {
			return $this->setNoRender();
		}
    
    if(!empty($module_type) && ($module_type === "sitereview")){
      $tableObj = Engine_Api::_()->getDbtable('modules', 'sitead');
      $tableName = $tableObj->info('name');
     
      $select = $tableObj->select()->from($tableName, array('module_id'))->where('table_name =?', 'sitereview_listing_' . $module_subject->listingtype_id);
    ...
    if(!empty($module_type) && ($module_type === "sitereview")){
      ...
      if(!empty($ad_exist)) {
		...
		$info = Engine_Api::_()->getDbtable('modules', 'sitead')->getModuleInfo($module_type);
