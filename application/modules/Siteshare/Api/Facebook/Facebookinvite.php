...
class Siteshare_Api_Facebook_Facebookinvite extends Core_Api_Abstract
{
  ...
  public function getFBInstance()
  {
    ...
    return Engine_Api::_()->getApi('Facebook_Facebookinvite', 'siteshare')->getApi();
  }
  ...
  public function find_friends_added_thisapp()
  {
    ...
    $facebook = Siteshare_Api_Facebook_Facebookinvite::getFBInstance();
    try {
  ...
  public function facebook_invitefriend($var_fbconnect_invitef_redirect, $pageinvite_id = '', $moduletype = '', $resource_type = '')
  {
    ...
    $excluded_friends = Siteshare_Api_Facebook_Facebookinvite::find_friends_added_thisapp();
    ...
    if( !empty($moduletype) )
      $var_fbconnect_invitef_content = Siteshare_Api_Facebook_Facebookinvite::getInviteMessage($pageinvite_id, $moduletype, $resource_type);
    else
      $var_fbconnect_invitef_content = Siteshare_Api_Facebook_Facebookinvite::parseString($invite_mess);
    ...
    $callbackURL = $var_site_name . '/siteshare/auth/social-Signup/?type=facebook&refuser=' . $refuser;

    $content = Siteshare_Api_Facebook_Facebookinvite::parseString($var_fbconnect_invitef_content);
  ...
  public function checkConnection(User_Model_User $user = null, $fbapi)
  {
    ...
    try {
      ...
      if( (isset($session->aaf_fbaccess_token) && !empty($session->aaf_fbaccess_token) ) ) {
        ...
        $result = Siteshare_Api_Facebook_Facebookinvite::getAccessTokenFB($code);
  ...
  public function getApi()
  {
    ...
    if( empty($settings['secret']) ||
      ...
      $this->_api = null;
      Zend_Registry::set('Siteshare_Api_Facebook_Api', $this->_api);
      return false;
    }

    $this->_api = new Siteshare_Api_Facebook_Api(array(
  ...
  public function getAccessTokenFB($code, $callbackURL = '')
  {
    ...
    if( $callbackURL )
      ...
      $postString = "client_id=" . $settings['appid'] . "&client_secret=" . $settings['secret'] . "&redirect_uri=" . ( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getBaseUrl() . "/siteshare/auth/facebook&code=" . $code;
