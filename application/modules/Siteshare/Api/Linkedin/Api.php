...
class Siteshare_Api_Linkedin_Api { 
...
public function sendInvite ($friendsToJoin, $user_data = null, $pageinvite_id = null, $moduletype = null, $resource_type = null) { 
  ...
  $callbackURL = ( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getBaseUrl(). '/siteshare/auth/social-Signup/?type=linkedin&refuser=' . $viewer->getIdentity();
  }
  else 
    $callbackURL = Zend_Controller_Front::getInstance()->getBaseUrl(). '/siteshare/auth/social-Signup/?type=linkedin&refuser=' . $viewer->getIdentity();
  ...
  $Api_linkedin = Engine_Api::_()->getApi('linkedin_Api', 'siteshare');
...
//      }
   ...
   // $inviteOnlySetting = $settings->getSetting('user.signup.inviteonly', 0);
    if (is_array($friendsToJoin) && !empty($friendsToJoin) && !empty($pageinvite_id) ) {
      ...
      Siteshare_Api_Linkedin_Api::parseUserContacts ($friendsToJoin, $moduletype, $subject);
      ...
      foreach ($friendsToJoin as $recipient => $recipient_name) { 
        ...
        if (!empty($recipient)) {
          ...
          if (!isset($is_site_members[0]) || empty($is_site_members[0]->user_id)) { 
            ...
            $response = Siteshare_Api_Linkedin_Api::getBodyText (ucfirst($moduletype) . 'invite_User_Invite', $mailParams);
            ...
            if($response_linkedin['success'] === TRUE) {
              ...
              Siteshare_Api_Facebook_Facebookinvite::seacoreInvite($recipient_join, 'linkedin', $viewer);
            } 
          ...
          // IF THE PERSON IS A SITE MEMBER
            ...
            $response = Siteshare_Api_Linkedin_Api::getBodyText (strtoupper($moduletype) . 'INVITE_MEMBER_INVITE', $mailParams);
            //$recipient_join[$recipient] = $recipient_name;
            $response_linkedin =  $OBJ_linkedin->message(array('0' => $recipient), $response['subject'], $response['body'], FALSE);         
          }
      ...
      $response = Siteshare_Api_Linkedin_Api::getBodyText ('invite', $mailParams);
      $recipients = array();
      foreach ($friendsToJoin as $recipient => $recipient_name) {
      ...
      if($response_linkedin['success'] === TRUE ) {
        // message has been sent
        //saving the users info in database.
        Siteshare_Api_Facebook_Facebookinvite::seacoreInvite($friendsToJoin, 'linkedin', $viewer);
      } 
