...
class Siteshare_Api_Twitter_Api  { 
...
public function sendInvite ($friendsToJoin, $user_data = null, $pageinvite_id = null, $moduletype = '', $resource_type ='') { 
  ...
  $callbackURL = ( _ENGINE_SSL ? 'https://' : 'http://' ) . $HOST . Zend_Controller_Front::getInstance()->getBaseUrl(). '/siteshare/auth/social-Signup/?type=twitter&refuser=' . $viewer->getIdentity();
  
	if (!empty($appkey) && !empty($appsecret))
		$callbackURL = Engine_Api::_()->getApi('Bitly', 'siteshare')->get_bitly_short_url($callbackURL, $appkey, $appsecret, $format = 'txt');
 ...
 if (!empty($moduletype)) { 
   if ($pageinvite_id) {
    ...
    $inviteUrl = ( _ENGINE_SSL ? 'https://' : 'http://' )
            ...
            .$sitepage->getHref();
    if (!empty($appkey) && !empty($appsecret))
		$inviteUrl = Engine_Api::_()->getApi('Bitly', 'siteshare')->get_bitly_short_url($inviteUrl, $appkey, $appsecret, $format = 'txt');
  ...
  Siteshare_Api_Facebook_Facebookinvite::seacoreInvite($friendsToJoin, 'twitter', $viewer);
