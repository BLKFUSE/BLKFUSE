...
//     }
  ...
  public function addNewUserAction() {
    ...
    $db->beginTransaction();
    try {
      $isEmailExist = $userTable->isEmailExist($values['email']);
  ...
  public function saveUser($values, $defaultPhoto = null, $coverPhoto = null, $form = array()) {
    ...
    if(empty($values['first_name']) && empty($values['last_name'])) {
      //Display Name
      $firstNameFieldId = $this->getFieldId($profile_type, array('first_name'));
      $lastNameFieldId = $this->getFieldId($profile_type, array('last_name'));

      $first_name = Engine_Api::_()->user()->getprofileFieldValue(array('user_id' => $user_id, 'field_id' => $firstNameFieldId));
      $last_name = Engine_Api::_()->user()->getprofileFieldValue(array('user_id' => $user_id, 'field_id' => $lastNameFieldId));
    } else {
      $firstNameProfileId = $this->getFieldId($profile_type, array('first_name'));
      $lastNameProfileId = $this->getFieldId($profile_type, array('last_name'));
      $genderProfileId = $this->getFieldId($profile_type, array('gender'));
      $birthdayProfileId = $this->getFieldId($profile_type, array('birthdate'));
      ...
      if(engine_in_array($gender, array('Male', 'Female', 'Other'))) {
        $gender = Engine_Api::_()->user()->getOptionValue(array('field_id' => $genderProfileId, 'label' => $values['gender']));
      }
